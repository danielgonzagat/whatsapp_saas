...
export class NullableSerde<T extends Record<string, unknown>> {
  ...
  serialize(obj: T): Uint8Array {
    ...
    for (const field of this[SCHEMA].fields) {
      ...
      if (field.array && Array.isArray(val)) {
        ...
        for (const item of val) {
          if (field.nullable) {
            offset = this.writeNullableField(view, offset, field.type, item);
          } else {
            offset = this.writeField(view, offset, field.type, item);
          }
        }
      } else if (field.nullable) {
        offset = this.writeNullableField(view, offset, field.type, val);
      } else {
    ...
    for (const field of this[SCHEMA].fields) {
      if (field.array) {
        ...
        for (let i = 0; i < length; i++) {
          if (field.nullable) {
            const r = this.readNullableField(view, offset, field.type);
            arr.push(r.value);
            offset = r.nextOffset;
          } else {
  ...
  private writeNullableField(
    ...
    if (val == null) {
      ...
      return this.writeField(view, offset + 1, type, null);
    }
    view.setUint8(offset, 1);
    return this.writeField(view, offset + 1, type, val);
  }
  ...
  private readNullableField(
    view: DataView,
    offset: number,
    type: FieldType,
  ): { value: unknown; nextOffset: number } {
    const flag = view.getUint8(offset);
    const r = this.readField(view, offset + 1, type);
    if (flag === 0) {
      return { value: null, nextOffset: r.nextOffset };
    }
    return r;
  }
}
