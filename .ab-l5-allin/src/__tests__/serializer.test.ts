...
import { NullableSerde } from "../serializer/nullable-serde.js";
...
describe("BinarySerde", () => {
  ...
  describe('nullable fields', () => {
    it('serializes a nullable string as null (0x00) when undefined', () => {
      const schema: Schema = { fields: [{ name: 'opt', type: 'string', nullable: true }] };
      const serde = new NullableSerde<{ opt: string | null }>(schema);
      const buf = serde.serialize({ opt: null });
      assert.equal(buf.length, 3);
      assert.equal(buf[0], 0); // null flag = 0 (null)
      assert.equal(buf[1], 0); // length high
      assert.equal(buf[2], 0); // length low
    });

    it('serializes a nullable int32 with 0x01 flag when present', () => {
      const schema: Schema = { fields: [{ name: 'val', type: 'int32', nullable: true }] };
      const serde = new NullableSerde<{ val: number | null }>(schema);
      const buf = serde.serialize({ val: 42 });
      assert.equal(buf.length, 5);
      assert.equal(buf[0], 1); // null flag = 1 (not null)
    });

    it('round-trips nullable and non-nullable in same schema', () => {
      const schema: Schema = {
        fields: [
          { name: 'id', type: 'uint32' },
      ...
      const serde = new NullableSerde<{ id: number; email: string | null; name: string }>(schema);
      const original = { id: 1, email: null, name: 'Test' };
      const result = serde.deserialize(serde.serialize(original));
      assert.deepStrictEqual(result, original);
    });

    it('handles nullable int32 array', () => {
      const schema: Schema = { fields: [{ name: 'scores', type: 'int32', nullable: true, array: true }] };
      const serde = new NullableSerde<{ scores: (number | null)[] }>(schema);
      const original = { scores: [10, null, 30] };
      const result = serde.deserialize(serde.serialize(original));
      assert.deepStrictEqual(result, original);
    });
  });
