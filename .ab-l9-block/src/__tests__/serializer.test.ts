...
describe("BinarySerde", () => {
  ...
  describe("int64 / uint64 BigInt (RED - not yet implemented)", () => {
    it("round-trips a single int64 field as BigInt", () => {
      const schema: Schema = { fields: [{ name: "big", type: "int64" }] };
      const serde = new BinarySerde<{ big: bigint }>(schema);
      const original = { big: 9223372036854775807n };
      assert.deepStrictEqual(serde.deserialize(serde.serialize(original)), original);
    });

    it("round-trips a single uint64 field as BigInt", () => {
      const schema: Schema = { fields: [{ name: "big", type: "uint64" }] };
      const serde = new BinarySerde<{ big: bigint }>(schema);
      const original = { big: 18446744073709551615n };
      assert.deepStrictEqual(serde.deserialize(serde.serialize(original)), original);
    });

    it("round-trips int64 array", () => {
      const schema: Schema = { fields: [{ name: "vals", type: "int64", array: true }] };
      const serde = new BinarySerde<{ vals: bigint[] }>(schema);
      const original = { vals: [-1n, 0n, 1n, 9223372036854775807n] };
      assert.deepStrictEqual(serde.deserialize(serde.serialize(original)), original);
    });

    it("round-trips uint64 array", () => {
      const schema: Schema = { fields: [{ name: "vals", type: "uint64", array: true }] };
      const serde = new BinarySerde<{ vals: bigint[] }>(schema);
      const original = { vals: [0n, 1n, 18446744073709551615n] };
      assert.deepStrictEqual(serde.deserialize(serde.serialize(original)), original);
    });
  });
