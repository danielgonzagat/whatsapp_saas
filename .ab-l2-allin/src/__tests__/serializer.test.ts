...
describe("BinarySerde", () => {
  ...
  describe("round-trip", () => {
    ...
    it("handles nullable string with null value", () => {
      const schema: Schema = {
        fields: [{ name: "label", type: "string", nullable: true }],
      };
      const serde = new BinarySerde<{ label: string | null }>(schema);
      const buf = serde.serialize({ label: null });
      assert.equal(buf.length, 1);
      assert.equal(buf[0], 0);
      const result = serde.deserialize(buf) as { label: string | null };
      assert.equal(result.label, null);
    });

    it("handles nullable string with non-null value round-trip", () => {
      const schema: Schema = {
        fields: [{ name: "label", type: "string", nullable: true }],
      };
      const serde = new BinarySerde<{ label: string | null }>(schema);
      const original = { label: "Hello" };
      const result = serde.deserialize(serde.serialize(original)) as {
        label: string | null;
      };
      assert.deepStrictEqual(result, original);
    });

    it("handles nullable int32 with null value", () => {
      const schema: Schema = {
        fields: [{ name: "count", type: "int32", nullable: true }],
      };
      const serde = new BinarySerde<{ count: number | null }>(schema);
      const buf = serde.serialize({ count: null });
      assert.equal(buf.length, 1);
      assert.equal(buf[0], 0);
      const result = serde.deserialize(buf) as { count: number | null };
      assert.equal(result.count, null);
    });

    it("handles nullable int32 with non-null value round-trip", () => {
      const schema: Schema = {
        fields: [{ name: "count", type: "int32", nullable: true }],
      };
      const serde = new BinarySerde<{ count: number | null }>(schema);
    ...
    it("handles mixed nullable and non-nullable fields", () => {
      ...
      const original = { id: 1, nickname: null, score: 99.5 };
      const result = serde.deserialize(serde.serialize(original)) as typeof original;
      assert.deepStrictEqual(result, original);
    });
  });
