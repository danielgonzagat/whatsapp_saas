/**
 * Part 1: Internal helpers for consistency checking.
 */
...
export function isPulseSourcePath(filePath: string): boolean {
...
export function isPulseArtifactPath(filePath: string): boolean {
  const segments = pathSegments(filePath);
...
export function isMachineEvidencePath(filePath: string): boolean {
...
export function isPulseMachineUnit(unit: DirectiveUnitView): boolean {
...
export function isProductPrioritizedUnit(unit: DirectiveUnitView): boolean {
  if (isPulseMachineUnit(unit)) return false;
  if (String(unit.productImpact || '').toLowerCase() === 'machine') return false;
...
export function addProofDebtSignal(
  ...
  if (
    value === 'NAO' ||
    value === false ||
    discoverGateFailureClassLabels().has(String(value ?? ''))
  ) {
    signals.push({ source, field, value });
  }
}

export function collectMissingEvidenceGateSignals(
  signals: ProofDebtSignal[],
  source: string,
  value: unknown,
  prefix: string,
...
export function uniqueSources(values: string[]): string[] {
