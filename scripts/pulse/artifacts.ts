...
function buildReport(snapshot: PulseArtifactSnapshot): string {
  ...
  lines.push('');
  lines.push(`- Resolved modules: ${resolvedManifest.summary.resolvedModules}/${resolvedManifest.summary.totalModules}`);
  lines.push(`- Resolved flow groups: ${resolvedManifest.summary.resolvedFlowGroups}/${resolvedManifest.summary.totalFlowGroups}`);
  ...
  for (const tier of certification.tierStatus) {
    lines.push(`| ${tier.id} | ${tier.name} | ${tier.status.toUpperCase()} | ${tier.blockingGates.length > 0 ? tier.blockingGates.join(', ') : '—'} | ${tableCell(tier.reason, 160)} |`);
  }
  lines.push('');
  ...
  lines.push(`- Observability: ${tableCell(certification.evidenceSummary.observability.summary, 320)}`);
  ...
  lines.push(`- Customer scenarios: ${certification.evidenceSummary.customer.passed.length}/${certification.evidenceSummary.customer.declared.length} passed`);
  lines.push(`- Operator scenarios: ${certification.evidenceSummary.operator.passed.length}/${certification.evidenceSummary.operator.declared.length} passed`);
  lines.push(`- Admin scenarios: ${certification.evidenceSummary.admin.passed.length}/${certification.evidenceSummary.admin.declared.length} passed`);
  lines.push(`- Soak scenarios: ${certification.evidenceSummary.soak.passed.length}/${certification.evidenceSummary.soak.declared.length} passed`);
  lines.push('');
...
function buildMatrix(snapshot: PulseArtifactSnapshot): string {
  ...
  lines.push('');
  lines.push(`> Updated: ${certification.timestamp.slice(0, 10)} | Status: ${certification.status} | Score: ${certification.score}% | BREAKS: ${health.breaks.length} | MODULES(resolved): ${resolvedManifest.summary.totalModules}`);
  lines.push('');
  ...
  for (const moduleEntry of resolvedManifest.modules) {
    lines.push(`| **${moduleEntry.name}** | ${moduleEntry.moduleKind.toUpperCase()} | ${moduleEntry.state} | ${moduleEntry.critical ? 'YES' : 'NO'} | ${moduleEntry.resolution.toUpperCase()} | ${moduleEntry.pageCount} | ${tableCell(moduleEntry.routeRoots.join(', '), 80)} | ${moduleEntry.sourceModule || moduleEntry.legacySource || 'CODEBASE'} |`);
  }
  ...
  lines.push('');
  lines.push('| Flow Group | Kind | Resolution | Critical | Members | Module Scope | Matched Spec |');
  lines.push('|------------|------|------------|----------|---------|--------------|--------------|');
  for (const flowGroup of resolvedManifest.flowGroups.slice(0, 40)) {
    lines.push(`| ${flowGroup.id} | ${flowGroup.flowKind.toUpperCase()} | ${flowGroup.resolution.toUpperCase()} | ${flowGroup.critical ? 'YES' : 'NO'} | ${flowGroup.memberCount} | ${tableCell(flowGroup.moduleNames.join(', '), 60)} | ${flowGroup.matchedFlowSpec || '—'} |`);
  }
  ...
  if ((manifest?.legacyModules || []).length === 0) {
    lines.push('| **No legacy modules declared** | READY | The resolved manifest is already the only product taxonomy in scope |');
  }
  lines.push('');
  ...
  const summary = resolvedManifest.modules.reduce<Record<PulseModuleState, number>>((acc, moduleEntry) => {
...
function buildProductMap(snapshot: PulseArtifactSnapshot): string {
  ...
  lines.push('|--------|------|-------|----------|-------|--------|-------|');
  for (const moduleEntry of resolvedManifest.modules) {
    lines.push(`| **${moduleEntry.name}** | ${moduleEntry.moduleKind.toUpperCase()} | ${moduleEntry.state} | ${moduleEntry.critical ? 'YES' : 'NO'} | ${moduleEntry.pageCount} | ${tableCell(moduleEntry.routeRoots.join(', '), 80)} | ${tableCell(moduleEntry.notes, 160)} |`);
  }
  lines.push('');
  ...
  for (const flowGroup of resolvedManifest.flowGroups) {
    lines.push(`| ${flowGroup.id} | ${flowGroup.flowKind.toUpperCase()} | ${flowGroup.resolution.toUpperCase()} | ${tableCell(flowGroup.moduleNames.join(', '), 80)} | ${tableCell(flowGroup.pageRoutes.join(', '), 80)} | ${tableCell(flowGroup.endpoints.join(', '), 120)} |`);
  }
  lines.push('');
  ...
  lines.push(`- Unresolved flow groups: ${resolvedManifest.diagnostics.unresolvedFlowGroups.length}`);
  lines.push(`- Shared capability groups: ${resolvedManifest.diagnostics.sharedCapabilityGroups.length}`);
  lines.push(`- Grouped semantic flow groups: ${resolvedManifest.diagnostics.groupedFlowGroups.length}`);
  ...
  lines.push(`- Covered user-facing pages: ${certification.evidenceSummary.syntheticCoverage.coveredPages}/${certification.evidenceSummary.syntheticCoverage.userFacingPages}`);
  return lines.join('\n');
}
...
export function generateArtifacts(snapshot: PulseArtifactSnapshot, rootDir: string): PulseArtifactPaths {
  ...
  fs.writeFileSync(invariantEvidencePath, JSON.stringify(snapshot.certification.evidenceSummary.invariants, null, 2));
  fs.writeFileSync(observabilityEvidencePath, JSON.stringify(snapshot.certification.evidenceSummary.observability, null, 2));
  fs.writeFileSync(recoveryEvidencePath, JSON.stringify(snapshot.certification.evidenceSummary.recovery, null, 2));
  fs.writeFileSync(customerEvidencePath, JSON.stringify(snapshot.certification.evidenceSummary.customer, null, 2));
  fs.writeFileSync(operatorEvidencePath, JSON.stringify(snapshot.certification.evidenceSummary.operator, null, 2));
  fs.writeFileSync(adminEvidencePath, JSON.stringify(snapshot.certification.evidenceSummary.admin, null, 2));
