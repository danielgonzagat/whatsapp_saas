/**
 * PULSE API Contract & Fuzz Probe Engine — Payload Synthesizers
 *
 * Generates synthetic payloads from endpoint route/schema surfaces for
...
export function buildMassAssignmentPayloads(endpoint: APIEndpointProbe): unknown[] {
  ...
  if (fieldEntries.length === 0) {
    ...
    const baseName =
      routeParameters[deriveZeroValue()] ?? endpoint.path.replace(/\W+/g, '_') ?? 'payload';
    return [
      {
