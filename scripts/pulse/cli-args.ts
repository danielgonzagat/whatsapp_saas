/**
 * CLI argument parsing and flag derivation for the PULSE index entry point.
 * All exports are pure constants or pure functions derived from process.argv.
 */
...
export const inferredSyntheticModes = new Set<PulseSyntheticRunMode>(
  [
...
export const requestedSyntheticModes = [...inferredSyntheticModes];
export const queryModeRequested = flags.guidance || flags.prove || flags.vision || flags.selfTrust;
export const actorModeRequested = requestedSyntheticModes.length > 0;

export function deriveEffectiveTarget() {
  if (flags.profile) return getProfileSelection(flags.profile).certificationTarget;
  return { tier: flags.tier, final: flags.final, profile: null };
}

export function deriveEffectiveEnvironment() {
  if (flags.profile) return getProfileSelection(flags.profile).environment;
  if (flags.total) return 'total' as const;
  if (flags.deep) return 'deep' as const;
  return 'scan' as const;
}

export function compactReason(value: string, max: number = 500): string {
  const compact = value.replace(/\s+/g, ' ').trim();
  if (compact.length <= max) return compact;
  return `${compact.slice(0, max - 3)}...`;
}

export function deriveBrowserEvidenceFromActors(
  actorMode: boolean,
  browserEvidence: PulseBrowserEvidence,
  syntheticEvidence: ReturnType<typeof runSyntheticActors>,
): PulseBrowserEvidence {
  if (!actorMode || browserEvidence.executed) return browserEvidence;
  ...
  if (actorResults.length === 0) return browserEvidence;
