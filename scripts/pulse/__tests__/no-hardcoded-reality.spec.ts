...
function replaySession(overrides: Partial<ReplaySession> = {}): ReplaySession {
  ...
  return {
    ...
    durationMs: deriveHttpStatusFromObservedCatalog('OK') + deriveHttpStatusFromObservedCatalog('OK') + deriveHttpStatusFromObservedCatalog('OK') + deriveHttpStatusFromObservedCatalog('OK') + deriveHttpStatusFromObservedCatalog('OK'),
    url: '/opaque',
    events: [],
...
describe('PULSE no-hardcoded-reality contracts', () => {
  ...
  it('does not scan arbitrary roots when scripts/pulse is absent', () => {
    ...
    expect(auditPulseNoHardcodedReality(rootDir)).toEqual({
      scannedFiles: deriveZeroValue(),
      findings: [],
      predicates: [],
      summary: {
        totalFindings: deriveZeroValue(),
        byKind: {},
        topFiles: [],
        totalPredicates: deriveZeroValue(),
        byPredicateKind: {},
      },
    });
  });
