...
import type { computeCertification } from '../../certification';
import { PATH_PROOF_TASKS_ARTIFACT } from '../../path-proof-evidence';
...
export function writeProofReadinessSummary(
  rootDir: string,
...
export function writeJson(rootDir: string, relativePath: string, payload: unknown): void {
  const artifactPath = path.join(rootDir, relativePath);
  fs.mkdirSync(path.dirname(artifactPath), { recursive: true });
  fs.writeFileSync(artifactPath, `${JSON.stringify(payload, null, 2)}\n`);
}

export function makePathProofTask(): PathProofTask {
  return {
...
export function makePathProofPlan(tasks: PathProofTask[]): PathProofPlan {
  return {
    generatedAt: '2026-04-29T23:00:00.000Z',
    summary: {
...
export function baseInput(rootDir: string): CertificationInput {
