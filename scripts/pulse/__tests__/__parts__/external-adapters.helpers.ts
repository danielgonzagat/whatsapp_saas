...
export type LegacyAdapterStatus =
  | 'ready'
  | 'not_available'
  | 'stale'
  | 'invalid'
...
export interface LegacyConvergenceCycleInput {
  cycleId: string;
  timestamp: string;
  status: 'completed';
  score: number;
...
export type LegacyConvergenceCycle = PulseAutonomyIterationRecord & LegacyConvergenceCycleInput;

export interface LegacyConvergenceState {
  history: LegacyConvergenceCycle[];
}

export type LiveExternalSourceInput = Omit<
...
export function legacyCycle(input: LegacyConvergenceCycleInput): LegacyConvergenceCycle {
...
export function evaluateLegacyConvergenceGate(autonomyState: LegacyConvergenceState) {
...
export function buildLiveExternalState(
  sources: LiveExternalSourceInput[],
