...
import {
  ...
  bestStatus,
  buildCapabilityCompletion,
  buildSurfaceBlockers,
  chooseTruthMode,
  compact,
  deriveStateSequence,
  deriveUnitValue,
  deriveWeakestCapabilityStatus,
  deriveZeroValue,
  hasCount,
  hasItems,
  humanize,
  isExcludedCoverageStatus,
  isMaterializedState,
  leadingSpan,
  mergeModules,
  multiple,
  ...
  truthModeAspirationalLabel,
  truthModeInferredLabel,
  unique,
  unitHitsModule,
  weakestState,
} from './helpers';
...
export function buildProductVision(input: BuildProductVisionInput): PulseProductVision {
    ...
    .map((entry) => {
      ...
      return {
        ...
        truthMode: chooseTruthMode([
          ...
          !hasItems(unitHits) && !hasItems(runHits)
            ? truthModeAspirationalLabel()
            : truthModeInferredLabel(),
        ]),
  ...
  const experiences = input.resolvedManifest.scenarioSpecs
    ...
    .map((scenario) => {
      ...
      const status: PulseFlowProjectionStatus =
        stateFromCompletion(completion, flowSeq) ??
        flowWeak ??
        (deriveWeakestCapabilityStatus() as PulseCapabilityStatus);
      ...
      const blockers = unique([
        ...
        ...experienceSurfaces.flatMap((surface) =>
          surface.blockers.slice(
            deriveZeroValue(),
            Math.max(deriveUnitValue(), surface.blockers.length),
          ),
        ),
        ...experienceRuns
  ...
  const leadingBlockers = unique([
    ...
    deriveZeroValue(),
    leadingSpan(blockerSpan, promiseToProductionDelta.criticalGaps.length),
  );
