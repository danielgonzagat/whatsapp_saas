...
import { buildGovernedValidationEvidence } from './authority';
...
export function buildAutonomyProof(
  ...
  const nextStepAutonomy = Boolean(
    firstUnit &&
    firstUnit.validationArtifacts.length > deriveZeroValue() &&
    firstUnit.exitCriteria.length > deriveZeroValue(),
  );
  ...
  return {
    ...
    criteria: [
      ...
      },
      {
        id: 'runtime_reality',
        status:
          snapshot.certification.gates.runtimePass.status === GATE_PASS ? GATE_PASS : GATE_FAIL,
