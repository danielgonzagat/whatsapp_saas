  ...
  for (const scenario of manifest.scenarioSpecs) {
    ...
    for (const groupId of scenario.flowGroups) {
      ...
      synthesized.push({
        ...
        pageRoutes: unique(scenarioRoutes.length > 0 ? scenarioRoutes : scenario.routePatterns).sort(),
        actions: [],
        endpoints: [],
        backendRoutes: [],
        connected: false,
...
export function buildResolvedManifest(
  ...
  const flowGroups = [...resolvedFlowGroups, ...synthesizeScenarioFlowGroups(manifest, codebaseTruth, resolvedFlowGroups)]
    .sort((a, b) => a.id.localeCompare(b.id));
