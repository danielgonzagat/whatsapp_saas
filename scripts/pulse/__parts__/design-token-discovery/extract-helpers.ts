...
import type { DiscoveredDesignColorEvidence, DesignTokenSourceKind } from './types';

export function extractHexColorValues(content: string): Array<{ value: string; index: number }> {
  ...
  while (cursor < content.length) {
    ...
    if (index === -1) break;
    let end = index + 1;
    while (isHexChar(content[end])) end += 1;
...
export function extractFunctionColorValues(
  content: string,
...
export function extractColorValues(content: string): Array<{ value: string; index: number }> {
  const matches: Array<{ value: string; index: number }> = [];
...
export function extractCssVariableDeclarations(
  content: string,
  ...
  while (cursor < content.length) {
    ...
    if (start === -1) break;
    let nameEnd = start + 2;
    while (isIdentifierChar(content[nameEnd])) nameEnd += 1;
    ...
    if (tokenName && content[colonIndex] === ':') {
      ...
      while (valueStart < content.length && content[valueStart] === ' ') valueStart += 1;
...
export function extractCssVariableColors(
  content: string,
  sourcePath: string,
...
export function extractTokenSourceColors(
  content: string,
  sourcePath: string,
...
export function isTailwindConfig(relativePath: string): boolean {
...
export function isTokenFile(relativePath: string): boolean {
...
export function isThemeFile(relativePath: string): boolean {
  return normalizeRepoPath(relativePath)
    .toLowerCase()
    .split('/')
    .some((part) => {
...
export function isComponentPrimitiveStyleFile(relativePath: string): boolean {
...
export function classifySource(relativePath: string): DesignTokenSourceKind[] {
  const kinds: DesignTokenSourceKind[] = [];
  if (isTailwindConfig(relativePath)) kinds.push('tailwind-config');
  if (isTokenFile(relativePath)) kinds.push('token-file');
  if (isThemeFile(relativePath)) kinds.push('theme-file');
  if (isComponentPrimitiveStyleFile(relativePath)) kinds.push('component-primitive-style');
  return kinds;
}
