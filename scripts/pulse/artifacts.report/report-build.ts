...
export function buildReport(
  ...
  const selfTrustPass = isGateStatusPass(
    snapshot.certification.gates.pulseSelfTrustPass?.status ?? '',
  );
  const noOverclaimPass = isGateStatusPass(
    snapshot.certification.gates.noOverclaimPass?.status ?? '',
  );
  ...
  const gitnexusStatus =
    gitnexusSignals.length > deriveZeroValue()
      ? gitnexusSignals[deriveZeroValue()].summary
      : 'not configured';
  ...
  lines.push('');
  if (
    snapshot.productVision.surfaces &&
    snapshot.productVision.surfaces.length > deriveZeroValue()
  ) {
  ...
  if (
    snapshot.productVision.experiences &&
    snapshot.productVision.experiences.length > deriveZeroValue()
  ) {
