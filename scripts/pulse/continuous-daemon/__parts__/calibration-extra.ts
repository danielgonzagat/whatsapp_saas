/**
 * Part 3: calibration-extra — evidence deficits, kind/risk priority,
...
function mapCountsToCalibration(
  ...
  for (let [key, count] of Object.entries(counts)) {
    result[key] = {
      value: Math.max(calibrationFloor(count), Math.round((count / max) * Math.max(...values))),
      source,
      detail,
    };
  }
