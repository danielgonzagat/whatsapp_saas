...
function isAnimationContext(lines: string[], idx: number): boolean {
  ...
  if (isAnimationFile) return true;

  return (
    /useEffect|requestAnimationFrame|canvas|ctx\.|\.getContext|animation|animate|transition|keyframe/i.test(context) ||
      ...
      try {
        ...
        for (let i = 0; i < lines.length; i++) {
          ...
          if (/setTimeout\s*\(\s*\(\)\s*=>\s*set\w+\s*\(/.test(trimmed)) {
            ...
            // Clipboard feedback
            if (/clipboard|navigator\.clipboard|copyToClipboard|setCopied/i.test(context5)) continue;
            // UI state timers (thinking, loading indicators, toast auto-dismiss, animation triggers)
            if (/setIsThinking|setIsLoading|setIsProcessing|setShowToast|setNotification/i.test(trimmed)) continue;
            ...
            for (let j = i; j >= Math.max(0, i - 50); j--) {
              if (/(?:const|let|function|async function)\s+\w+\s*(?:=\s*(?:async\s+)?(?:\([^)]*\)\s*=>|\(\))|(?:\s*\())/.test(lines[j])) {
                funcStart = j;
                break;
              }
            }
            ...
            for (let j = funcStart; j < Math.min(funcStart + 60, lines.length); j++) {
              ...
              if (bodyStarted && depth === 0) { funcEnd = j + 1; break; }
            }

            const funcBody = lines.slice(funcStart, funcEnd).join('\n');
            // Check for direct API calls OR hook-provided mutation functions
            const hasApiCall = /apiFetch|await\s+fetch|api\.\w+|\.mutate\s*\(|Api\.\w+/.test(funcBody) ||
          ...
          if (/Math\.random\(\)/.test(trimmed)) {
            ...
            if (isAnimationContext(lines, i)) continue;
            if (isIdContext(lines, i)) continue;
            // ID generation: Math.round(Math.random() * 1e9), Math.random().toString(36)
            if (/\*\s*1e\d|\*\s*1000000|\.toString\s*\(\s*36\s*\)/.test(trimmed)) continue;
            // Retry jitter: Math.random() * delay or Math.random() * baseDelay
            if (/\*\s*(?:baseDelay|delay|timeout|interval|retryDelay|backoff)/i.test(trimmed)) continue;
          ...
          if (/useState\s*\(\s*\[/.test(trimmed)) {
            ...
            if (/\{\s*(?:q|label|name|title|text)\s*:\s*['"`]/.test(block) && /\}\s*,\s*\{/.test(block)) {
              facades.push({
                file: relFile,
                line: i + 1,
                type: 'hardcoded_data',
                severity: 'high',
                description: 'useState initialized with hardcoded data array that should come from backend',
                evidence: trimmed.slice(0, 120),
              });
            }
          }
          ...
          if (/FALLBACK_RESPONSES|fallbackResponses|FALLBACK_MESSAGES/i.test(trimmed)) {
            if (!isAnimationContext(lines, i)) {
              facades.push({
                ...
                type: 'hardcoded_data',
                severity: 'high',
                description: 'Hardcoded fallback responses used instead of real AI/backend responses',
                evidence: trimmed.slice(0, 120),
              });
            }
          }
          ...
          if (/setInterval\s*\(/.test(trimmed)) {
            ...
            if (/set\w+\s*\(\s*(?:prev|p|v)\s*=>\s*(?:prev|p|v)\s*[\+\-]/.test(block) ||
          ...
          if (relFile.includes('backend') && /\.service\.ts$/.test(file)) {
            if (/^\s*return\s+\[\s*\]\s*;?\s*$/.test(trimmed) ||
              ...
              if (!/catch|default|fallback|if\s*\(!|normalize|sanitize|safeParse|JSON\.parse/.test(context10)) {
                facades.push({
                  file: relFile,
                  line: i + 1,
                  type: 'hardcoded_data',
        ...
        process.stderr.write(`  [warn] Could not scan facades in ${file}: ${(e as Error).message}\n`);
      }
    }
  }
