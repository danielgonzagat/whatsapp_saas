...
const EMAIL_SEND_RE = /sendMail|sendEmail|transporter\.send|sgMail\.send|resend\.emails|ses\.send|mailer\./i;
...
export function checkCommunication(config: PulseConfig): Break[] {
  ...
  for (const file of backendFiles) {
    ...
    if (EMAIL_SEND_RE.test(content)) hasEmailSending = true;
    if (DKIM_RE.test(content)) hasDKIM = true;
    if (EMAIL_SERVICE_RE.test(content)) hasEmailService = true;
    if (SALE_NOTIFICATION_RE.test(content)) hasSaleNotification = true;
    if (PAYMENT_WEBHOOK_RE.test(content)) hasPaymentWebhookHandler = true;

    if (/welcome|bem.vindo|boas.vindas/i.test(content) && EMAIL_SEND_RE.test(content)) hasWelcomeEmail = true;
    if (/reset.*password|password.*reset|redefinir.*senha/i.test(content) && EMAIL_SEND_RE.test(content)) hasPasswordResetEmail = true;
    if (/order.*confirm|confirm.*order|pedido.*confirm|pagamento.*confirm/i.test(content) && EMAIL_SEND_RE.test(content)) hasOrderConfirmEmail = true;
    if (/payment.*fail|fail.*payment|pagamento.*falhou|cobrança.*falh/i.test(content) && EMAIL_SEND_RE.test(content)) hasPaymentFailEmail = true;
  }
  ...
  if (hasEmailSending) {
    ...
    if (!hasDKIM && !hasDKIMEnv) {
      breaks.push({
        ...
        file: 'backend/src/',
        line: 0,
        description: 'Email sent without DKIM configuration — emails will be marked as spam by major providers',
        detail: 'Configure DKIM_PRIVATE_KEY env var and add DKIM signing to email transport; also set SPF and DMARC DNS records',
      });
    }
    ...
    if (!hasEmailService) {
      breaks.push({
        ...
        file: 'backend/src/',
        line: 0,
        description: 'No dedicated email service (SendGrid/Resend/SES) detected — raw SMTP has poor deliverability',
        detail: 'Use Resend or SendGrid for transactional email; they handle DKIM, bounce handling, and deliverability',
      });
    }
  } else {
    ...
    breaks.push({
      ...
      file: 'backend/src/',
      line: 0,
      description: 'No email sending found in backend — transactional emails (welcome, reset, receipt) not implemented',
      detail: 'Add email sending for: welcome, password reset, order confirmation, payment failure, trial expiry',
    });
  }

  // CHECK 2: Push notifications
  const hasPWAManifest = fs.existsSync(path.join(config.frontendDir, 'public', 'manifest.json')) ||
  ...
  const hasPushImpl = /FCM|fcm|webPush|pushNotif|service.worker|serviceWorker/i.test(
    walkFiles(config.backendDir, ['.ts']).reduce((acc, f) => {
      try { return acc + fs.readFileSync(f, 'utf8'); } catch { return acc; }
    }, '')
  );
  ...
  if (hasPWAManifest && !hasPushImpl) {
    breaks.push({
      ...
      line: 0,
      description: 'PWA manifest exists but push notifications are not implemented — key retention feature missing',
      detail: 'Implement Web Push (FCM) for sale alerts, message notifications, and trial reminders',
    });
  }
  ...
  if (hasPaymentWebhookHandler && !hasSaleNotification) {
    breaks.push({
      ...
      file: 'backend/src/',
      line: 0,
      description: 'Payment webhook processed without notifying workspace owner of sale — seller does not know when they earn money',
      detail: 'After PAYMENT_CONFIRMED webhook, send email + WhatsApp notification to workspace owner with sale details',
    });
  }

  // CHECK 4: Transactional email coverage
  const missingEmails: string[] = [];
  if (!hasWelcomeEmail) missingEmails.push('welcome/onboarding');
  if (!hasPasswordResetEmail) missingEmails.push('password reset');
  if (!hasOrderConfirmEmail) missingEmails.push('order confirmation for buyer');
  if (!hasPaymentFailEmail) missingEmails.push('payment failure');
  ...
  if (hasEmailSending && missingEmails.length > 0) {
    breaks.push({
      ...
      detail: 'Implement the missing email templates; each is critical for user experience and trust',
    });
  }
  ...
  for (const file of marketingEmailFiles) {
    ...
    if (EMAIL_SEND_RE.test(content) && !/unsubscribe|descadastrar|optOut|opt.out/i.test(content)) {
      breaks.push({
        ...
        detail: 'Add unsubscribe link to all marketing emails; track opt-outs in DB and honor them immediately',
      });
    }
  }
