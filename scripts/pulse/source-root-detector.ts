...
function inferKind(relativePath: string, packageName: string | null): SourceRootKind {
  const haystack = `${relativePath} ${packageName ?? ''}`.toLowerCase();
  if (haystack.includes('frontend') || haystack.includes('web') || haystack.includes('next')) {
    return 'frontend';
  }
  if (haystack.includes('backend') || haystack.includes('server') || haystack.includes('api')) {
    return 'backend';
  }
...
function addRoot(
  ...
  evidence: string,
  evidenceBasis: SourceRootEvidenceBasis,
  options: { weakCandidate?: boolean } = {},
): void {
  const normalized = normalizeRelative(relativePath);
  if (hasSkippedSegment(normalized)) return;
  const weakCandidate = options.weakCandidate === true;
  if (!normalized || (!weakCandidate && !hasSourceFiles(rootDir, normalized))) return;
  ...
  roots.set(normalized, {
    ...
    kind: inferKind(normalized, packageName),
    packageName,
    evidence: [evidence],
    evidenceBasis: [evidenceBasis],
    weakCandidate,
    languageExtensions: languageExtensionsFor(rootDir, normalized),
  });
}
...
function addFileEvidenceRoots(roots: Map<string, DetectedSourceRoot>, rootDir: string): void {
  ...
  for (const relativePath of candidates) {
    addRoot(roots, rootDir, relativePath, null, 'file-evidence:source-files', 'file-evidence');
  }
}
