/**
 * Coverage Sidecar Emitter — helpers: constants, parsers, normalizers,
 * path helpers, coverage collection, and tag injection.
 *
...
export function findCoverageFiles(dir) {
...
export function toCoverageEntry(raw) {
...
export function sidecarRepoPath(repoRel) {
  return join(SOURCE_MIRROR_DIR, repoRel + SIDECAR_SUFFIX);
}

export function mirrorRelPath(repoRel) {
  return repoRel + '.md';
}
...
export function findExistingSidecars(root) {
  const out = [];
...
// ── Below-threshold tag injection ───────────────────────────────────────────

export function handleBelowTag(relMirror, mirrorAbs, dry, threshold, pct) {
