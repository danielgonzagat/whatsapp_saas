...
export const MODE_DRY = args.includes('--dry');
export const MODE_JSON = args.includes('--json');
export const MODE_FIX_ORPHANS = args.includes('--fix-orphans');
const categoryArgIdx = args.indexOf('--category');
export const CATEGORY_FILTER = categoryArgIdx >= 0 ? args[categoryArgIdx + 1]?.toUpperCase() : null;
