...
async function partA(): Promise<void> {
  ...
  // replace_range: 'foo' literal -> null  (the thesis example, by range)
  {
    ...
    check("range swap produces null", r.newText === "const phone = null;\n", JSON.stringify(r.newText));
...
async function partB(): Promise<void> {
  ...
  try {
    ...
    const okSha = (await client.callTool({
      ...
      arguments: { file: fixtureRel, module: "./z", name: "Zed", expectedSha256: sha(cur), preview: true },
    })) as { content: { text: string }[] };
    check("sha guard passes on correct hash", JSON.parse(okSha.content[0].text).ok === true, okSha.content[0].text);
...
async function partC(): Promise<void> {
  ...
  // edit_symbol insert_after
  {
    ...
    check(
      ...
      r.validation.ok && r.newText.includes("export const VERSION = 1;") && r.newText.includes("function helper"),
  ...
  // edit_symbol remove
  {
    ...
    check(
      ...
      r.validation.ok && !r.newText.includes("function helper") && r.newText.includes("class UserService"),
      r.newText,
    );
  }
