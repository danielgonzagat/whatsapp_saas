...
export async function partBSetup(ctx: PartBCtx): Promise<void> {
  ...
  const names = tools.tools.map((t: { name: string }) => t.name).sort();
    check(
      'server lists all 55 tools (incl. universal native engine: atomic_ast_search + atomic_ast_edit + atomic_native_status + atomic_create_file + atomic_delete_file + code_file_stat + analyzer transaction + product apex layer + rename property key + add await to call + insert after anchor + insert before anchor + replace between anchors + replace text in anchor region + atomic_edit unified router + code_outline_batch)',
      names.length === 55 &&
