...
export async function renameSymbolCrossFile(
  ...
  if (ownerTypeName) {
    ...
    const renameKeyIn = (obj: unknown): void => {
      ...
      for (const prop of o.getProperties?.() ?? []) {
        const p = prop as {
          getKind?: () => number;
          getNameNode?: () =>
            | { getText?: () => string; replaceWithText?: (t: string) => void }
            | undefined;
        };
        const pk = p.getKind?.();
        if (
          pk !== K.PropertyAssignment &&
          pk !== K.MethodDeclaration &&
          pk !== K.GetAccessor &&
          pk !== K.SetAccessor
        )
          continue;
        ...
        const q =
          raw.length > 1 && (raw[0] === "'" || raw[0] === '"' || raw[0] === '`') ? raw[0] : '';
    ...
    const factoryObj = (init: unknown): unknown => {
      ...
      const body = f?.getBody?.() as
        | { getKind?: () => number; getExpression?: () => unknown; getStatements?: () => unknown[] }
        | undefined;
      ...
      if (bk === K.Block) {
        for (const st of body?.getStatements?.() ?? []) {
          ...
          if (s.getKind?.() === K.ReturnStatement) {
            const e = s.getExpression?.() as
              | { getKind?: () => number; getExpression?: () => unknown }
              | undefined;
    ...
    const renameAccessesOfVar = (nameNode: unknown): void => {
      ...
      let refs: unknown[] = [];
      try {
        refs = n?.findReferencesAsNodes?.() ?? [];
      } catch {
        return;
      }
      for (const ref of refs) {
        const r = ref as { getParent?: () => unknown };
        const parent = r.getParent?.() as
          | {
              getKind?: () => number;
      ...
      try {
        for (const obj of f.getDescendantsOfKind(K.ObjectLiteralExpression)) {
          const o = obj as unknown as {
            getProperty?: (n: string) => { getInitializer?: () => unknown } | undefined;
          };
          const provideVal = unwrap(o.getProperty?.('provide')?.getInitializer?.()) as
            | { getText?: () => string }
            | undefined;
          if (provideVal?.getText?.() !== ownerTypeName) continue;
          const useValue = unwrap(o.getProperty?.('useValue')?.getInitializer?.()) as
            | {
                getKind?: () => number;
                getSymbol?: () =>
                  | { getValueDeclaration?: () => { getInitializer?: () => unknown } | undefined }
                  | undefined;
              }
            | undefined;
          if (useValue?.getKind?.() === K.ObjectLiteralExpression) renameKeyIn(useValue);
          else if (useValue?.getKind?.() === K.Identifier) {
            const vdecl = useValue.getSymbol?.()?.getValueDeclaration?.() as
              | { getInitializer?: () => unknown; getNameNode?: () => unknown }
              | undefined;
        ...
        for (const v of f.getDescendantsOfKind(K.VariableDeclaration)) {
          ...
          if (
            tn !== ownerTypeName &&
            !new RegExp(`\\b(?:Partial|Pick|Record|Mocked)\\s*<\\s*${ownerTypeName}\\b`).test(tn)
          )
            continue;
          const init = vd.getInitializer?.();
          if (
            (init as { getKind?: () => number } | undefined)?.getKind?.() ===
        ...
        /* never let coverage break the validated rename */
      }
    }
  }
...
export async function renameMemberCrossFile(
  ...
  if (!nameNode?.getStart) {
    throw new Error(
      `member "${memberName}" not found on ${className} in ${path.basename(absFile)}`,
    );
  }
