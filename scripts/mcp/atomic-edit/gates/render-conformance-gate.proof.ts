...
import gate from './render-conformance-gate.js';
...
try {
  ...
  // ============================================================
  {
    ...
    const res = gate.run(ctx) as ReturnType<typeof gate.run> & { reds: { fact: string }[] };
    const r = res as { green: boolean; reds: { fact: string }[]; unjudged?: boolean };
  ...
  // ============================================================
  {
    ...
    const overlay = new Map<string, string>([[rel, newText]]);
    const ctx = makeContext(repoRoot, overlay, [rel]);
    const r = gate.run(ctx) as { green: boolean; reds: { fact: string }[]; unjudged?: boolean };
  ...
  // ============================================================
  {
    ...
    const r = gate.run(ctx) as { green: boolean; reds: unknown[]; unjudged?: boolean };
