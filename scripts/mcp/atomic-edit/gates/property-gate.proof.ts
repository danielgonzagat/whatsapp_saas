 ...
 * transaction — snapshot → write an EPHEMERAL sibling driver → run it (a REAL
 * `npx tsx` child process, twice, no mock) → revert (unlink driver) — then asserts:
 *
 ...
 *               counterexample over array(int) inputs.
...
async function main(): Promise<void> {
  ...
  {
    const file =
      '// @property fn=absNonNeg invariant=result >= 0 gen=int runs=300\n' +
      'export function absNonNeg(x: number): number { return Math.abs(x); }\n';
    const root = mkrepo({ 'good.ts': file });
    const ctx = makeContext(root, new Map(), ['good.ts']);
    ...
    expectCleanTree(root, 'good.ts', file);
  ...
  {
    const file =
      '// @property fn=dbl invariant=result > input gen=int runs=500\n' +
      'export function dbl(x: number): number { return x * 2; }\n';
    const root = mkrepo({ 'bug.ts': file });
    const ctx = makeContext(root, new Map(), ['bug.ts']);
    const r = (await gate.run(ctx)) as GateResult;
    show('CASE 2 (double(x) > x is false for x<=0)', r);
    expect(!r.green && !r.unjudged, 'a violated invariant is RED');
    expect(r.reds.some((x) => x.file === 'bug.ts' && /counterexample/i.test(x.fact)), 'red names file + states a counterexample');
    ...
    expectCleanTree(root, 'bug.ts', file);
    fs.rmSync(root, { recursive: true, force: true });
  }
  ...
  {
    const file =
      '// @property fn=mysort invariant=result.length === input.length gen=array(int) runs=400\n' +
      'export function mysort(a: number[]): number[] {\n' +
      '  return Array.from(new Set(a)).sort((x, y) => x - y);\n' + // drops duplicates → length shrinks
      '}\n';
    const root = mkrepo({ 'sort.ts': file });
    const ctx = makeContext(root, new Map(), ['sort.ts']);
    const r = (await gate.run(ctx)) as GateResult;
    ...
    expectCleanTree(root, 'sort.ts', file);
    fs.rmSync(root, { recursive: true, force: true });
  }
  ...
  // ── CASE 3b: GREEN — the SAME shape, with a correct implementation ──
  {
    const file =
      ...
      '}\n';
    const root = mkrepo({ 'sortok.ts': file });
    const ctx = makeContext(root, new Map(), ['sortok.ts']);
    const r = (await gate.run(ctx)) as GateResult;
    show('CASE 3b (correct sort preserves length)', r);
    expect(r.green && !r.unjudged, 'a correct array property converges GREEN');
    expectCleanTree(root, 'sortok.ts', file);
    fs.rmSync(root, { recursive: true, force: true });
  }
  ...
  {
    const file =
      '// @property fn=nd invariant=result === 0 gen=int runs=200\n' +
      'export function nd(_x: number): number { return Date.now() + Math.floor(Math.random() * 1e9); }\n';
    const root = mkrepo({ 'nondet.ts': file });
    const ctx = makeContext(root, new Map(), ['nondet.ts']);
    const r = (await gate.run(ctx)) as GateResult;
    ...
    expectCleanTree(root, 'nondet.ts', file);
    fs.rmSync(root, { recursive: true, force: true });
  }
  ...
  {
    const file =
      '// @property fn=notThere invariant=result === 1 gen=int runs=50\n' +
      'export const notThere = 42;\n'; // exists, but not callable
    const root = mkrepo({ 'noexport.ts': file });
    const ctx = makeContext(root, new Map(), ['noexport.ts']);
    const r = (await gate.run(ctx)) as GateResult;
    ...
    expectCleanTree(root, 'noexport.ts', file);
    fs.rmSync(root, { recursive: true, force: true });
  }
  ...
  // ── CASE 6: TOKEN-CORRECT no-op at the GATE level — directive only in a string ──
  {
    const file =
      ...
      'export function real(x: number): number { return x; }\n';
    const root = mkrepo({ 'stringonly.ts': file });
    const ctx = makeContext(root, new Map(), ['stringonly.ts']);
    const r = (await gate.run(ctx)) as GateResult;
  ...
  {
    const file = 'export const plain = (n: number): number => n + 1;\n';
  ...
  {
    const file =
      '// @property fn=add invariant=result === input[1] + input[0] gen=int, int runs=200\n' +
      'export function add(a: number, b: number): number { return a + b; }\n';
    const root = mkrepo({ 'add.ts': file });
    const ctx = makeContext(root, new Map(), ['add.ts']);
    const r = (await gate.run(ctx)) as GateResult;
    show('CASE 8 (multi-arg add commutativity)', r);
    expect(r.green && !r.unjudged, 'a true multi-arg property converges GREEN');
    expectCleanTree(root, 'add.ts', file);
    fs.rmSync(root, { recursive: true, force: true });
  }
