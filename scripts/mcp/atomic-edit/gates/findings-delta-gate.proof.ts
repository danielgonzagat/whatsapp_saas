...
try {
  ...
  const r1 = gate.run(ctx1) as ReturnType<typeof gate.run> & { reds: { file: string; locus?: string; fact: string }[] };
  const res1 = r1 as { green: boolean; reds: { file: string; locus?: string; fact: string }[]; unjudged?: boolean };
  ...
  const res2 = gate.run(ctx2) as { green: boolean; reds: unknown[] };
  ...
  const res3 = gate.run(ctx3) as { green: boolean; reds: { fact: string }[] };
  ...
  const res4 = gate.run(ctx4) as { green: boolean; reds: unknown[] };
  ...
  const res5 = gate.run(ctx5) as { green: boolean; unjudged?: boolean };
  ...
  //          (proves the literal-blanking, i.e. token-correct perception).
  // ─────────────────────────────────────────────────────────────────────────
  const f6 = 'src/e.ts';
  ...
  const res6 = gate.run(ctx6) as { green: boolean; reds: unknown[] };
  ...
  const res7 = gate.run(ctx7) as { green: boolean; reds: { fact: string }[] };
