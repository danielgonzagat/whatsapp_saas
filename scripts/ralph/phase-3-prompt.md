...
5. Validation gate (every step must pass before commit):
   - Backend file:
     `npm --prefix backend run typecheck && npm --prefix backend test \
      -- --testPathPattern=<related> --runInBand && \
...
6. If green:
   - Non-sensitive:
     `git fetch origin main && git rebase origin/main && \
      git add <file> && \
      git commit -m "refactor(codacy): type debt in <path>" && git push`
   - Sensitive:
     `gh pr create --title "refactor(codacy): type debt in <path>" \
      --body "Phase 3 Ralph iteration. Validation green." \
      --label auto-merge`
