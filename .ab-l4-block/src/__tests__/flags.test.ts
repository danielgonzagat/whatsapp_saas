...
describe('FeatureFlagStore', () => {
  ...
  it('time_window rule: matches within window [startHour, endHour)', () => {
    const store = new FeatureFlagStore();
    store.setFlag('peakHours', true, [{
      type: 'time_window',
      config: { startHour: 9, endHour: 17 },
    }]);
    assert.strictEqual(store.isEnabled('peakHours', { currentHour: 12 }), true);
    assert.strictEqual(store.isEnabled('peakHours', { currentHour: 8 }), false);
    assert.strictEqual(store.isEnabled('peakHours', { currentHour: 17 }), false);
  });

  it('time_window rule: inclusive start excludes end', () => {
    const store = new FeatureFlagStore();
    store.setFlag('allDay', true, [{
      type: 'time_window',
      config: { startHour: 0, endHour: 24 },
    }]);
    assert.strictEqual(store.isEnabled('allDay', { currentHour: 0 }), true);
    assert.strictEqual(store.isEnabled('allDay', { currentHour: 23 }), true);
    assert.strictEqual(store.isEnabled('allDay', { currentHour: 24 }), false);
  });

  it('time_window rule: no context defaults to no match', () => {
    const store = new FeatureFlagStore();
    store.setFlag('window', true, [{
      type: 'time_window',
      config: { startHour: 9, endHour: 17 },
    }]);
    assert.strictEqual(store.isEnabled('window'), false);
  });
});

  describe('hash_based rule', () => {
    it('same user always gets the same result', () => {
      const store = new FeatureFlagStore();
      store.setFlag('hashBeta', true, [{ type: 'hash_based', config: { percentage: 50 } }]);
      const ctx: FlagContext = { userId: 'consistent-user' };
      const first = store.isEnabled('hashBeta', ctx);
      for (let i = 0; i < 20; i++) {
        assert.strictEqual(store.isEnabled('hashBeta', ctx), first);
      }
    });
    it('0% excludes all users', () => {
      const store = new FeatureFlagStore();
      store.setFlag('off', true, [{ type: 'hash_based', config: { percentage: 0 } }]);
      for (let i = 1; i <= 100; i++) {
        assert.strictEqual(store.isEnabled('off', { userId: `user${i}` }), false);
      }
    });
    it('100% includes all users', () => {
      const store = new FeatureFlagStore();
      store.setFlag('all', true, [{ type: 'hash_based', config: { percentage: 100 } }]);
      for (let i = 1001; i <= 1100; i++) {
        assert.strictEqual(store.isEnabled('all', { userId: `user${i}` }), true);
      }
    });
    it('50% splits users across buckets', () => {
      const store = new FeatureFlagStore();
      store.setFlag('split50', true, [{ type: 'hash_based', config: { percentage: 50 } }]);
      const results = new Set<boolean>();
      for (let i = 1; i <= 500; i++) {
        results.add(store.isEnabled('split50', { userId: `user${i}` }));
      }
      assert.strictEqual(results.has(true), true);
      assert.strictEqual(results.has(false), true);
    });
    it('no context defaults to no match', () => {
      const store = new FeatureFlagStore();
      store.setFlag('noCtx', true, [{ type: 'hash_based', config: { percentage: 50 } }]);
      assert.strictEqual(store.isEnabled('noCtx'), false);
    });
  });
