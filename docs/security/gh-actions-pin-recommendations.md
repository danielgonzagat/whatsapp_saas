...
> **Status:** Recommendation (ci-cd.yml is a protected file) **Semgrep rule:**
...
repository can silently replace a tag's target, injecting malicious code into
every workflow that references it. Pinning to a full commit SHA makes the
reference immutable and auditable.
...
after the SHA preserves the human-readable version for Dependabot and reviewers.
...
update GitHub Actions so pinned SHAs stay current:
