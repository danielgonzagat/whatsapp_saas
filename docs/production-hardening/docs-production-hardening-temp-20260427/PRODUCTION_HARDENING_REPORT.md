...
### 1. Codacy HIGH Issues: 1116 Total
...
Backup frequency 1440 min exceeds RPO target of 60 min. Up to 1440 min of financial data at risk.

### 4. Race Conditions - Read-Modify-Write Without Transactions

8 critical race conditions in billing-webhook.service.ts, checkout-catalog.service.ts, checkout-product.service.ts, checkout-social-recovery.service.ts. Write-after-read without `$transaction` or optimistic lock.
...
checkout-order-query.service.ts:281, reports-orders.service.ts:128/157/193 - payment status set to PAID without verifying PROCESSING intermediate state.

### 7. Financial Error Swallowing

payment-webhook-stripe.handlers2.ts:252 - catch block does not rethrow.
...
Payment webhook error handlers lack Sentry/Datadog alerts. Outbound HTTP calls lack correlation ID headers. No Datadog monitors configured.

### 9. Transactions Without Isolation Levels (14 instances)

ledger.service.ts (5x), payment-webhook-stripe handlers (7x), ledger-adjustments.helper.ts (1x). `$transaction()` used without `isolationLevel: 'Serializable'` in financial code.

### 10. Autopilot No Rate Limits (COST_LLM_NO_LIMIT)

6 services send WhatsApp messages without per-workspace daily limit.

### 11. Data Consistency Gaps

billing.service.ts:191 - `createCheckoutSession` without plan existence validation. wallet.service.ts:270 - `requestWithdrawal` without referenced entity existence check.
...
E2E specs exist but not included in GitHub Actions CI pipeline.

### 14. SSR-Unsafe Browser Access (4 instances)

PixelTracker.tsx:143, MarketingView.tsx:43/47, facebook-sdk.ts:30 - document/window access at module scope crashes SSR.

### 15. Cron Jobs Without Error Handling (5 instances)

checkout-social-recovery, followup, marketplace-treasury-maturation, connect-ledger-maturation, whatsapp-watchdog - `@Cron()` methods without try/catch.

### 16. Fetch Without Timeout

url-safety.ts:191 - fetch() without AbortController.

### 17. JSON.parse Without Try/Catch

kloel.autonomy-proof.helpers.ts:162 - JSON.parse outside try/catch.

### 18. LGPD Compliance - No Unsubscribe

marketing-connect.controller.ts - marketing emails without unsubscribe link.

### 19. Missing Transactional Emails

No welcome/onboarding email templates.
...
Unlabeled input elements in Dashboard, WhatsAppExperience, ProductNerveCenter, ProdutosView, ProductAfterPayTab.

### 22. Dead UI Handlers (5 instances)

Unused/wired click handlers in kloel-auth-screen, chat-container, ProductAfterPayTab, ProductIATab.

### 23. Browser Compatibility Issues (2 instances)

CSS without @supports fallback; root layout missing viewport meta.

### 24. Storage Quota Missing

File uploads without per-workspace storage quota.
...
moderate-product.dto.ts - @IsArray without @ArrayMaxSize.
