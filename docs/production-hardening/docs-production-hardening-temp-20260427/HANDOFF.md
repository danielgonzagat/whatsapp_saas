...
6. Production hardening documentation created (`docs/production-hardening/`):
...
1. **Reduce Codacy HIGH issues from 1116 → target threshold**
   - Critical file: `backend/prisma/migrations/20251209150035_init_baseline/migration.sql` (37 HIGH — RAC table access rules)
   - Critical file: `backend/src/auth/email.service.ts` (2 HIGH — HTML template string + missing template string indicator)
   - Critical file: `backend/src/autopilot/autopilot.service.ts` (1 HIGH — missing template string indicator, line 878)
   - Protected file: `package.json` (1 HIGH — dependency hijack; human-required)
...
3. **Fix race conditions in checkout/billing**
   ...
   - Wrap read-modify-write in `$transaction()` or add optimistic locking.
...
4. **Add idempotency to financial endpoints**
   ...
   - Accept `X-Idempotency-Key` header; deduplicate via Redis/DB.

5. **Fix payment state machine transitions**
   - `checkout-order-query.service.ts:281`, `reports-orders.service.ts:128/157/193`
   - Enforce PENDING → PROCESSING → PAID; never jump directly.
...
7. **Add observability**
   - Wire Sentry/Datadog exception capture in payment webhook error handlers.
   - Add X-Request-ID to all outbound HTTP calls.
   - Configure Datadog monitors for financial services.

8. **Add transaction isolation levels**
   - `ledger.service.ts` (5x), `payment-webhook-stripe.handlers.ts` (7x), `ledger-adjustments.helper.ts`
   - Add `isolationLevel: 'Serializable'` to all financial `$transaction()` calls.

9. **Add Autopilot rate limits**
   - Implement per-workspace daily message counter.
   - Enforce plan-based limits (e.g., 1000/day free plan).

10. **Collect customer journey evidence**
    - customer-auth-shell, customer-product-and-checkout, customer-whatsapp-and-inbox
    - Requires real HTTP/Playwright/DB execution.

11. **Collect operator/admin/soak scenario evidence**
    - Operator: Autopilot Run, Campaigns and Flows.
    - Admin: Settings KYC Banking.
    - Soak: all 8 scenarios.
    - Run `npx tsx scripts/pulse/pulse.ts` with `--runtime` mode.
...
17. Add unsubscribe links to marketing emails.
