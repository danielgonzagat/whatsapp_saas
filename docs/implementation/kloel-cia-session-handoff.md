...
Next action: continue the V23 anti-fake scan on runtime/product surfaces with repo evidence, prioritizing business-state `localStorage` and remaining placeholder/fake runtime strings that are not tests or design placeholders. Leave `AGENTS.md` and untracked OpenCode prompt drafts out of CIA commits unless explicitly brought into the current slice.
