...
| Row #39 — local `normalizePhone` / `digitsOnly` re-exports across 6+ modules (`checkout/checkout-social-lead.util`, `worker/providers/checkout-social-lead-enrichment`, `autopilot/autopilot-ops.service`, `whatsapp/inbound-processor.helpers`, `whatsapp/whatsapp-catchup-history.service`, `whatsapp/whatsapp-normalization.util`, `marketing/channels/whatsapp/whatsapp-digits.util`, `meta/meta-whatsapp.service`, `prisma/checkout-paid-effects/whatsapp`) | `backend/src/common/phone/phone-normalization.util.ts` exporting `extractAsciiDigits`, `normalizePhone` (structured `NormalizedPhone`), `extractPhoneFromChatId`, `phonesMatch`. Each migrated caller keeps a thin local shim delegating to the canonical util so existing return-type contracts (`string` vs `string \| null`) survive. | 2026-07-01 | IN PROGRESS — Wave 1 landed canonical util + spec + migrated `checkout-order-support` and `prisma/checkout-paid-effects/whatsapp`. Wave 2 (this PR) migrates `checkout/checkout-social-lead.util` (`digitsOrNull → extractAsciiDigits`) and `autopilot/autopilot-ops.service` (`digitsOnly → extractAsciiDigits`); cumulative count 5/10 backend callers. `worker/providers/checkout-social-lead-enrichment.ts` deferred — worker is a separate deploy unit that does not import from backend at runtime (see `worker/redis-client.ts` precedent), so it needs either a duplicated worker-local util or a future shared package before its `normalizePhone` can migrate. |
