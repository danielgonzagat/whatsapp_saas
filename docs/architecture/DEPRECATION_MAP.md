...
| #10 `backend/src/meta/instagram/` (InstagramService + InstagramController) | `backend/src/marketing/channels/instagram/` | Wave W4 (verified empty) | MIGRATED 2026-05-27 — files moved via `git mv` (instagram.service, instagram.service.spec, instagram.controller), consumers updated (meta.module, kloel/channel-transport.providers + spec, marketing/instagram/instagram-marketing.service, marketing channels dispatch adapter now local), HTTP routes (`meta/instagram/*`) preserved, re-export stubs left at old paths with `@deprecated` JSDoc banner per ADR-0012 W3 |
