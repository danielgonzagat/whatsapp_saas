...
Total cross-file duplicates: 280. Sorted by severity (most-duplicated first).
...
| `extractProductName` | 3 | `backend/src/checkout/checkout-payment.helpers.ts`<br>`backend/src/kloel/guest-chat.action-intent.helpers.ts`<br>`backend/src/kloel/guest-chat.product-args.helpers.ts` |
...
| `asString` | 2 | `backend/src/common/types.ts`<br>`backend/src/kloel/kloel-tool-dispatcher.helpers.ts` |
...
| `mapRecentConversation` | 2 | `backend/src/dashboard/dashboard.recent-conversations.helpers.ts`<br>`frontend/src/components/kloel/search/command-palette-utils.ts` |
