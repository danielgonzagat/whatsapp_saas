...
| `Contact` | `Lead` (as person), `Customer` (as person), `Client`/`Cliente` (as person), `Prospect`, `User` (in messaging), `buyer` | `Lead` only as funnel-stage label (`KloelLead`, CRM pipeline stage); `Customer` only as denormalized snapshot fields on `CheckoutOrder.customer*`; `Client`/`Cliente` only as `@prisma/client` import, `resolveClientIp`/`extractClientIp` infra, or the platform-tenant admin view (`AdminClientsService` → `prisma.workspace`, NOT a person); `buyer` is legacy and must migrate (see `mercado-pago-pix.service.ts`); `Prospect` has **0 code occurrences** (doc-comment only). Evidence (backend/src, tool-measured 2026-05-29): `Lead`=75 occ, `Contact`=58 occ, `Customer`=49 occ, `Client`=26 occ, `Cliente`=82 occ, `Prospect`=0 occ | `backend/prisma/schema.prisma` — `model Contact` (table `RAC_Contact`, has documented `Lead → Contact provenance bridge`: `kloelLeadId`/`scrapingJobId`/`checkoutSocialLeadId` nullable pointers) |
...
| `Message` (channel message) | `KloelMessage`, `MindMessage`, `ChatMessage`, `FbMessage`, `PartnerMessage`, `AdminChatMessage` | Each is a separate Prisma model with a distinct lifecycle: `KloelMessage` (`RAC_KloelMessage`) = legacy Mind/Brain runtime log; **`MindMessage` (`RAC_MindMessage`) is now a real model** (post-Wave-4, schema line 3827) and is the canonical cognitive-log table — `KloelMessage` is being backfilled into it (3 direct callers remain, see §6.1); `ChatMessage` = admin/dashboard chat thread message; `FbMessage` = Facebook DM ingest; `PartnerMessage` = partner-side comms; `AdminChatMessage` = staff chat. `Message` canonical applies only to customer channel messages tied to `Conversation` | `backend/prisma/schema.prisma` — `model Message`, `model MindMessage`, `model KloelMessage` |
| `ChannelSession` | `whatsappSession`, `waSession`, `whatsappApiSession`, `connection` (session), `instance` (session), `botSession`, `WAHASession` | `whatsappApiSession` is the live JSON field name on `Workspace.providerSettings` — acceptable at data layer; `WhatsappSessionService` is the canonical NestJS service name at code layer; `connection` allowed only for TCP/socket-level talk; `instance` allowed only for process-lock semantics (watchdog); `waSession` is local-variable legacy and must migrate; `botSession` / `WAHASession` are dead aliases (no current code references) | runtime via `backend/src/marketing/channels/whatsapp/whatsapp-session.service.ts` (moved post-Wave-4 — `backend/src/kloel/whatsapp/` no longer exists; frontend proxy `/api/whatsapp-api/*` retained for URL stability); data via `providerSettings.whatsappApiSession` on `model Workspace` |
...
| Canonical | Deprecated aliases | When deprecated still allowed | Backend reference file |
|---|---|---|---|
| `ChannelMessageDispatchService` (OmniCore) | `MessageDispatchService` (WhatsApp-only), `WahaService.sendMessage`, `WhatsappApiService.sendText`, `MessageWorker.process` (direct) | All allowed as provider-internal hops; new product code MUST go through the OmniCore `ChannelMessageDispatchService.dispatch(workspaceId, channel, to, message, options)`. The old WhatsApp-only `sendMessage` canonical was superseded post-Wave-4 by cross-channel dispatch over `ChannelDispatchRegistry`/`ChannelDispatchPort` | `backend/src/marketing/channel-message-dispatch.service.ts` (canonical); `backend/src/common/channel-dispatch/channel-dispatch.{registry,port}.ts`; `backend/src/inbox/**` |
...
| `Mind*` (cognitive prefix) | `Brain*`, `AI*`, `ML*`, `Intelligence*` | `Brain*` allowed only on legacy services that still have a `Mind*` re-export shim — see `DEPRECATION_MAP.md` items #2-#10 (10 services, ADR-0013 +4wk deadline) plus the load-bearing `Brain*` policy/event types kept indefinitely; after the deadline all migratable `Brain*` paths fail the gate. The standalone `Brain*Service` files are dissolved — surviving `Brain*` symbols are now DTOs/types/one controller inside `backend/src/kloel/mind/coordination/**`. See Section 6 for full status | `backend/src/kloel/mind/**` (canonical); `backend/src/admin/brain/mind-audit.controller.ts` (class already renamed `MindAuditController`; folder + route URL kept) and `backend/src/kloel/whatsapp-brain.controller.ts` (`WhatsAppBrainController`, route `kloel/whatsapp` — legacy alive) |
...
| Platform tenant / paying account on KLOEL | `Workspace` (admin label "Client") | `Contact` | The admin panel `AdminClientsService` (`backend/src/admin/clients/`) calls its rows "Client", but the data source is `prisma.workspace` — a **tenant account on the KLOEL platform**, NOT a CRM person. `Client` here is a UI label for `Workspace`, never an entity. Bug class: confusing platform-Client (workspace) with workspace-Client (a Contact in that workspace's CRM). |
...
- **Cognitive engine namespace**: `Mind*` is the canonical prefix for ALL
  cognitive-loop services. **16 live Prisma models** carry the prefix with the
  `RAC_Mind*` table prefix (tool-verified `grep -cE '^model Mind' schema.prisma`
  → 16, 2026-05-29): `MindBelief`, `MindPrediction`, `MindPolicy`,
  `MindWorkspaceState`, `MindCase`, `MindConceptDetection`, `MindGraphNode`,
  `MindGraphEdge`, `MindOutboxEvent`, `MindBanditArm`, `MindGuardAudit`,
  `MindDailyReport`, `MindGlobalPrior`, `MindSelfModel`, `MindMessage`,
  `MindMemory`. (Up from 13 at Wave 54 — `MindSelfModel`, `MindMessage`, and
  `MindMemory` landed in the post-Wave-4 unification work.)
...
- **`MindMessage` is canonical**: `backend/src/kloel/mind/aliases/mind-message.service.ts`
  exposes `MindMessageService`. As of the post-Wave-4 work there is now a
  **real `model MindMessage`** Prisma model (`RAC_MindMessage`, schema line 3827)
  alongside the legacy `KloelMessage` (`RAC_KloelMessage`). New code MUST call
  `MindMessageService`. **Migration is NOT complete**: 3 legacy direct
  `prisma.kloelMessage.*` call sites remain outside the wrapper —
  `backend/src/kloel/kloel-conversation-store.ts`,
  `backend/src/kloel/kloel.service.ts`,
  `backend/src/kloel/state/conversation-state.types.ts` (tool-verified
  2026-05-29). These must route through `MindMessageService` before the
  `KloelMessage` table can be retired.

### 6.2 What is deprecated-but-still-live

- **`KloelMessage` Prisma table** (`RAC_KloelMessage`) is the legacy storage.
  **Update (post-Wave-4):** `model MindMessage` (`RAC_MindMessage`) now exists
  in the schema (line 3827) — PR-5 of `BRAIN_MIND_UNIFICATION_PLAN.md` has
  partially landed. The two tables coexist during the backfill window;
  `KloelMessage` remains permitted at the data layer until the 3 remaining
  direct `prisma.kloelMessage.*` callers (see §6.1) are migrated to
  `MindMessageService` and a row backfill completes.
...
## 9. Gate status (Wave 54 baseline; post-Wave-4 refresh 2026-05-29)
...
- Breakdown of remaining soft warnings:
  ...
  - `Webhook` aliases — 9
- Brain→Mind unification (post-Wave-4 status, tool-verified 2026-05-29):
  - **Phase 1 (alias)** active. Standalone `Brain*Service` files dissolved;
    surviving `Brain*` symbols are DTOs/types/one controller inside
    `kloel/mind/coordination/**` (290 `Brain` occurrences across 53 non-spec
    backend files, mostly load-bearing policy/event types + legacy `brain.*`
    metric/audit literals frozen by design).
  - **Phase 2 (table backfill)** PARTIALLY landed: `model MindMessage`
    (`RAC_MindMessage`) and `model MindMemory` (`RAC_MindMemory`) now exist
    alongside legacy `KloelMessage`/`KloelMemory`. 3 direct
    `prisma.kloelMessage.*` callers remain (see §6.1) before retirement.
  - **Phase 3 (`Brain*` removal)** gated on ADR-0013 +4wk deadline.
  - `BrainAuditController` already renamed → `MindAuditController`
    (`admin/brain/mind-audit.controller.ts`); only folder + route URL kept.
- Mind canonical surface: **16 Prisma models live** (was 13 — added
  `MindSelfModel`, `MindMessage`, `MindMemory`), **10 Mind coordinator
  services live**, **2 Mind alias services live**
  (`MindMessageService`, `MindMemoryItemService`).
- prismaAny: **~0 production usages** — `grep prismaAny backend/src` returns
  only 3 hits, all inside one spec file
  (`checkout-post-payment-effects.service.spec.ts`), none in production code.
- Legacy folders dissolved (tool-verified): `backend/src/whatsapp/` (→
  `marketing/channels/whatsapp/`), `backend/src/ai-brain/`,
  `backend/src/brain/`, `backend/src/cia/`, `backend/src/kloel/hypproof/`.
