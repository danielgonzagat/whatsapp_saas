...
Use this exact high-level atomic operation first:
...
That operation is the atomic intent: read symbol, create helper, add import and
remove the original symbol through atomic MCP operations. Do not decompose it
unless that command fails.
