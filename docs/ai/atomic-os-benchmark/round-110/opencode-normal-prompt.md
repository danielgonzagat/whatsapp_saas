You are the NORMAL OpenCode lane in A/B Round 110.
...
shell commands, but you must not use Atomic OS tooling. Round 109 passed this
task in both lanes, but NORMAL lost operationally by `max_timeout`; this round
repeats the same complexity before any scale-up. Do not create the known
runtime helper defect: unused `ToolArgs` in
