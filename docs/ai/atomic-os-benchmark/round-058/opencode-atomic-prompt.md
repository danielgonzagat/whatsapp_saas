...
- If direct MCP path handling is unclear, use:
  ...
  worktree, refuses path escape, and accepts aliases `filePath`, `specifier`
  and `action`.
...
- Shell mutation paths: redirection, `tee`, `sed -i`, `perl -pi`,
  Node/Python/Ruby writers, native patching. Heredoc is allowed only to pass
  JSON arguments to `atomic-call.cjs`, never to write files.
...
- Do not perform cosmetic blank-line cleanup unless `git diff --check` or
  TypeScript/Jest proves it is necessary.
...
- Remove the local helper without changing call sites.
