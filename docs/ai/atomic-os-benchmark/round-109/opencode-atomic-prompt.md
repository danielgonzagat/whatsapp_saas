You are the ATOMIC OpenCode lane in A/B Round 109.
...
only. Round 107 confirmed stability at the single-helper tier; this round
repeats the Round 108 multi-module helper split until the Atomic lane passes
all gates. Round 108 failed because
`unified-agent-runtime.helpers.ts` imported unused `ToolArgs`; do not recreate
that defect.
