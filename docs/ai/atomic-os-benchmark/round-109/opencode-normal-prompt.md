You are the NORMAL OpenCode lane in A/B Round 109.
...
shell commands, but you must not use Atomic OS tooling. Round 108 exposed a
real failure mode in this task: unused `ToolArgs` in
`unified-agent-runtime.helpers.ts`. Do not create that defect.
