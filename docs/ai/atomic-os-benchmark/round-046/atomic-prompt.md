...
Worktree: `/private/tmp/kloel-ab046-atomic-20260516233536`.
...
- Every MCP `file`, `dir`, `cwd`, and `allowedPaths` argument must be absolute
  and under `/private/tmp/kloel-ab046-atomic-20260516233536`.
...
- Every created/modified `backend/src/kloel/unified-agent*.ts` file must end at
  `<=320` lines. This is a benchmark target; split helpers if needed.
- Do not add `as any`, ts-ignore/expect-error/nocheck, lint/Codacy suppressions,
  `NOSONAR`, or `noqa`.
...
- Extract tiny shared/runtime helpers to `unified-agent-runtime.helpers.ts` or
  `unified-agent.shared.ts`; avoid one 350+ line helper.
- Do not rediscover governance docs; this prompt is the operating contract.
- Read minimally: one outline/read of the service, then mutate. Avoid full-diff
  proof loops; use validation commands for proof.
...
8. Run:
   `node /Users/danielpenin/whatsapp_saas/docs/ai/atomic-os-benchmark/tools/trace-isolation-check.cjs --worktree /private/tmp/kloel-ab046-atomic-20260516233536 --coordinator /Users/danielpenin/whatsapp_saas --since '<your start time>' --json`
