You are the NORMAL OpenCode lane in A/B Round 118.

Worktree:
`/Users/danielpenin/kloel-ab-worktrees/kloel-ab118-normal-20260518070102`
...
Task:
Split `backend/src/kloel/unified-agent.service.ts` into exactly these four
helpers:
...
- Service imports helper functions and keeps public `async executeTool(` and
...
- `npm --prefix backend run typecheck` (report global pre-existing errors
  separately from touched Kloel errors)
