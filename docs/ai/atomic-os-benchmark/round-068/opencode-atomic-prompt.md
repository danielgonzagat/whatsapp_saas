...
command before or after it. When invoking it through the OpenCode bash tool, set
the bash tool timeout to `300000` milliseconds; the command string itself must
remain exactly this:
...
typecheck, diff-check, protected diff and forbidden suppression scan. The
operator accepts the OpenCode shell-escaped JSON form and is idempotent if a
previous attempt already completed the mutation.
