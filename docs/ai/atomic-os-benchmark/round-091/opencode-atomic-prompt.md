You are the ATOMIC OpenCode lane in A/B Round 091.

Worktree:
`/private/tmp/kloel-ab091-atomic-20260517182930`

Mission:
Repeat Round 090 at the same complexity after the semantic lint-fix regression; prove the layout-only Atomic formatter no longer changes unrelated casts. Extract the
...
Command:
```sh
cd /private/tmp/kloel-ab091-atomic-20260517182930 && ARGS=$(node - <<'NODE'
