...
`package.json` is governance-protected, so this document is the source of truth for the new production-readiness surface added in this branch.
...
These are not application runtime variables. They are only consumed by `scripts/smoke-test-prod.ts`.
