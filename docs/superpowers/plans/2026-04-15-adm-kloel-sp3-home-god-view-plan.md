...
`resolveAdminHomeRange(period, from?, to?, compareTo?)` that returns a canonical
`{ from, to, previousFrom, previousTo }` object. Create `list-home.dto.ts` with
enum-validated query params.

**Verify.** Unit tests for 6 period codes (TODAY/7D/30D/90D/12M/CUSTOM) plus the
two compareTo modes (PREVIOUS/YOY). Edge cases: month boundary, year boundary,
DST.

## Step 2 — Per-KPI queries

**What.** Create `queries/gmv.query.ts` , `approval-rate.query.ts` ,
`producers.query.ts` , `breakdowns.query.ts` , `series.query.ts` . Each is a
pure function taking `(prisma, from, to)` and returning the shaped KPI. No side
effects, no audit writes, no NestJS deps.

**Verify.** Each query has a `.spec.ts` with seeded data (use `prisma-mock` or a
test DB via Docker). Assert empty-state (zero rows) returns zeros, not nulls or
throws.

## Step 3 — AdminDashboardService + controller

**What.** `admin-dashboard.service.ts` composes the per-KPI queries into the
full response shape. `admin-dashboard.controller.ts` exposes
`GET /admin/dashboard/home` with
...
prisma, assert the endpoint returns the contract shape. Permission probe: STAFF
can view, non-admin rejected.
...
AdminAuthModule, AdminUsersModule, etc.

**Verify.** `backend && npm run build` green. Nest boot smoke shows
`Mapped {/admin/dashboard/home, GET}`.

## Step 5 — Backend unit + integration tests green

**Verify.** `cd backend && npm run lint && npm run typecheck && npm test` all
green. Permission matrix unchanged. Boot smoke passes.

## Step 6 — Frontend-admin: install recharts

**What.** Add `recharts@^3.8.1` + `date-fns@^4.1.0` (already in frontend/ at
those versions). Run `npm install` in frontend-admin.
...
`adminDashboardApi.home(filters)` calling `GET /admin/dashboard/home`.
...
- `MetricNumber` — uses JetBrains Mono via `font-mono` class, accepts a number
  or `null` (renders `—`), optional currency BRL format.
...
(Hoje / 7d / 30d / 90d / 12m / Custom). URL query string persistence
(`?period=30d`).
...
recharts PieChart. Both consume the breakdown data from the API response.

**Verify.** Empty state: passing `series: []` renders a centered `—`
placeholder, not a blank chart area.
...
unavailable KPIs, no crash, no fake data.
