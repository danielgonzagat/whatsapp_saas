---
title: SP-9 — Tesouraria do marketplace
status: draft
...
1. `MarketplaceTreasury` — saldo da Kloel por moeda, em cents, com buckets
   `available` / `pending` / `reserved` (reserva para chargebacks).
2. `MarketplaceTreasuryLedger` — append-only, espelho do padrão já consagrado em
...
3. `MarketplaceFee` — configuração de taxa do marketplace por método de pagamento e
...
5. Reconciliação — job diário que:
   ...
   - soma os fees retidos das orders do dia e compara com os créditos de
     `marketplace_fee_credit` no ledger,
...
- **I-ADMIN-W1 (histórico de taxa)**: toda `CheckoutOrder` com split deve
  referenciar `marketplaceFeeSnapshotId` — nunca recalcular a taxa a partir da
  configuração atual.
- **I-ADMIN-W2 (ledger append-only)**: `MarketplaceTreasuryLedger` não tem
  `updatedAt` e jamais é atualizado/deletado. Reforçado pela ausência de
  `update/delete` no service e por um teste
  `marketplace-treasury-ledger.invariant.spec.ts` que falha o build se a assinatura
...
- **I-ADMIN-W3 (atomicidade)**: toda mutação de saldo é emitida em
  ...
  `MarketplaceTreasuryService.append(tx, ...)` — nunca chamar
  `prisma.marketplaceTreasury.update()` direto.
- **I-ADMIN-W4 (moeda ⊂ cents)**: todos os valores são `BigInt` em cents.
  `Float` nunca entra na tesouraria do marketplace — zero dependência do legado
  `KloelWallet.availableBalance:Float`.
- **I-ADMIN-W5 (idempotência do split)**: o split de uma `CheckoutOrder` nunca
  pode creditar a `MarketplaceTreasury` duas vezes. A unicidade é garantida por
  `@@unique([orderId, kind])` no `MarketplaceTreasuryLedger` com
  `kind in ('marketplace_fee_credit','chargeback_reserve','refund_debit','chargeback_debit')`
  .
- **I-ADMIN-W6 (aprovação humana)**: `MarketplaceTreasuryPayout` só avança de `PENDING`
...
- **Gráfico de receita**: linha diária de `marketplace_fee_credit` dos últimos 30
  dias (mesma biblioteca Recharts/LineChart já usada no God View).
- **Aba Ledger**: tabela paginada, filtro por `kind` e data range.
- **Aba Payouts**: lista de `MarketplaceTreasuryPayout` com ação "Aprovar" gated por
  `CARTEIRA:APPROVE`.
...
- Anticipation/antecipação programática do produtor — essa já existe em
  `WalletAnticipation` (schema.prisma:2239) e **não** pertence à tesouraria do marketplace.
...
- Unit: `MarketplaceTreasuryService` — casos `creditFee`, `debitRefund`,
  `creditChargebackReserve`, `moveReservedToAvailable`.
- Invariant: `marketplace-treasury-ledger.invariant.spec.ts` — bloqueia
