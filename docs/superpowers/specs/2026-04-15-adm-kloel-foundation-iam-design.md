...
No `fabric`, no `reactflow`, no `jspdf`, no mercadopago — admin doesn't need them.
...
- No `NEXT_PUBLIC_META_*`, no Asaas, no Mercado Pago — admin does not need client payment SDKs.
