...
- Add a backend marketing skills package under `backend/src/kloel/marketing-skills/`.
...
- Landing/site generation and draft persistence via Kloel site endpoints/services.
...
- Affiliate/split recommendations constrained by SplitEngine, LedgerEngine, and FraudEngine context.
...
- State what is known from real data, what is inferred, and what still requires confirmation.
- If execution is possible, say so concretely and use tools.
- If execution is not yet wired, return an honest action plan with the exact next platform action.
...
- Extend `UnifiedAgentService` tests to verify marketing packets are injected for marketing asks.
