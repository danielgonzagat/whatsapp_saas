...
- Initial audit found seller-side `on_behalf_of` coupling in the checkout charge flow. Active sale creation has since been moved to a platform-owned marketplace PaymentIntent path, while post-payment transfer timing and older compatibility surfaces still require review before certification.
...
- RISK-0001: The active checkout charge path is now marketplace-owned, but the repository still contains older compatibility surfaces and immediate post-payment transfer logic that require further refactor before the end-to-end marketplace certification story is complete.
