...
export class BinarySerde<T extends Record<string, unknown>> {
  ...
  serialize(obj: T): Uint8Array {
    ...
    for (const field of this[SCHEMA].fields) {
      ...
      if (field.array && Array.isArray(val)) {
        ...
        for (const item of val) {
          if (field.nullable) {
            view.setUint8(offset, item === null ? 0 : 1);
            offset += 1;
          }
        ...
        if (field.nullable) {
          view.setUint8(offset, val === null ? 0 : 1);
          offset += 1;
        }
  ...
  deserialize(buf: Uint8Array): T {
    ...
    for (const field of this[SCHEMA].fields) {
      if (field.array) {
        if (field.nullable) {
          const nullFlag = view.getUint8(offset);
          offset += 1;
        }
        ...
        for (let i = 0; i < length; i++) {
          let isNull = false;
          if (field.nullable) {
            isNull = view.getUint8(offset) === 0;
            offset += 1;
          }
          const r = this.readField(view, offset, field.type);
          arr.push(isNull ? null : r.value);
          offset = r.nextOffset;
        }
        ...
        const r = this.readField(view, offset, field.type);
        result[field.name] = isNull ? null : r.value;
        offset = r.nextOffset;
      }
    }
