...
export class BinarySerde<T extends Record<string, unknown>> {
  ...
  private fieldByteSize(type: FieldType, val: unknown, length?: number): number {
    switch (type) {
      case 'int8':
      case 'uint8':
      case 'bool':
  ...
  private writeField(view: DataView, offset: number, type: FieldType, val: unknown, length?: number): number {
    switch (type) {
      case 'int8':
      ...
      case 'uint32':
        ...
        return offset + 4;
      case 'int64':
        view.setBigInt64(offset, (val as bigint), true);
        return offset + 8;
      case 'uint64':
        view.setBigUint64(offset, (val as bigint), true);
        return offset + 8;
      case 'float32':
      ...
      case 'string': {
        ...
        if (length !== undefined) {
          const writeLen = Math.min(str.length, length);
  ...
  private readField(
    ...
    switch (type) {
      ...
      case 'string': {
        ...
        return { value: str, nextOffset: offset + slen };
      }
      default:
