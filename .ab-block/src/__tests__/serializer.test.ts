...
describe("BinarySerde", () => {
  ...
  describe("int64", () => {
    it("serializes a large int64 value", () => {
      const schema: Schema = { fields: [{ name: "big", type: "int64" }] };
      const serde = new BinarySerde<{ big: bigint }>(schema);
      const val = 9007199254740991n;
      const buf = serde.serialize({ big: val });
      assert.equal(buf.length, 8);
      const view = new DataView(buf.buffer);
      assert.equal(view.getBigInt64(0, true), val);
    });

    it("deserializes a negative int64 value", () => {
      const schema: Schema = { fields: [{ name: "neg", type: "int64" }] };
      const serde = new BinarySerde<{ neg: bigint }>(schema);
      const val = -9007199254740991n;
      const buf = serde.serialize({ neg: val });
      assert.equal(serde.deserialize(buf).neg, val);
    });

    it("round-trips int64 through serialize-deserialize", () => {
      const schema: Schema = { fields: [{ name: "val", type: "int64" }] };
      const serde = new BinarySerde<{ val: bigint }>(schema);
      const original = { val: -123456789012345n };
      assert.deepStrictEqual(serde.deserialize(serde.serialize(original)), original);
    });
  });

  describe("uint64", () => {
    it("serializes a large uint64 value", () => {
      const schema: Schema = { fields: [{ name: "big", type: "uint64" }] };
      const serde = new BinarySerde<{ big: bigint }>(schema);
      const val = 18446744073709551615n;
      const buf = serde.serialize({ big: val });
      assert.equal(buf.length, 8);
      const view = new DataView(buf.buffer);
      assert.equal(view.getBigUint64(0, true), val);
    });

    it("round-trips uint64 through serialize-deserialize", () => {
      const schema: Schema = { fields: [{ name: "val", type: "uint64" }] };
      const serde = new BinarySerde<{ val: bigint }>(schema);
      const original = { val: 12345678901234567890n };
      assert.deepStrictEqual(serde.deserialize(serde.serialize(original)), original);
    });
  });

  describe("fixed-length string", () => {
    it("serializes a fixed-length string without length prefix", () => {
      const schema: Schema = { fields: [{ name: "code", type: "string", length: 4 }] };
      const serde = new BinarySerde<{ code: string }>(schema);
      const buf = serde.serialize({ code: "AB" });
      assert.equal(buf.length, 4);
    ...
    it("deserializes a fixed-length string trimming null bytes", () => {
      const schema: Schema = { fields: [{ name: "code", type: "string", length: 4 }] };
      const serde = new BinarySerde<{ code: string }>(schema);
      const buf = serde.serialize({ code: "XY" });
      assert.equal(serde.deserialize(buf).code, "XY");
    });

    it("round-trips a fixed-length string", () => {
      const schema: Schema = { fields: [{ name: "code", type: "string", length: 8 }] };
      const serde = new BinarySerde<{ code: string }>(schema);
      const original = { code: "HELLO" };
      assert.deepStrictEqual(serde.deserialize(serde.serialize(original)), original);
    });

    it("truncates string longer than fixed length", () => {
      const schema: Schema = { fields: [{ name: "code", type: "string", length: 3 }] };
      const serde = new BinarySerde<{ code: string }>(schema);
      const buf = serde.serialize({ code: "ABCDEF" });
      assert.equal(buf.length, 3);
      const decoded = serde.deserialize(buf);
      assert.equal(decoded.code, "ABC");
    });
  });
