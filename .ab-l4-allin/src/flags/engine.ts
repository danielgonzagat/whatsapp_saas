...
export class FeatureFlagStore extends ReactiveStore<FlagsState> {
  ...
  #evaluateRule(rule: FlagRule, context?: FlagContext): boolean {
    switch (rule.type) {
      ...
      case 'hash_based':
        return this.#evaluateHashBased(rule, context);
      default:
        return false;
    }
  }
  ...
  #evaluateHashBased(rule: FlagRule, context?: FlagContext): boolean {
    const percentage = rule.config.percentage ?? 0;
    if (percentage <= 0) return false;
    if (percentage >= 100) return true;
  ...
  #evaluateTimeWindow(rule: FlagRule, context?: FlagContext): boolean {
    const hour = context?.currentHour;
    if (hour === undefined) return false;
    const start = rule.config.startHour ?? 0;
    const end = rule.config.endHour ?? 0;
