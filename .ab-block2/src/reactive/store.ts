...
> {
  ...
  computed<U>(fn: Computed<S, U>, equals?: (a: U, b: U) => boolean): Selector<S, U> {
    let lastState: S | undefined;
    let lastResult: U | undefined;
    let initialized = false;
    const eq = equals ?? fn.equals;
    ...
    return (state: S): U => {
      ...
      if (initialized) {
        const same = eq ? eq(lastResult as U, result) : lastResult === (result as unknown);
        if (same) {
          lastState = state;
          return lastResult as U;
        }
