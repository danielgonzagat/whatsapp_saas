...
describe('ReactiveStore', () => {
  ...
  it('computed returns correct value', () => {
    const store = makeStore(5);
    const double = store.computed((s) => s.count * 2);
    assert.strictEqual(double(store.getState()), 10);
    store.setState({ count: 3 });
    assert.strictEqual(double(store.getState()), 6);
  });

  it('computed only recalculates when state changes', () => {
    const store = makeStore(1);
    let calls = 0;
    const double = store.computed((s) => {
      calls++;
      return s.count * 2;
    });

    double(store.getState());
    assert.strictEqual(calls, 1);
    double(store.getState());
    assert.strictEqual(calls, 1);
    store.setState({ count: 2 });
  ...
  it('computed with custom equals', () => {
    const store = new ReactiveStore({
      initial: { items: [1, 2] },
    });
    let computeCalls = 0;
    const first = store.computed(
      (s) => {
        computeCalls++;
        return s.items[0];
      },
    ...
    assert.strictEqual(first(store.getState()), 1);
    ...
    store.setState({ items: [5, 2, 3] });
    assert.strictEqual(first(store.getState()), 5);
  ...
  it('multiple computeds work independently', () => {
    ...
    const triple = store.computed((s) => {
      tripleCalls++;
      return s.count * 3;
    });

    assert.strictEqual(double(store.getState()), 6);
    assert.strictEqual(triple(store.getState()), 9);
    assert.strictEqual(doubleCalls, 1);
    assert.strictEqual(tripleCalls, 1);

    store.setState({ count: 4 });
    assert.strictEqual(double(store.getState()), 8);
    assert.strictEqual(triple(store.getState()), 12);
    assert.strictEqual(doubleCalls, 2);
    assert.strictEqual(tripleCalls, 2);
  });
});
