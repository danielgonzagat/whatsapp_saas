...
export function WhatsAppSessionHarness({
  mode,
}: WhatsAppSessionHarnessProps) {
      ...
      <main className="min-h-screen bg-black px-6 py-12 text-white">
        <div className="mx-auto flex max-w-2xl flex-col gap-6 rounded border border-white/10 bg-white/5 p-8">
          ...
          <p className="text-sm text-white/70">
            Dedicated route used by Playwright to validate the Meta embedded-signup
            fallback without rendering a QR image.
          </p>
          <button
            type="button"
          ...
          {connecting ? (
            <div className="rounded border border-white/10 bg-black/30 p-4">
              ...
              <p className="mt-2 text-sm text-white/70">Use o fluxo guiado para concluir a conexao.</p>
            </div>
          ) : null}
        </div>
      </main>
  ...
  return (
    <main className="min-h-screen bg-black px-6 py-12 text-white">
      <div className="mx-auto flex max-w-3xl flex-col gap-6 rounded border border-white/10 bg-white/5 p-8">
        ...
        <p className="text-sm text-white/70">
          Drawer-style WhatsApp surface used by Playwright to validate the current
          QR guidance contract.
        </p>
        <button
          type="button"
        ...
        {drawerOpen ? (
          <section className="rounded border border-white/10 bg-black/30 p-4">
            ...
            {connecting ? (
              <p className="mt-4 text-sm text-white/70">Gerando QR Code...</p>
            ) : null}
          </section>
        ) : null}
      </div>
