...
function OnboardingChatContent() {
  ...
  useEffect(() => {
    if (completed) {
      router.push('/dashboard?panel=whatsapp&autoConnect=1&source=onboarding');
    }
  }, [completed, router]);
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (statusData.completed) {
        addMessage(
          'assistant',
          'Perfeito. Agora vamos conectar seu WhatsApp oficial — vou abrir a conexão da Meta na próxima tela.',
        );
        setCompleted(true);
      }
    } catch (error) {
  ...
  const goToDashboard = () => {
    if (!workspaceId) return;
    router.push('/dashboard?panel=whatsapp&source=onboarding');
  };
