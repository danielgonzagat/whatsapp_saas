    ...
    >
      <LegalSection id="quem-somos" title={kloelT(`1. Quem somos`)}>
        <p>
          ...
          {kloelT(`, nome fantasia`)} <strong>{company.tradeName}</strong>
          {kloelT(`,
          inscrita no CNPJ sob o nº`)}{' '}
          <strong>{company.cnpj}</strong>
          {kloelT(`, com endereço em`)} {company.addressLine1}, {company.addressLine2}.
        </p>
        <p>
          ...
          <strong>{company.emailDpo}</strong>
          {kloelT(`. Para suporte operacional, usamos`)} <strong>{company.emailSupport}</strong>.
        </p>
        <p>
          {kloelT(`A Kloel opera como`)} <strong>controladora</strong>{' '}
          {kloelT(`em relação aos dados da sua própria
          ...
          <strong>{kloelT(`operadora/processadora`)}</strong>
          {kloelT(`, seguindo instruções contratuais do
          cliente-controlador.`)}
        </p>
      </LegalSection>
      ...
      <LegalSection id="dados-coletados" title={kloelT(`2. Dados que coletamos`)}>
        ...
        <p>
          <strong>{kloelT(`2.1 Dados fornecidos diretamente.`)}</strong>{' '}
          {kloelT(`Quando você cria uma conta, assina um
        ...
        <p>
          <strong>{kloelT(`2.2 Dados coletados automaticamente.`)}</strong>{' '}
          {kloelT(`Ao acessar nossos sites, checkouts e
        ...
        <p>
          <strong>{kloelT(`2.3 Dados recebidos de terceiros.`)}</strong>{' '}
          {kloelT(`Em fluxos de autenticação e integração,
          recebemos dados diretamente dos provedores autorizados por você:`)}
        </p>
        <LegalList
          items={[
            <>
              <strong>{kloelT(`Google.`)}</strong>{' '}
              {kloelT(`Nome, email, foto de perfil, identificador da Conta Google e
              preferência de idioma via escopos`)}{' '}
              <code>openid</code>, <code>email</code> e <code>profile</code>
              {kloelT(`. Quando a funcionalidade estiver aprovada e habilitada por
              feature flag, a Kloel poderá solicitar separadamente`)}{' '}
              <code>{kloelT(`user.phonenumbers.read`)}</code> e{' '}
              <code>{kloelT(`user.addresses.read`)}</code>{' '}
              {kloelT(`para
              preenchimento acelerado de checkout.`)}
            </>,
            <>
              <strong>{kloelT(`Meta/Facebook.`)}</strong>{' '}
              {kloelT(`Nome, email, foto de perfil e identificador do usuário
              via permissões`)}{' '}
              <code>email</code> e <code>public_profile</code>{' '}
              {kloelT(`para autenticação. Em
            ...
            <>
              <strong>{kloelT(`Apple.`)}</strong>{' '}
              {kloelT(`Nome e email via Sign in with Apple, quando esse provedor
              estiver ativado no ambiente.`)}
            </>,
          ]}
        />
        <p>
          <strong>{kloelT(`2.4 Dados de clientes finais dos nossos clientes.`)}</strong>{' '}
          {kloelT(`Leads, contatos,
      ...
      <LegalSection id="direitos" title={kloelT(`8. Direitos do titular`)}>
        ...
        <p>
          {kloelT(`Nosso canal para exercício desses direitos é`)}{' '}
          <strong>{company.emailDpo}</strong>
          {kloelT(`. Buscamos
          responder em até`)}{' '}
          <strong>{kloelT(`15 dias`)}</strong>
          {kloelT(`, salvo quando legislação específica exigir prazo
      ...
      <LegalSection id="contato" title={kloelT(`12. Contato, ANPD e canais de privacidade`)}>
        <p>
          {kloelT(`O contato principal do encarregado/DPO é`)} <strong>{company.emailDpo}</strong>
          {kloelT(`. O suporte
          geral da plataforma permanece disponível em`)}{' '}
