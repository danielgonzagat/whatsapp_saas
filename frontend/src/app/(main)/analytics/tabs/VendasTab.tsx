  ...
  return (
    <>
      <div style={{ display: 'flex', gap: 14, marginBottom: 20, flexWrap: 'wrap' }}>
        <MetricCard
          title={kloelT(`Total de operacoes`)}
          value={summary ? R$(summary.totalRevenue || 0) : '...'}
          sub={
            summary
              ? `${Fmt(summary.totalCount || 0)} operacoes · Ticket medio ${R$(summary.ticketMedio || 0)}`
              : ''
          }
          color={V.em}
          icon={ICONS.dollar}
        ...
        />
        <MetricCard
          title={kloelT(`Total comissoes`)}
          value={summary ? R$(summary.totalCommission || 0) : '...'}
          sub={kloelT(`Comissoes do periodo`)}
          color={V.g2}
          icon={ICONS.users}
          loading={ls}
        />
      ...
      <div
        style={{ ...chartCardStyle, overflowX: isMobile ? ('auto' as const) : ('hidden' as const) }}
      >
        <TableHeader
          cols={[
            { l: 'Pedido', w: '0.7fr' },
        ...
        {lv ? (
          ...
          <div style={{ padding: 20, textAlign: 'center', color: V.t3, fontSize: 12 }}>
            {kloelT(`Nenhuma operacao no periodo`)}
          </div>
        ) : (
          rows.map((s: ReportRow, i: number) => {
            ...
            return (
              ...
              >
                <span style={{ fontFamily: FONT_MONO, fontSize: 9, color: V.t3 }}>
                ...
                <span style={{ fontSize: 9, color: V.t2, fontFamily: FONT_MONO }}>
                  {s.createdAt ? new Date(s.createdAt).toLocaleDateString('pt-BR') : '—'}
                </span>
                <span style={{ fontFamily: FONT_MONO, fontSize: 11, fontWeight: 600, color: V.t }}>
                  {R$(s.totalInCents || 0)}
                </span>
