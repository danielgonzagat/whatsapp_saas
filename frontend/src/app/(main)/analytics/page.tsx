...
import {
  VendasTab, AfterPayTab, ChurnTab, AbandonosTab, SatisfacaoTab,
  EnvioRelatoriosTab, AfiliadosTab, IndicadoresTab, AssinaturasTab,
  IndProdTab, RecusaTab, OrigemTab, MetricasTab, EstornosTab,
  ChargebackTab, EngajamentoTab,
} from './tabs';
...
export default function KloelRelatorio() {
  ...
  const {
    active, setActive, page, setPage, showFilter, setShowFilter,
    filters, setFilters, baseFilters,
  } = useAnalyticsFilters();
  ...
  return (
    <div style={{ background: V.void, minHeight: '100vh', fontFamily: FONT_SORA, color: V.t, padding: isMobile ? '20px 16px 28px' : '28px 32px' }}>
      ...
      <div style={SUBINTERFACE_PILL_ROW_STYLE}>
        {TABS.map((t) => (
          <button type="button" key={t.k} onClick={() => { setActive(t.k); setPage(1); router.replace(`/analytics?tab=${t.k}`); }} style={getSubinterfacePillStyle(active === t.k, isMobile)}>
      ...
      <div style={{ animation: 'fadeIn .3s ease', maxWidth: 1240, margin: '0 auto' }} key={active}>
        {active === 'vendas' && <VendasTab filters={filters} baseFilters={baseFilters} page={page} setPage={setPage} isMobile={isMobile} />}
        {active === 'afterpay' && <AfterPayTab filters={filters} setFilters={setFilters} baseFilters={baseFilters} page={page} setPage={setPage} />}
        {active === 'churn' && <ChurnTab filters={filters} />}
        {active === 'abandonos' && <AbandonosTab baseFilters={baseFilters} page={page} setPage={setPage} />}
        {active === 'satisfacao' && <SatisfacaoTab />}
        {active === 'envio' && <EnvioRelatoriosTab filters={filters} isMobile={isMobile} />}
        {active === 'exportacoes' && <AnalyticsExportPanel setPage={setPage} setActive={setActive} />}
        {active === 'afiliados' && <AfiliadosTab filters={filters} />}
        {active === 'indicadores' && <IndicadoresTab filters={filters} />}
        {active === 'assinaturas' && <AssinaturasTab baseFilters={baseFilters} page={page} setPage={setPage} />}
        {active === 'ind_prod' && <IndProdTab filters={filters} />}
        {active === 'recusa' && <RecusaTab baseFilters={baseFilters} page={page} setPage={setPage} />}
        {active === 'origem' && <OrigemTab filters={filters} />}
        {active === 'metricas' && <MetricasTab filters={filters} />}
        {active === 'estornos' && <EstornosTab filters={filters} setFilters={setFilters} baseFilters={baseFilters} page={page} setPage={setPage} />}
        {active === 'chargeback' && <ChargebackTab baseFilters={baseFilters} page={page} setPage={setPage} />}
        {active === 'engajamento' && <EngajamentoTab filters={filters} />}
      </div>

      <AnalyticsFilterDrawer open={showFilter} onClose={() => setShowFilter(false)} filters={filters} setFilters={setFilters} />
    </div>
  );
}
