  ...
  return (
    ...
    >
      ...
      <div style={{ marginTop: 10 }}>
        <NeuroPulse color={color} w={140} h={16} />
      </div>
    </div>
  );
}
...
export function EmptyState({ message }: { message: string }) {
  return (
    <div style={{ ...chartCardStyle, padding: 40, textAlign: 'center' }}>
  ...
  return (
    ...
    >
      ...
      {payload.map((p) => (
        <div
          key={p.name}
          style={{ fontSize: 12, fontFamily: FONT_MONO, fontWeight: 600, color: p.color || V.em }}
        >
          {p.name}:{' '}
          {typeof p.value === 'number' && p.value > 999
            ? `R$ ${(p.value / 100).toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
...
export function Pagination({
  ...
  return (
    ...
    >
      <span style={{ fontSize: 10, color: V.t3, fontFamily: FONT_MONO }}>
        {(page - 1) * perPage + 1} ate {Math.min(page * perPage, total)} de {total}
      </span>
...
export function FilterBar({ children }: { children: ReactNode }) {
  return (
    <div style={{ ...chartCardStyle, padding: 16, marginBottom: 20 }}>
