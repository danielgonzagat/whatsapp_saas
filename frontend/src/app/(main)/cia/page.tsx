...
function workItemStateBadgeVariant(state: string): 'success' | 'warning' | 'error' | 'info' | undefined {
  switch (state) {
...
export default function CiaPage() {
  ...
  const [conversationActionRegistry, setConversationActionRegistry] = useState<CiaConversationActionRegistry | null>(null);
  ...
  async function loadAdvancedData() {
    ...
    if (inputSessionsRes.data) setInputSessions(Array.isArray(inputSessionsRes.data) ? inputSessionsRes.data : []);
      ...
      <Section spacing="lg">
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          ...
          <div className="flex items-center gap-2">
            <Button
              variant="secondary"
              onClick={() => { void loadSurface(); void loadAdvancedData(); }}
                  ...
                  {session.status !== 'COMPLETED' && (
                    <div className="mt-3">
                      ...
                      <Button
                        ...
                        disabled={
                          workspaceLoading ||
                          !sessionAnswers[session.id]?.trim()
                        }
                      >
                        Responder
                      </Button>
            ...
            {!cycleProof ? (
              ...
              <div className="space-y-3">
                {cycleProof.summary && (
                  ...
                  >
                    <p className="text-xs uppercase tracking-widest mb-1" style={{ color: colors.text.muted }}>
                      Sumário
                    </p>
                    <p className="text-sm" style={{ color: colors.text.primary }}>
                      {cycleProof.summary}
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[
                    ...
                    stat.value !== undefined ? (
                      ...
                      >
                        <p className="text-lg font-bold font-mono" style={{ color: colors.text.primary }}>
                          {stat.value}
                        </p>
                        <p className="text-xs" style={{ color: colors.text.muted }}>
                          {stat.label}
        ...
        <Surface className="p-5">
          ...
          >
            <div className="flex items-center gap-2">
              ...
              {capabilityRegistry && (
                <Badge variant="info">
                  {capabilityRegistry.items.length} capacidades
                </Badge>
              )}
              {conversationActionRegistry && (
                <Badge variant="info">
                  {conversationActionRegistry.items.length} ações
                </Badge>
              )}
            </div>
            {registriesExpanded ? (
