...
const CATEGORY_CARDS: { key: Category; icon: string; title: string; count: number; roadmapCount: number; color: string }[] = [
  {
    key: 'impulsione',
    icon: CAPABILITY_CATEGORY_META.impulsione.icon,
    title: CAPABILITY_CATEGORY_META.impulsione.title,
    count: ALL_TOOLS.filter((tool) => tool.category === 'impulsione' && tool.status !== 'planned').length,
    roadmapCount: ALL_TOOLS.filter((tool) => tool.category === 'impulsione' && tool.status === 'planned').length,
  ...
  },
  {
    ...
    count: ALL_TOOLS.filter((tool) => tool.category === 'recupere' && tool.status !== 'planned').length,
    roadmapCount: ALL_TOOLS.filter((tool) => tool.category === 'recupere' && tool.status === 'planned').length,
  ...
  },
  {
    ...
    roadmapCount: ALL_TOOLS.filter((tool) => tool.category === 'fale' && tool.status === 'planned').length,
  ...
  },
  {
    ...
    count: ALL_TOOLS.filter((tool) => tool.category === 'gerencie' && tool.status !== 'planned').length,
    roadmapCount: ALL_TOOLS.filter((tool) => tool.category === 'gerencie' && tool.status === 'planned').length,
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        {liveTools.length} ferramenta{liveTools.length !== 1 ? 's' : ''} operacional{liveTools.length !== 1 ? 'is' : ''} encontrada{liveTools.length !== 1 ? 's' : ''}
        ...
        <>
          ...
          {roadmapTools.length > 0 ? (
            <div style={{ marginTop: liveTools.length > 0 ? 28 : 0 }}>
              <div style={{ fontFamily: typography.fontFamily.mono, fontSize: 11, color: colors.text.dust, letterSpacing: '0.16em', textTransform: 'uppercase', marginBottom: 12 }}>
                Roadmap publicado
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 12 }}>
