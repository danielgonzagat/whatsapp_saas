...
function createRequest(options?: {
  ...
  return {
    ...
    nextUrl: new URL(options?.url || 'https://auth.kloel.com/api/auth/login'),
    json: vi.fn(async () => options?.body || {}),
  } as any;
}

describe('login auth proxy route', () => {
  beforeEach(() => {
    vi.restoreAllMocks();
  ...
  it('sets shared cookies for camelCase auth payloads returned by the backend', async () => {
    ...
    const response = await POST(
      createRequest({
        body: { email: 'daniel@kloel.com', password: 'very-secret' },
