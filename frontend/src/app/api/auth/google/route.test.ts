...
function createRequest(options?: {
  ...
  return {
    ...
    nextUrl: new URL(options?.url || 'https://auth.kloel.com/api/auth/google'),
    json: vi.fn(async () => options?.body || {}),
  } as any;
}

describe('google auth proxy route', () => {
  beforeEach(() => {
    vi.restoreAllMocks();
  ...
  it('forwards the Google credential and sets shared cookies for camelCase auth payloads', async () => {
    const fetchSpy = vi.spyOn(globalThis, 'fetch').mockResolvedValue(
      new Response(
        JSON.stringify({
          accessToken: 'kloel-access-token',
          refreshToken: 'kloel-refresh-token',
          workspace: { id: 'ws-google' },
        }),
        {
          status: 200,
    ...
    expect(fetchSpy).toHaveBeenCalledWith(
      'https://backend.example.com/auth/oauth/google',
