...
function createRequest(options?: {
  ...
  return {
    ...
    nextUrl: new URL(options?.url || 'https://auth.kloel.com/api/auth/facebook'),
...
describe('facebook auth proxy route', () => {
  ...
  it('forwards the Facebook access token to the backend and sets shared cookies for camelCase auth payloads', async () => {
    const fetchSpy = vi.spyOn(globalThis, 'fetch').mockResolvedValue(
      new Response(
        JSON.stringify({
          accessToken: 'kloel-access-token',
          refreshToken: 'kloel-refresh-token',
          workspace: { id: 'ws-facebook' },
        }),
        {
          status: 200,
    ...
    const response = await POST(
      createRequest({
        body: { accessToken: 'facebook-user-token' },
