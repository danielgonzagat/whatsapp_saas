...
function createRequest(options?: {
  ...
  return {
    ...
    nextUrl: new URL(options?.url || 'https://auth.kloel.com/api/auth/sessions/revoke-current'),
  } as any;
}

describe('revoke-current-session proxy route', () => {
  beforeEach(() => {
    vi.restoreAllMocks();
  ...
  it('forwards the authenticated revoke-current request to the backend', async () => {
    ...
    expect(fetchSpy).toHaveBeenCalledWith(
      'https://backend.example.com/auth/sessions/revoke-current',
