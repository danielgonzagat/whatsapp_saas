...
export function useCheckoutProduct(productId: string | null) {
  const updateProduct = useCallback(async (body: any) => {
...
export function useCheckoutOrders(params?: { status?: string; page?: number; limit?: number }) {
  ...
  const q = qs.toString();
  const { data, isLoading, mutate } = useSWR<any>(
    `/checkout/orders${q ? `?${q}` : ''}`, swrFetcher, { keepPreviousData: true }
  );
  const orders = Array.isArray(data) ? data : data?.orders || [];
  const total = data?.total ?? orders.length;

  const updateOrderStatus = useCallback(async (id: string, status: string, extra?: { trackingCode?: string; trackingUrl?: string }) => {
    const res = await apiFetch(`/checkout/orders/${id}/status`, { method: 'PATCH', body: { status, ...extra } });
    mutate();
    return res;
  }, [mutate]);
...
export function useCheckoutOrder(id: string | null) {
  const { data, isLoading, mutate } = useSWR<any>(
    id ? `/checkout/orders/${id}` : null, swrFetcher, { keepPreviousData: true }
  );
...
export function usePixels(planId: string | null) {
  ...
  const createPixel = useCallback(async (body: { type: string; pixelId: string; accessToken?: string }) => {
  ...
  const updatePixel = useCallback(async (pixelId: string, body: Partial<{ type: string; pixelId: string; accessToken: string }>) => {
    const res = await apiFetch(`/checkout/pixels/${pixelId}`, { method: 'PUT', body });
    mutate();
    return res;
  }, [mutate]);

  const deletePixel = useCallback(async (pixelId: string) => {
