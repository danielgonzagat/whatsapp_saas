...
export function useMarketingStats() {
  const { data, isLoading, error } = useSWR<MarketingStats>('/marketing/stats', swrFetcher, { refreshInterval: 30000 });
  return { stats: data || { totalMessages: 0, totalLeads: 0, totalSales: 0, totalRevenue: 0 }, isLoading, error };
}
...
export function useChannelStats(channel: string | null) {
  const { data, isLoading } = useSWR<ChannelStats>(channel ? `/marketing/channel/${channel}/stats` : null, swrFetcher);
