...
export function useAdSpendMutations() {
  ...
  const registerAdSpend = async (payload: AdSpendEntry) => {
    ...
    mutate((key: unknown) => typeof key === 'string' && key.startsWith('/reports/ad-spend'), undefined, { revalidate: true });
    return res.data;
  };
...
export interface NpsData {
  ...
  avg: string;
  total: number;
  responses: Array<{ id: string; details: { score?: number; comment?: string; orderId?: string }; createdAt: string }>;
}
