...
const TONES: {v:string; l:string; icon: typeof MessageSquare; desc:string}[] = [
  {v:"CONSULTIVE",l:"Consultivo", icon: MessageSquare, desc:"Faz perguntas, entende a necessidade e recomenda."},
  {v:"DIRECT",l:"Direto", icon: Zap, desc:"Vai direto ao ponto, objetivo e rápido."},
  {v:"EMPATHETIC",l:"Empático", icon: Heart, desc:"Acolhe, valida sentimentos e cria conexão."},
  {v:"EDUCATIVE",l:"Educativo", icon: BookOpen, desc:"Explica com detalhes, ensina antes de vender."},
  {v:"URGENT",l:"Urgente", icon: AlertTriangle, desc:"Cria senso de urgência, foco em ação imediata."},
  {v:"AUTO",l:"Automático", icon: Sparkles, desc:"A IA escolhe o melhor tom conforme o contexto."},
]
...
const CONTRAINDICATIONS = ["Gestantes","Lactantes","Menores de 18 anos","Hipertensos","Diabéticos","Alérgicos a componentes","Uso de anticoagulantes","Doença renal","Doença hepática","Cardiopatas"]
...
const UPSELL_WHEN = ["Cliente pergunta sobre o plano principal","Cliente aceita o preço","Cliente quer mais resultados","Cliente compra pela segunda vez","Cliente pergunta por garantia estendida","Cliente demonstra poder aquisitivo"]
const UPSELL_ARGUMENTS = ["Mais resultados por pouco a mais","Melhor custo-benefício a longo prazo","Brindes e bônus exclusivos","Garantia estendida inclusa","Suporte prioritário"]
const DOWNSELL_WHEN = ["Cliente diz que está caro","Cliente quer pensar","Cliente compara com concorrente","Cliente quer apenas experimentar","Cliente desiste do carrinho"]
const DOWNSELL_ARGUMENTS = ["Versão mais acessível","Teste sem compromisso","Menos unidades, mesmo resultado","Garantia mantida"]
...
export function PlanAIConfigTab({ planId, productId }: { planId: string; productId: string }) {
    ...
    const loadPlans = async () => {
      try {
        ...
        if (Array.isArray(data)) {
          setSiblingPlans(data.filter((p: any) => p.id !== planId).map((p: any) => ({ id: p.id, name: p.name || p.title || `Plano ${p.id}` })))
        }
      } catch {}
    }
    loadPlans()
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      await apiFetch(`/products/${productId}/ai-config`, {
        method: "PUT",
        body: {
          planId, genders, ages, moments, knowledge, buyingPower, problem,
          tier, whenOffer, differentiators, scarcity, objectionStates,
          socialProof, socialProofValues, guarantee, guaranteeValues,
          benefits, benefitsValues, urgencyArgs, urgencyValues,
          upsellEnabled, upsellTargetPlan, upsellWhen, upsellArgument,
          downsellEnabled, downsellTargetPlan, downsellWhen, downsellArgument,
          tone, persistence, messageLimit, followUpHours, followUpMax,
  ...
  const sectionTitle = (t: string) => (
    <h3 className="mb-3 mt-2 text-sm font-semibold uppercase" style={{ fontFamily: typography.fontFamily.display, color: colors.text.starlight, letterSpacing: "0.02em" }}>{t}</h3>
  )
  ...
  const s5Complete = (upsellEnabled && upsellTargetPlan !== "") || (downsellEnabled && downsellTargetPlan !== "") || (!upsellEnabled && !downsellEnabled)
  ...
  const completenessColor = (complete: boolean, partial: boolean) => complete ? colors.state.success : partial ? colors.brand.amber : colors.text.void
  const completenessLabel = (complete: boolean, partial: boolean) => complete ? "Completo" : partial ? "Parcial" : "Não configurado"
    ...
    <div className="space-y-8">
      ...
      <div className="flex items-start gap-3 rounded-xl p-5" style={{ background: `${colors.accent.webb}10`, border: `1px solid ${colors.border.space}` }}>
        ...
        <div>
          ...
          <p className="mt-1 text-sm" style={{ color: colors.text.moonlight }}>Quanto mais detalhado, melhores as vendas. Todas as configurações alimentam a IA automaticamente.</p>
        </div>
      </div>
      ...
      <div className="rounded-xl p-5" style={cardStyle}>
        <div className="grid gap-6 md:grid-cols-2">
          <div>
            <label className="mb-2 block" style={labelStyle}>Gênero</label>
            <div className="flex flex-wrap gap-2">
              {GENDERS.map(g => (
                <label key={g} className="flex items-center gap-1.5 text-sm cursor-pointer" style={{ color: colors.text.starlight }}>
                  <input type="checkbox" checked={genders.includes(g)} onChange={() => toggleList(genders, g, setGenders)} style={{ accentColor: colors.accent.webb }} />{g}
                </label>
              ))}
            </div>
          </div>
          <div>
            <label className="mb-2 block" style={labelStyle}>Faixa etária</label>
            <div className="flex flex-wrap gap-2">
              {AGE_RANGES.map(a => (
                <label key={a} className="flex items-center gap-1.5 text-sm cursor-pointer" style={{ color: colors.text.starlight }}>
                  <input type="checkbox" checked={ages.includes(a)} onChange={() => toggleList(ages, a, setAges)} style={{ accentColor: colors.accent.webb }} />{a}
                </label>
              ))}
            </div>
          </div>
          <div>
            <label className="mb-2 block" style={labelStyle}>Momento de vida</label>
            <div className="space-y-1">
              {LIFE_MOMENTS.map(m => (
                <label key={m} className="flex items-center gap-1.5 text-sm cursor-pointer" style={{ color: colors.text.starlight }}>
                  <input type="checkbox" checked={moments.includes(m)} onChange={() => toggleList(moments, m, setMoments)} style={{ accentColor: colors.accent.webb }} />{m}
                </label>
              ))}
            </div>
          </div>
          <div className="space-y-4">
            <div>
              <label className="mb-2 block" style={labelStyle}>Nível de conhecimento</label>
              {KNOWLEDGE_LEVELS.map(k => (
                <label key={k.v} className="flex items-center gap-1.5 text-sm cursor-pointer" style={{ color: colors.text.starlight }}>
                  <input type="radio" name="knowledge" checked={knowledge===k.v} onChange={() => setKnowledge(k.v)} style={{ accentColor: colors.accent.webb }} />{k.l}
                </label>
              ))}
            </div>
            <div>
              <label className="mb-2 block" style={labelStyle}>Poder aquisitivo</label>
              {BUYING_POWER.map(b => (
                <label key={b.v} className="flex items-center gap-1.5 text-sm cursor-pointer" style={{ color: colors.text.starlight }}>
                  <input type="radio" name="buying" checked={buyingPower===b.v} onChange={() => setBuyingPower(b.v)} style={{ accentColor: colors.accent.webb }} />{b.l}
                </label>
              ))}
            </div>
            <div>
              <label className="mb-2 block" style={labelStyle}>Problema principal</label>
              <select value={problem} onChange={e => setProblem(e.target.value)} className={selectClass} style={inputStyle}>
      ...
      <div className="rounded-xl p-5" style={cardStyle}>
        <div className="grid gap-6 md:grid-cols-2">
          <div>
            <label className="mb-2 block" style={labelStyle}>Este plano é o quê?</label>
            {TIERS.map(t => (
              <label key={t.v} className="flex items-center gap-1.5 text-sm cursor-pointer" style={{ color: colors.text.starlight }}>
                <input type="radio" name="tier" checked={tier===t.v} onChange={() => setTier(t.v)} style={{ accentColor: colors.accent.webb }} />{t.l}
              </label>
            ))}
          </div>
          <div>
            <label className="mb-2 block" style={labelStyle}>Quando a IA deve oferecer?</label>
            <div className="space-y-1">
              {WHEN_TO_OFFER.map(w => (
                <label key={w} className="flex items-center gap-1.5 text-sm cursor-pointer" style={{ color: colors.text.starlight }}>
                  <input type="checkbox" checked={whenOffer.includes(w)} onChange={() => toggleList(whenOffer, w, setWhenOffer)} style={{ accentColor: colors.accent.webb }} />{w}
                </label>
              ))}
            </div>
          </div>
          <div>
            <label className="mb-2 block" style={labelStyle}>O que diferencia?</label>
            <div className="space-y-1">
              {DIFFERENTIATORS.map(d => (
                <label key={d} className="flex items-center gap-1.5 text-sm cursor-pointer" style={{ color: colors.text.starlight }}>
                  <input type="checkbox" checked={differentiators.includes(d)} onChange={() => toggleList(differentiators, d, setDifferentiators)} style={{ accentColor: colors.accent.webb }} />{d}
                </label>
              ))}
            </div>
          </div>
          <div>
            <label className="mb-2 block" style={labelStyle}>Escassez/Urgência</label>
            <select value={scarcity} onChange={e => setScarcity(e.target.value)} className={selectClass} style={inputStyle}>
              {SCARCITY.map(s => <option key={s.v} value={s.v}>{s.l}</option>)}
            </select>
          </div>
        </div>
      </div>
          ...
          return (
            ...
            >
              <div className="flex items-center gap-3">
                <button type="button" onClick={() => setObjectionStates({...objectionStates, [obj.id]: {...st, enabled: !st?.enabled}})}
                  className="relative inline-flex h-5 w-9 items-center rounded-full transition-colors flex-shrink-0"
                  style={{ backgroundColor: isEnabled ? colors.accent.webb : colors.background.corona }}
                >
                  <span className={`inline-block h-3.5 w-3.5 transform rounded-full bg-white transition-transform ${isEnabled ? "translate-x-[18px]" : "translate-x-[3px]"}`} />
                </button>
                <span className="text-sm font-semibold" style={{ color: isEnabled ? colors.text.starlight : colors.text.dust }}>{obj.label}</span>
              </div>
              <select
                value={st?.response}
                onChange={e => setObjectionStates({...objectionStates, [obj.id]: {...st, response: e.target.value}})}
                disabled={!isEnabled}
                className="max-w-[180px] rounded-lg px-2 py-1 text-xs focus:outline-none disabled:cursor-not-allowed"
                style={{ ...inputStyle, fontSize: "12px" }}
              >
                {obj.responses.map(r => <option key={r} value={r}>{r}</option>)}
              </select>
            </div>
          )
        })}
      ...
      <div className="rounded-xl p-5" style={cardStyle}>
        <div className="grid gap-6 md:grid-cols-2">
          <div>
            <p className="mb-2 text-xs font-semibold" style={{ color: colors.accent.webb }}>Prova Social</p>
            {SOCIAL_PROOF.map(s => (
              <div key={s} className="flex items-center gap-1.5 py-0.5">
                <label className="flex items-center gap-1.5 text-sm cursor-pointer" style={{ color: colors.text.starlight }}>
          ...
          <div>
            <p className="mb-2 text-xs font-semibold" style={{ color: colors.accent.webb }}>Garantia e Segurança</p>
            {GUARANTEE.map(g => (
          ...
          <div>
            <p className="mb-2 text-xs font-semibold" style={{ color: colors.accent.webb }}>Benefícios do Plano</p>
            {BENEFITS.map(b => (
          ...
          <div>
            <p className="mb-2 text-xs font-semibold" style={{ color: colors.accent.webb }}>Urgência</p>
            {URGENCY.map(u => (
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <div className="rounded-xl p-5" style={cardStyle}>
          ...
          {upsellEnabled && (
            <div className="mt-4 space-y-4 pl-2" style={{ borderLeft: `2px solid ${colors.accent.webb}30` }}>
              <div>
                <label className="mb-1.5 block" style={labelStyle}>Plano alvo</label>
                <select value={upsellTargetPlan} onChange={e => setUpsellTargetPlan(e.target.value)} className={selectClass} style={inputStyle}>
                  <option value="">Selecione o plano</option>
                  {siblingPlans.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
                </select>
              </div>
              <div>
                <label className="mb-1.5 block" style={labelStyle}>Quando oferecer</label>
                <div className="space-y-1">
                  {UPSELL_WHEN.map(w => (
                    <label key={w} className="flex items-center gap-1.5 text-sm cursor-pointer" style={{ color: colors.text.starlight }}>
                      <input type="checkbox" checked={upsellWhen.includes(w)} onChange={() => toggleList(upsellWhen, w, setUpsellWhen)} style={{ accentColor: colors.accent.webb }} />{w}
                    </label>
                  ))}
                </div>
              </div>
              <div>
                <label className="mb-1.5 block" style={labelStyle}>Argumento principal</label>
                {UPSELL_ARGUMENTS.map(a => (
                  <label key={a} className="flex items-center gap-1.5 text-sm cursor-pointer" style={{ color: colors.text.starlight }}>
                    <input type="radio" name="upsell_arg" checked={upsellArgument===a} onChange={() => setUpsellArgument(a)} style={{ accentColor: colors.accent.webb }} />{a}
                  </label>
                ))}
              </div>
            </div>
        ...
        <div className="rounded-xl p-5" style={cardStyle}>
          ...
          {downsellEnabled && (
            <div className="mt-4 space-y-4 pl-2" style={{ borderLeft: `2px solid ${colors.accent.gold}30` }}>
              <div>
                <label className="mb-1.5 block" style={labelStyle}>Plano alvo</label>
                <select value={downsellTargetPlan} onChange={e => setDownsellTargetPlan(e.target.value)} className={selectClass} style={inputStyle}>
                  <option value="">Selecione o plano</option>
                  {siblingPlans.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
                </select>
              </div>
              <div>
                ...
                <div className="space-y-1">
                  {DOWNSELL_WHEN.map(w => (
                    <label key={w} className="flex items-center gap-1.5 text-sm cursor-pointer" style={{ color: colors.text.starlight }}>
                      <input type="checkbox" checked={downsellWhen.includes(w)} onChange={() => toggleList(downsellWhen, w, setDownsellWhen)} style={{ accentColor: colors.accent.webb }} />{w}
                    </label>
                  ))}
                </div>
              </div>
              <div>
                ...
                {DOWNSELL_ARGUMENTS.map(a => (
                  <label key={a} className="flex items-center gap-1.5 text-sm cursor-pointer" style={{ color: colors.text.starlight }}>
                    <input type="radio" name="downsell_arg" checked={downsellArgument===a} onChange={() => setDownsellArgument(a)} style={{ accentColor: colors.accent.webb }} />{a}
                  </label>
                ))}
              </div>
            </div>
      ...
      <div className="rounded-xl p-5" style={cardStyle}>
        <label className="mb-3 block" style={labelStyle}>Tom da conversa</label>
          ...
          {TONES.map(t => {
            ...
            return (
              ...
              >
                ...
                <span className="text-xs font-semibold" style={{ color: isSelected ? colors.accent.webb : colors.text.starlight }}>{t.l}</span>
                <span className="text-[10px] leading-tight" style={{ color: colors.text.moonlight }}>{t.desc}</span>
              </button>
            )
          })}
        </div>
          ...
          <div>
            ...
            <div className="relative mt-2">
              ...
              <div className="flex justify-between mt-1">
                <span className="text-[10px]" style={{ color: colors.text.dust }}>Passivo</span>
                <span className="text-[10px]" style={{ color: colors.text.dust }}>Agressivo</span>
              </div>
            </div>
          </div>
          <div>
            <label className="mb-1.5 block" style={labelStyle}>Limite de mensagens</label>
            <select value={messageLimit} onChange={e => setMessageLimit(Number(e.target.value))} className={selectClass} style={inputStyle}>
              <option value={3}>3</option><option value={5}>5</option><option value={10}>10</option><option value={15}>15</option><option value={0}>Sem limite</option>
            </select>
          </div>
          <div>
            <label className="mb-1.5 block" style={labelStyle}>Follow-up</label>
            <select value={followUpHours} onChange={e => setFollowUpHours(e.target.value)} className={selectClass} style={inputStyle}>
              <option value="24">24h</option><option value="48">48h</option><option value="72">72h</option><option value="168">1 semana</option><option value="0">Nunca</option>
            </select>
            <select value={followUpMax} onChange={e => setFollowUpMax(e.target.value)} className={`${selectClass} mt-2`} style={inputStyle}>
              <option value="1">1 tentativa</option><option value="2">2 tentativas</option><option value="3">3 tentativas</option><option value="5">5 tentativas</option>
            </select>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-xl p-5" style={cardStyle}>
        ...
        {hasTechInfo && (
          <div className="grid gap-4 md:grid-cols-2 mt-4">
            <div>
              <label className="mb-1 block" style={labelStyle}>Modo de uso</label>
              <select value={usageMode} onChange={e => setUsageMode(e.target.value)} className={selectClass} style={inputStyle}>
                <option value="">Selecione</option>
                {USAGE_MODES.map(u => <option key={u} value={u}>{u}</option>)}
              </select>
            </div>
            <div>
              <label className="mb-1 block" style={labelStyle}>Duração</label>
              <select value={duration} onChange={e => setDuration(e.target.value)} className={selectClass} style={inputStyle}>
                <option value="">Selecione</option>
                {DURATIONS.map(d => <option key={d} value={d}>{d}</option>)}
              </select>
            </div>
            <div>
              <label className="mb-1 block" style={labelStyle}>Contraindicações</label>
              <div className="space-y-1">
                {CONTRAINDICATIONS.map(c => (
                  <label key={c} className="flex items-center gap-1.5 text-sm cursor-pointer" style={{ color: colors.text.starlight }}>
                    <input type="checkbox" checked={contraindications.includes(c)} onChange={() => toggleList(contraindications, c, setContraindications)} style={{ accentColor: colors.accent.webb }} />{c}
                  </label>
                ))}
              </div>
            </div>
            <div>
              <label className="mb-1 block" style={labelStyle}>Resultados esperados em</label>
              <select value={expectedResults} onChange={e => setExpectedResults(e.target.value)} className={selectClass} style={inputStyle}>
                <option value="">Selecione</option>
      ...
      <div className="rounded-xl p-5" style={{ background: colors.background.space, border: `1px solid ${colors.accent.webb}30`, boxShadow: "none" }}>
        <div className="flex items-start gap-3 mb-4">
          ...
          <div>
            <h4 className="text-sm font-semibold" style={{ fontFamily: typography.fontFamily.display, color: colors.text.starlight }}>Resumo do que a IA sabe sobre este plano</h4>
            <p className="mt-2 text-sm" style={{ color: colors.text.moonlight }}>{summary}</p>
          </div>
        </div>

        <div className="grid gap-2 md:grid-cols-4 lg:grid-cols-7 mt-4 pt-4" style={{ borderTop: `1px solid ${colors.border.space}` }}>
          {[
