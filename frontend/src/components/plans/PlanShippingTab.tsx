...
const SHIP_FROM = [{v:"my_address",l:"Meu endereço"},{v:"supplier",l:"Fornecedor"},{v:"distribution",l:"Centro de distribuição"},{v:"multiple",l:"Múltiplos endereços"}]
...
const FAQ_ANSWERS: Record<number, string[]> = {
  ...
  3: ["Envie fotos pelo WhatsApp para iniciarmos a troca", "Recusa na entrega e solicite reenvio", "Abra reclamação e garantimos o reenvio"],
...
export function PlanShippingTab({ planId, productId }: { planId: string; productId: string }) {
  ...
  const CosmosRadioGroup = ({ value, onChange, label, options }: { value: string; onChange: (v: string) => void; label: string; options: {value:string; label:string}[] }) => (
    <div>
      <label className="mb-2 block" style={labelStyle}>{label}</label>
      <div className="space-y-2">
        {options.map(opt => (
          <label key={opt.value} className="flex cursor-pointer items-start gap-2.5">
            <input type="radio" name={label} value={opt.value} checked={value === opt.value} onChange={() => onChange(opt.value)} style={{ accentColor: colors.accent.webb }} className="mt-0.5" />
            <span className="text-sm font-medium" style={{ color: colors.text.starlight }}>{opt.label}</span>
          </label>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="rounded-xl p-5" style={cardStyle}>
        ...
        <div className="grid gap-4 md:grid-cols-5">
          <div className="md:col-span-2">
            ...
            <select value={packageType} onChange={e => setPackageType(e.target.value)} className={`${selectClass} mt-1.5`} style={inputStyle}>
      ...
      <div className="rounded-xl p-5" style={cardStyle}>
        ...
        <div className="grid gap-8 md:grid-cols-2">
          <div className="space-y-4">
            <CosmosRadioGroup value={whoShips} onChange={setWhoShips} label="Quem realiza o envio? *" options={[{value:"self",label:"Eu mesmo"},{value:"supplier",label:"Meu fornecedor"},{value:"fulfillment",label:"Fulfillment"},{value:"dropshipping",label:"Dropshipping"}]} />

            {/* From where — 4 radio options */}
            <CosmosRadioGroup value={shipFrom} onChange={setShipFrom} label="De onde sai o envio? *" options={SHIP_FROM.map(s => ({value: s.v, label: s.l}))} />

            <div>
              <label style={labelStyle}>Prazo de despacho *</label>
              <select value={dispatchTime} onChange={e => setDispatchTime(e.target.value)} className={`${selectClass} mt-1.5`} style={inputStyle}>
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="space-y-1.5 mt-1.5">
                {CARRIERS.map(c => (
                  <label key={c} className="flex items-center gap-2 text-sm cursor-pointer" style={{ color: colors.text.starlight }}>
                    <input type="checkbox" checked={selectedCarriers.includes(c)} onChange={e => setSelectedCarriers(e.target.checked ? [...selectedCarriers, c] : selectedCarriers.filter(x => x !== c))} style={{ accentColor: colors.accent.webb }} />
                    {c}
                  </label>
                ))}
              </div>
            </div>
            <CosmosRadioGroup value={freightType} onChange={setFreightType} label="Frete para o comprador *" options={[{value:"free",label:"Grátis"},{value:"calculated",label:"Calculado pelo CEP"},{value:"fixed",label:"Fixo"},{value:"variable",label:"Variável por região"}]} />
            {freightType === "fixed" && (
              <div>
                <label style={labelStyle}>Valor fixo (R$)</label>
                <input type="number" step="0.01" value={fixedFreight} onChange={e => setFixedFreight(e.target.value)} className={`${inputClass} mt-1.5`} style={inputStyle} />
              </div>
            )}
          </div>
        </div>
      ...
      <div className="rounded-xl p-5" style={cardStyle}>
        ...
        <div className="overflow-x-auto rounded-lg" style={{ border: `1px solid ${colors.border.space}` }}>
          <table className="w-full text-sm">
            <thead style={{ background: colors.background.nebula, borderBottom: `1px solid ${colors.border.space}` }}>
              <tr>
                <th className="px-4 py-3 text-left" style={labelStyle}>Região</th>
                <th className="px-4 py-3 text-left" style={labelStyle}>Prazo estimado</th>
                <th className="px-4 py-3 text-left" style={labelStyle}>Observação</th>
              </tr>
            </thead>
            <tbody>
              {REGIONS.map((r, i) => (
                <tr key={r} style={{ background: i % 2 === 0 ? colors.background.space : colors.background.void, borderBottom: `1px solid ${colors.border.void}` }}>
                  <td className="px-4 py-3 font-medium" style={{ color: colors.text.starlight }}>{r}</td>
                  <td className="px-4 py-3">
                    <select value={regionPrazos[r]?.prazo} onChange={e => setRegionPrazos({...regionPrazos, [r]: {...regionPrazos[r], prazo: e.target.value}})}
                      className="rounded-lg px-2 py-1.5 text-xs focus:outline-none" style={inputStyle}>
                      {PRAZO_OPTIONS.map(p => <option key={p} value={p}>{p}</option>)}
                    </select>
                  </td>
                  <td className="px-4 py-3">
                    <select value={regionPrazos[r]?.obs} onChange={e => setRegionPrazos({...regionPrazos, [r]: {...regionPrazos[r], obs: e.target.value}})}
                      className="rounded-lg px-2 py-1.5 text-xs focus:outline-none" style={inputStyle}>
      ...
      {/* Tracking */}
      <div className="rounded-xl p-5" style={cardStyle}>
        <CosmosRadioGroup value={hasTracking} onChange={setHasTracking} label="O produto possui código de rastreamento?" options={[{value:"all",label:"Sim, todos os envios"},{value:"sedex",label:"Sim, apenas SEDEX"},{value:"no",label:"Não"}]} />
      </div>
      ...
      <div className="rounded-xl p-5" style={cardStyle}>
        ...
        <div className="space-y-3">
          {FAQ_QUESTIONS.map((q, i) => {
            ...
            return (
              <div key={i} className="rounded-xl overflow-hidden transition-all" style={{ background: colors.background.nebula, border: `1px solid ${colors.border.space}` }}>
                <button type="button" onClick={() => toggleFaq(i)} className="flex w-full items-center justify-between px-4 py-3 text-left transition-colors hover:opacity-80">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-medium" style={{ color: colors.text.starlight }}>{q}</span>
                    <span className="flex items-center gap-1 rounded-full px-2 py-0.5 text-[9px] font-semibold uppercase flex-shrink-0"
                      style={{ background: `${colors.accent.webb}15`, color: colors.accent.webb, letterSpacing: "0.05em" }}>
                      <Bot className="h-2.5 w-2.5" /> IA usa esta resposta
                    </span>
                  </div>
                  {isOpen ? <ChevronUp className="h-4 w-4 flex-shrink-0" style={{ color: colors.text.dust }} /> : <ChevronDown className="h-4 w-4 flex-shrink-0" style={{ color: colors.text.dust }} />}
                </button>
                <div className="overflow-hidden transition-all" style={{ maxHeight: isOpen ? "200px" : "0px", opacity: isOpen ? 1 : 0, transition: "max-height 300ms ease, opacity 250ms ease" }}>
                  <div className="px-4 pb-4">
                    <select value={faqAnswers[i]} onChange={e => setFaqAnswers({...faqAnswers, [i]: e.target.value})} className={selectClass} style={inputStyle}>
      ...
      <div className="flex items-start gap-3 rounded-xl p-5" style={{ background: `${colors.accent.webb}05`, border: `1px solid ${colors.accent.webb}30`, boxShadow: `0 0 20px ${colors.accent.webb}10, 0 0 40px ${colors.accent.webb}05` }}>
        <Sparkles className="mt-0.5 h-5 w-5 flex-shrink-0" style={{ color: colors.accent.webb }} />
        <p className="text-sm" style={{ color: colors.text.moonlight }}>A IA do Kloel usará todas as informações configuradas nesta página para responder automaticamente perguntas dos seus clientes sobre entrega, rastreamento e prazos.</p>
      </div>
