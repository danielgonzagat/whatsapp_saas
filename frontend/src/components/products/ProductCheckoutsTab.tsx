...
export function ProductCheckoutsTab({ productId }: { productId: string }) {
  ...
  const handleSubmit = async () => {
    setCreating(true)
    try {
      const body = { name: form.name, active: form.active, config: { paymentMethods: form.paymentMethods } }
      if (editingCheckoutId) {
        await apiFetch(`/products/${productId}/checkouts/${editingCheckoutId}`, { method: "PUT", body })
      } else {
  ...
  const handleEdit = (checkout: Checkout) => {
    ...
    setForm({
      name: checkout.name || "",
      paymentMethods: Array.isArray(checkout.config?.paymentMethods) && checkout.config.paymentMethods.length > 0 ? checkout.config.paymentMethods : ["PIX", "CARTAO"],
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => { resetForm(); setShowModal(true) }} className="flex items-center gap-2 rounded-md px-4 py-2 text-sm font-semibold" style={{ backgroundColor: colors.ember.primary, color: '#fff' }}><Plus className="h-4 w-4" /> Novo checkout</button>
      </div>
      <DataTable columns={[
        ...
        { key: "active", label: "Status", width: "10%", render: v => <span className="rounded-full px-2 py-0.5 text-xs font-medium" style={{ backgroundColor: v ? 'rgba(16,185,129,0.12)' : 'rgba(232,93,48,0.12)', color: v ? '#7FE2BC' : colors.ember.primary }}>{v ? 'ATIVO' : 'OFF'}</span> },
        { key: "config", label: "Resumo", width: "24%", render: v => <span className="text-xs" style={{ color: colors.text.muted }}>{Array.isArray(v?.paymentMethods) && v.paymentMethods.length > 0 ? `${v.paymentMethods.length} forma(s) habilitada(s)` : 'Checkout pronto para configurar no Kloel'}</span> },
        { key: "id", label: "Acoes", width: "16%", render: (_, row) => <div className="flex gap-1.5"><button onClick={() => handleEdit(row)} className="rounded-full p-1.5" style={{ backgroundColor: 'rgba(232,93,48,0.12)', color: colors.ember.primary }}><Pencil className="h-3.5 w-3.5" /></button><button onClick={() => handleDelete(row.id)} className="rounded-full p-1.5" style={{ backgroundColor: 'rgba(232,93,48,0.12)', color: colors.ember.primary }}><Trash2 className="h-3.5 w-3.5" /></button></div> },
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ backgroundColor: 'rgba(0,0,0,0.6)' }}>
          <div className="w-full max-w-md rounded-md p-6" style={{ backgroundColor: colors.background.surface, border: `1px solid ${colors.border.space}` }}>
            <div className="mb-4 flex items-center justify-between"><h3 className="text-lg font-semibold" style={{ color: colors.text.silver }}>{editingCheckoutId ? 'Editar checkout' : 'Novo checkout'}</h3><button onClick={() => { setShowModal(false); resetForm() }}><X className="h-5 w-5" style={{ color: colors.text.dim }} /></button></div>
            <div className="space-y-4">
              ...
              <label className="flex items-center gap-2 text-sm" style={{ color: colors.text.muted }}>
                <input type="checkbox" checked={form.active} onChange={e => setForm({ ...form, active: e.target.checked })} style={{ accentColor: colors.ember.primary }} />
                Checkout ativo
              </label>
            </div>
            <div className="mt-6 flex justify-end gap-3"><button onClick={() => { setShowModal(false); resetForm() }} className="rounded-md px-4 py-2 text-sm" style={{ border: `1px solid ${colors.border.space}`, color: colors.text.muted, backgroundColor: 'transparent' }}>Fechar</button><button onClick={handleSubmit} disabled={creating || !form.name} className="rounded-md px-4 py-2 text-sm font-semibold disabled:opacity-50" style={{ backgroundColor: colors.ember.primary, color: '#fff' }}>{creating ? "Salvando..." : editingCheckoutId ? "Salvar checkout" : "Criar checkout"}</button></div>
          </div>
        </div>
      )}
    </div>
