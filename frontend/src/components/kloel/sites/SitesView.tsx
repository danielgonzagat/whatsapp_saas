...
const IC: Record<string, (s: number) => React.ReactElement> = {
  ...
  server:  (s) => <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><rect x="2" y="2" width="20" height="8" rx="2"/><rect x="2" y="14" width="20" height="8" rx="2"/><circle cx="6" cy="6" r="1" fill="currentColor"/><circle cx="6" cy="18" r="1" fill="currentColor"/></svg>,
  ...
  trash:   (s) => <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 01-2 2H8a2 2 0 01-2-2L5 6m3 0V4a2 2 0 012-2h4a2 2 0 012 2v2"/></svg>,
  ...
  lock:    (s) => <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0110 0v4"/></svg>,
  refresh: (s) => <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><polyline points="23 4 23 10 17 10"/><path d="M20.49 15a9 9 0 11-2.12-9.36L23 10"/></svg>,
  chart:   (s) => <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/></svg>,
  eye:     (s) => <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>,
  cloud:   (s) => <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><path d="M18 10h-1.26A8 8 0 109 20h9a5 5 0 000-10z"/></svg>,
  ...
  upload:  (s) => <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><polyline points="16 16 12 12 8 16"/><line x1="12" y1="12" x2="12" y2="21"/><path d="M20.39 18.39A5 5 0 0018 9h-1.26A8 8 0 103 16.3"/></svg>,
  ...
  alert:   (s) => <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>,
...
function Badge({ children, color = EMBER }: { children: React.ReactNode; color?: string }) {
  return (
    <span style={{ fontFamily: MONO, fontSize: 10, padding: '2px 8px', borderRadius: 4, background: `${color}15`, color, border: `1px solid ${color}30` }}>
      {children}
    </span>
  );
}
...
function StatusDot({ status }: { status: 'online' | 'offline' | 'warning' | 'building' }) {
  ...
  return (
    <span style={{ display: 'inline-block', width: 8, height: 8, borderRadius: '50%', background: colors[status], boxShadow: status === 'online' ? `0 0 6px ${colors[status]}` : 'none' }} />
  );
}

function Btn({ children, variant = 'primary', onClick, disabled, small }: { children: React.ReactNode; variant?: 'primary' | 'ghost' | 'danger'; onClick?: () => void; disabled?: boolean; small?: boolean }) {
  ...
  return (
    <button onClick={onClick} disabled={disabled} style={{
      fontFamily: SORA, fontSize: small ? 11 : 12, padding: small ? '4px 10px' : '8px 16px', borderRadius: 6,
      cursor: disabled ? 'not-allowed' : 'pointer', opacity: disabled ? 0.5 : 1,
      display: 'flex', alignItems: 'center', gap: 6, whiteSpace: 'nowrap', fontWeight: 600, transition: 'all .2s',
      ...styles[variant],
    }}>
      {children}
    </button>
...
function Card({ children, style }: { children: React.ReactNode; style?: React.CSSProperties }) {
  return (
    <div style={{ background: BG_CARD, borderRadius: 8, border: `1px solid ${BORDER}`, padding: 16, ...style }}>
      {children}
    </div>
  );
}
...
function ProgressBar({ value, max = 100, color = EMBER }: { value: number; max?: number; color?: string }) {
...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return (
    <div style={{ fontFamily: SORA, fontSize: 10, color: TEXT_MUTED, letterSpacing: '0.25em', textTransform: 'uppercase', marginBottom: 10 }}>
      {children}
    </div>
  );
}

function Stat({ label, value, sub, icon }: { label: string; value: string; sub?: string; icon?: (s: number) => React.ReactElement }) {
  return (
    <Card style={{ textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
      {icon && <span style={{ color: EMBER, opacity: 0.6 }}>{icon(20)}</span>}
      <div style={{ fontFamily: SORA, fontSize: 10, color: TEXT_MUTED, textTransform: 'uppercase', letterSpacing: '0.15em' }}>{label}</div>
...
function EmptyState({ icon, title, subtitle }: { icon: (s: number) => React.ReactElement; title: string; subtitle: string }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 40, gap: 12 }}>
      ...
      <div style={{ fontFamily: SORA, fontSize: 13, color: TEXT_DIM, maxWidth: 400, textAlign: 'center' }}>{subtitle}</div>
    </div>
  );
}
...
function Toggle({ checked, onChange, label }: { checked: boolean; onChange: (v: boolean) => void; label?: string }) {
  return (
    <button onClick={() => onChange(!checked)} style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>
...
function VisaoGeral({ switchTab }: { switchTab: (id: string) => void }) {
  const sites = [
    { name: 'Landing Page Principal', domain: 'meusite.com.br', status: 'online' as const, views: 12450, uptime: 99.9 },
    { name: 'Pagina de Vendas', domain: 'vendas.meusite.com.br', status: 'online' as const, views: 8320, uptime: 99.8 },
    { name: 'Blog', domain: 'blog.meusite.com.br', status: 'building' as const, views: 0, uptime: 0 },
  ];
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <Card style={{ padding: 0, overflow: 'hidden' }}>
        <div style={{ padding: '12px 16px', borderBottom: `1px solid ${BORDER}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{ fontFamily: SORA, fontSize: 12, color: TEXT_DIM }}>Trafego em tempo real</span>
          <Badge>LIVE</Badge>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {sites.map((s, i) => (
            <Card key={i} style={{ display: 'flex', alignItems: 'center', gap: 14, cursor: 'pointer' }}>
              ...
              <Badge color={s.status === 'online' ? '#10B981' : '#8b5cf6'}>{s.status === 'online' ? 'Online' : 'Construindo'}</Badge>
              {s.views > 0 && <span style={{ fontFamily: MONO, fontSize: 11, color: TEXT_DIM }}>{Fmt(s.views)} visitas</span>}
              {s.uptime > 0 && <span style={{ fontFamily: MONO, fontSize: 11, color: '#10B981' }}>{s.uptime}%</span>}
            </Card>
          ))}
        </div>
      </div>
      ...
      <div>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          <Btn variant="primary" onClick={() => switchTab('criar')}>{IC.plus(14)} Criar Novo Site</Btn>
          <Btn variant="ghost" onClick={() => switchTab('dominios')}>{IC.globe(14)} Gerenciar Dominios</Btn>
          <Btn variant="ghost" onClick={() => switchTab('apps')}>{IC.puzzle(14)} Instalar Apps</Btn>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <Card>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <Input value={newDomain} onChange={setNewDomain} placeholder="meunovodominio.com.br" style={{ flex: 1 }} />
          <Btn variant="primary" disabled={!newDomain.trim()} onClick={() => setNewDomain('')}>{IC.plus(14)} Adicionar</Btn>
        </div>
      </Card>
      ...
      <Card style={{ padding: 0, overflow: 'hidden' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr 80px', gap: 0, padding: '10px 16px', borderBottom: `1px solid ${BORDER}` }}>
          {['Dominio', 'SSL', 'DNS', 'Status', 'Expira', ''].map((h, i) => (
            <div key={i} style={{ fontFamily: SORA, fontSize: 10, color: TEXT_MUTED, textTransform: 'uppercase', letterSpacing: '0.15em' }}>{h}</div>
          ))}
        </div>
        {domains.map((d, i) => (
          <div key={i} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr 80px', gap: 0, padding: '12px 16px', borderBottom: `1px solid ${BORDER}`, alignItems: 'center' }}>
            <div>
            ...
            <div>
              {d.ssl ? (
                <span style={{ display: 'flex', alignItems: 'center', gap: 4, color: '#10B981' }}>{IC.lock(12)} <span style={{ fontFamily: MONO, fontSize: 11 }}>Ativo</span></span>
              ) : (
                <span style={{ display: 'flex', alignItems: 'center', gap: 4, color: '#F59E0B' }}>{IC.alert(12)} <span style={{ fontFamily: MONO, fontSize: 11 }}>Pendente</span></span>
              )}
            </div>
            ...
            <div style={{ display: 'flex', gap: 4 }}>
              <button style={{ background: 'none', border: 'none', cursor: 'pointer', color: TEXT_DIM, padding: 4 }}>{IC.edit(14)}</button>
              <button style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#ef4444', padding: 4 }}>{IC.trash(14)}</button>
            </div>
          </div>
        ))}
      </Card>
          ...
          {[
            ...
            <React.Fragment key={i}>
              ...
              <div style={{ fontFamily: MONO, fontSize: 12, color: TEXT_DIM, padding: '6px 10px', background: BG_ELEVATED, borderRadius: 4, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                {r.value}
                <button onClick={() => navigator.clipboard.writeText(r.value)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: TEXT_DIM, padding: 2 }}>{IC.copy(12)}</button>
              </div>
            </React.Fragment>
          ))}
        </div>
          ...
          {[
            ...
            <div key={i}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                ...
                <span style={{ fontFamily: MONO, fontSize: 11, color: TEXT_DIM }}>{typeof r.value === 'number' && r.value < 100 ? r.value : r.value}{r.label === 'CPU' ? '%' : r.label === 'Memoria RAM' ? 'MB' : 'GB'} / {r.max}{r.label === 'CPU' ? '%' : r.label === 'Memoria RAM' ? 'MB' : 'GB'}</span>
              </div>
...
function CriarSite({ mode }: { mode?: string }) {
  ...
  if (phase === 'ask') return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: 400, gap: 20 }}>
      ...
      <textarea value={prompt} onChange={e => setPrompt(e.target.value)}
        ...
        onBlur={e => { e.currentTarget.style.borderColor = BORDER; }}
      />
      <Btn variant="primary" onClick={handleGenerate} disabled={!prompt.trim()}>{IC.zap(16)} Gerar Site com IA</Btn>
      ...
      {(loadingSites || savedSites.length > 0) && (
        <div style={{ width: '100%', maxWidth: 500, marginTop: 16 }}>
          ...
          {loadingSites && <div style={{ fontFamily: MONO, fontSize: 12, color: TEXT_DIM }}>Carregando...</div>}
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {savedSites.map(site => (
              <Card key={site.id} style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', padding: '10px 14px' }}>
                <span style={{ color: EMBER }} onClick={() => loadSavedSite(site)}>{IC.site(16)}</span>
                <span style={{ fontFamily: SORA, fontSize: 13, color: TEXT, flex: 1, cursor: 'pointer' }} onClick={() => loadSavedSite(site)}>{site.name || 'Site sem titulo'}</span>
                {site.published && <Badge color="#10B981">Publicado</Badge>}
                <span style={{ fontFamily: MONO, fontSize: 10, color: TEXT_DIM }}>{site.updatedAt ? new Date(site.updatedAt).toLocaleDateString('pt-BR') : ''}</span>
                <button onClick={() => handleDelete(site.id)} style={{ fontFamily: MONO, fontSize: 10, padding: '2px 8px', borderRadius: 4, border: `1px solid ${BORDER}`, background: 'transparent', color: '#ef4444', cursor: 'pointer' }}>X</button>
              </Card>
            ))}
          </div>
        </div>
  ...
  if (phase === 'building') return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: 400, gap: 20 }}>
      ...
      <div style={{ fontFamily: SORA, fontSize: 18, color: TEXT }}>Gerando seu site com IA...</div>
      <div style={{ fontFamily: MONO, fontSize: 12, color: EMBER }}>Isso pode levar alguns segundos</div>
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <Btn variant="ghost" small onClick={() => { setPhase('ask'); setError(''); setPublishedUrl(''); }}>Voltar</Btn>
          <span style={{ fontFamily: SORA, fontSize: 18, color: TEXT }}>Editor do Site</span>
          {savedSiteId && <span style={{ fontFamily: MONO, fontSize: 10, color: TEXT_MUTED }}>ID: {savedSiteId.slice(0, 8)}...</span>}
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
        ...
        <Input value={siteName} onChange={setSiteName} placeholder="Nome do site" style={{ maxWidth: 300 }} />
      </div>
      <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
        <Input value={editPrompt} onChange={setEditPrompt} placeholder="Pedir alteracao para a IA... Ex: Mude as cores para azul, adicione mais depoimentos" />
      ...
      <Card style={{ padding: 0, overflow: 'hidden', minHeight: 500 }}>
        <div style={{ background: BG_ELEVATED, padding: '6px 12px', borderBottom: `1px solid ${BORDER}`, display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <span style={{ fontFamily: MONO, fontSize: 10, color: TEXT_MUTED, marginLeft: 8 }}>Preview</span>
        </div>
...
function EditarSite({ mode }: { mode?: string }) {
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {savedSites.map(site => (
              <Card key={site.id} style={{ display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer', padding: '12px 16px' }}>
                <span style={{ color: EMBER }} onClick={() => setSelectedSite(site)}>{IC.site(20)}</span>
                ...
                {site.published && <Badge color="#10B981">Publicado</Badge>}
                <Btn variant="ghost" small onClick={() => setSelectedSite(site)}>{IC.edit(14)} Editar</Btn>
                <Btn variant="danger" small onClick={() => handleDelete(site.id)}>{IC.trash(14)}</Btn>
              </Card>
            ))}
          </div>
        )}
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <Btn variant="ghost" small onClick={() => setSelectedSite(null)}>Voltar</Btn>
          <span style={{ fontFamily: SORA, fontSize: 18, color: TEXT }}>{selectedSite.name || 'Site sem titulo'}</span>
          <span style={{ fontFamily: MONO, fontSize: 10, color: TEXT_MUTED }}>ID: {selectedSite.id?.slice(0, 8)}...</span>
        </div>
      ...
      <Card style={{ padding: 0, overflow: 'hidden', minHeight: 500 }}>
        ...
        <iframe ref={iframeRef} srcDoc={selectedSite.htmlContent || ''} sandbox="allow-scripts" style={{ width: '100%', height: 500, border: 'none', background: '#fff' }} title="Site Preview" />
      </Card>
    </div>
  );
}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 10 }}>
          {availableApps.map((app, i) => (
            <Card key={i} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <Btn variant="ghost" small>{IC.plus(12)} Instalar</Btn>
            </Card>
          ))}
        </div>
      </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <Card style={{ textAlign: 'center' }}>
        <div style={{ fontFamily: SORA, fontSize: 10, color: TEXT_MUTED, textTransform: 'uppercase', letterSpacing: '0.2em', marginBottom: 8 }}>Pontuacao de Seguranca</div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div>
                ...
                <div style={{ fontFamily: SORA, fontSize: 11, color: TEXT_DIM }}>Criptografia de dados em transito</div>
              </div>
            </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div>
                ...
                <div style={{ fontFamily: SORA, fontSize: 11, color: TEXT_DIM }}>Mitigacao de ataques distribuidos</div>
              </div>
            </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div>
                ...
                <div style={{ fontFamily: SORA, fontSize: 11, color: TEXT_DIM }}>Bloqueio de requisicoes maliciosas</div>
              </div>
            </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div>
                <div style={{ fontFamily: SORA, fontSize: 13, color: TEXT }}>Backups Automaticos</div>
                <div style={{ fontFamily: SORA, fontSize: 11, color: TEXT_DIM }}>Backup diario com 7 dias de retencao</div>
              </div>
            </div>
      ...
      <Card>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {[
            ...
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', background: BG_ELEVATED, borderRadius: 6 }}>
      ...
      <Card>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          {[
            ...
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', background: BG_ELEVATED, borderRadius: 6 }}>
              ...
              <Badge color={item.severity === 'alta' ? '#ef4444' : item.severity === 'media' ? '#F59E0B' : item.severity === 'baixa' ? '#3B82F6' : TEXT_DIM}>{item.severity}</Badge>
            </div>
          ))}
        </div>
      </Card>
      ...
      <div style={{ display: 'flex', gap: 4, marginBottom: 24, overflowX: 'auto', paddingBottom: 8 }}>
        {TABS.map(t => (
          <button key={t.id} onClick={() => switchTab(t.id)} style={{
            ...
            color: tab === t.id ? EMBER : TEXT_DIM, transition: 'all .2s',
          }}>
