...
function VisaoGeral({ switchTab }: { switchTab: (id: string) => void }) {
  ...
  useEffect(() => {
    apiFetch('/kloel/site/list')
      .then((res) => {
        if (res.data?.sites) setSites(res.data.sites);
      })
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      >
        <Stat label="Sites Criados" value={loading ? '--' : String(sites.length)} icon={IC.site} />
        <Stat label="Publicados" value={loading ? '--' : String(publishedCount)} icon={IC.globe} />
        <Stat
          label="SSL Ativos"
          value={loading ? '--' : String(publishedCount)}
          sub="Auto-provisionado"
          icon={IC.lock}
        />
      </div>
      ...
      <div>
        ...
        {loading ? (
          <Card style={{ padding: 20, textAlign: 'center' }}>
            <span style={{ fontFamily: MONO, fontSize: 12, color: TEXT_DIM }}>
              Carregando sites...
            </span>
          </Card>
        ) : sites.length === 0 ? (
          <EmptyState
            ...
            subtitle="Crie seu primeiro site com IA na aba Criar Site."
          />
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {sites.map((s) => (
              ...
              >
                <StatusDot status={s.published ? 'online' : 'offline'} />
                <div style={{ flex: 1 }}>
                  <div style={{ fontFamily: SORA, fontSize: 14, color: TEXT }}>
                    {s.name || 'Site sem titulo'}
                  </div>
                  {s.slug && (
                ...
                <Badge color={s.published ? '#10B981' : TEXT_DIM}>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <Card style={{ textAlign: 'center', padding: '40px 24px' }}>
        ...
        <div style={{ fontFamily: SORA, fontSize: 16, color: TEXT, marginTop: 16 }}>
          Gerenciamento de dominios em breve
        </div>
        <div
          style={{
            fontFamily: SORA,
        ...
        >
          Seus sites publicados estao disponiveis em URLs da Kloel automaticamente. O gerenciamento
          de dominios personalizados sera disponibilizado em uma atualizacao futura.
        </div>
      </Card>

      {/* DNS Instructions — kept as useful reference */}
      <Card>
        <SectionLabel>Referencia DNS (futuro)</SectionLabel>
        <div style={{ fontFamily: SORA, fontSize: 13, color: TEXT_DIM, marginBottom: 12 }}>
          Quando o gerenciamento de dominios estiver ativo, aponte os registros DNS para os
          servidores KLOEL:
        </div>
...
function Hospedagem() {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      {/* CDN Info */}
      <Card style={{ textAlign: 'center', padding: '40px 24px' }}>
        <span style={{ color: '#10B981' }}>{IC.cloud(48)}</span>
        <div style={{ fontFamily: SORA, fontSize: 16, color: TEXT, marginTop: 16 }}>
          Seus sites sao hospedados automaticamente na Kloel CDN
        </div>
        <div
          style={{
            fontFamily: SORA,
        ...
        >
          Todos os sites publicados pela Kloel sao entregues via CDN global com baixa latencia. Nao
          e necessario gerenciar servidores.
        </div>
      </Card>

      {/* What's Included */}
      <Card>
        <SectionLabel>Incluido no seu plano</SectionLabel>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          {[
            { label: 'SSL/TLS', value: "Automatico (Let's Encrypt)", icon: IC.lock },
            { label: 'CDN', value: 'Edge global, cache automatico', icon: IC.cloud },
            { label: 'Disponibilidade', value: 'Redundancia automatica', icon: IC.server },
            { label: 'Deploy', value: 'Instantaneo ao publicar', icon: IC.zap },
          ].map((info, i) => (
            <div
              key={i}
              style={{
            ...
            >
              <span style={{ color: EMBER, flexShrink: 0 }}>{info.icon(18)}</span>
              <div>
                <div
                  style={{
                    fontFamily: SORA,
...
function Protecao() {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      {/* Status Card */}
      <Card style={{ textAlign: 'center', padding: '40px 24px' }}>
        <span style={{ color: '#10B981' }}>{IC.shield(48)}</span>
        <div style={{ fontFamily: SORA, fontSize: 16, color: TEXT, marginTop: 16 }}>
          Protecao SSL e CDN ativas em todos os seus sites
        </div>
        <div
          style={{
            fontFamily: SORA,
        ...
        >
          Todos os sites publicados pela Kloel possuem certificado SSL automatico via Let&apos;s
          Encrypt e entrega via CDN com protecao contra ameacas basicas.
        </div>
      </Card>

      {/* What's Active */}
      <Card>
        <SectionLabel>Protecoes ativas</SectionLabel>
          ...
          {[
            {
              ...
              color: '#10B981',
              label: 'SSL/TLS (HTTPS)',
              desc: 'Certificado automatico para todos os sites publicados',
            },
            {
              icon: IC.cloud,
              color: '#3B82F6',
              label: 'CDN com cache',
              desc: 'Entrega otimizada com edge caching global',
            },
            {
              icon: IC.shield,
              color: EMBER,
              label: 'Headers de seguranca',
              desc: 'HSTS, X-Content-Type-Options, X-Frame-Options aplicados',
            },
          ].map((item, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <span style={{ color: item.color, flexShrink: 0 }}>{item.icon(18)}</span>
              <div style={{ flex: 1 }}>
                <div style={{ fontFamily: SORA, fontSize: 13, color: TEXT }}>{item.label}</div>
                <div style={{ fontFamily: SORA, fontSize: 11, color: TEXT_DIM }}>{item.desc}</div>
              </div>
