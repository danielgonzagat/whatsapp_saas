...
export function CenterStage({ size = 'L', children, className }: StageProps) {
  return (
    <div
      className={cn('w-full mx-auto px-4 sm:px-6', STAGE_WIDTHS[size], className)}
      style={{
...
export function Surface({ children, className, variant = 'card', padding = 'md' }: SurfaceProps) {
  return (
    <div
      className={cn('rounded-xl', SURFACE_PADDING[padding], className)}
      style={{
...
export function ModalSurface({ children, className, open = true, onClose }: ModalSurfaceProps) {
  ...
  return (
    ...
    >
      ...
      {/* Content */}
      <div
        className={cn('relative w-full max-w-lg rounded-2xl p-6', className)}
        style={{
          backgroundColor: colors.background.surface1,
          border: `1px solid ${colors.stroke}`,
          boxShadow: shadows.elevated,
...
export function Divider({ className, orientation = 'horizontal' }: DividerProps) {
  return (
    <div
      className={cn(orientation === 'horizontal' ? 'w-full h-px' : 'h-full w-px', className)}
