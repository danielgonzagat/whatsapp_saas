      ...
      <div style={{
        ...
        <input
          ...
          style={{
            ...
            borderRadius: 6, color: C.text, fontFamily: FONT.sans, fontSize: 13, outline: 'none',
        ...
        <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
          <button
            onClick={onClose}
            style={{
              padding: '9px 18px', background: 'none', border: `1px solid ${C.border}`,
              borderRadius: 6, color: C.secondary, fontFamily: FONT.sans, fontSize: 13, fontWeight: 500, cursor: 'pointer',
            }}
          >
            Cancelar
          </button>
          <button
            ...
            style={{
              padding: '9px 22px', background: C.ember, border: 'none', borderRadius: 6,
              color: '#fff', fontFamily: FONT.sans, fontSize: 13, fontWeight: 600,
              cursor: sending ? 'wait' : 'pointer', opacity: !email.trim() ? 0.5 : 1,
            }}
          >
...
function AffiliateDetailModal({ affiliate, onClose, onChat, onRevoke }: {
  affiliate: any;
  onClose: () => void;
  onChat: () => void;
  onRevoke: () => void;
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div onClick={onClose} style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)' }} />
      <div style={{
        position: 'relative', width: '100%', maxWidth: 560, background: C.card,
        border: `1px solid ${C.border}`, borderRadius: 6, padding: 28,
        maxHeight: '85vh', overflowY: 'auto' as const, animation: 'slideIn 200ms ease',
      }}>
        <button onClick={onClose} style={{ position: 'absolute', top: 16, right: 16, background: 'none', border: 'none', cursor: 'pointer', padding: 4 }}>
          <span style={{ color: C.secondary }}>{IC.x(16)}</span>
        </button>
        ...
        <div style={{ marginBottom: 20 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
            <span style={{ fontFamily: FONT.sans, fontSize: 11, color: C.secondary }}>Temperatura</span>
            <span style={{ fontFamily: FONT.mono, fontSize: 11, color: (a.temperature || 0) > 70 ? '#10B981' : '#F59E0B', fontWeight: 600 }}>
              {a.temperature || 0}%
            </span>
          </div>
          <TempBar value={a.temperature || 0} max={100} color={(a.temperature || 0) > 70 ? '#10B981' : '#F59E0B'} />
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, marginBottom: 20 }}>
          {statCards.map((sc, i) => (
            <div key={i} style={{
              ...
              <div style={{ fontFamily: FONT.mono, fontSize: 18, fontWeight: 700, color: sc.color, marginTop: 4 }}>
                {sc.value}
              </div>
            ...
            {chartData.map((v, i) => (
              <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                <div style={{
                  ...
                  borderRadius: 2, opacity: 0.6 + (v / chartMax) * 0.4, transition: 'height 300ms ease',
                }} />
              </div>
            ))}
          </div>
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          {(a.products && a.products.length > 0) ? (
            <div style={{ display: 'flex', flexWrap: 'wrap' as const, gap: 6 }}>
              {a.products.map((p: string, i: number) => (
                <span key={i} style={{
                  ...
                  background: C.bg, border: `1px solid ${C.border}`, borderRadius: 6,
                  fontFamily: FONT.sans, fontSize: 12, color: C.text,
                }}>
            ...
            <p style={{ fontFamily: FONT.sans, fontSize: 12, color: C.muted }}>Nenhum produto vinculado</p>
          )}
        </div>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={onChat} style={{
            ...
            padding: '10px 16px', background: C.ember, border: 'none', borderRadius: 6,
            color: '#fff', fontFamily: FONT.sans, fontSize: 13, fontWeight: 600, cursor: 'pointer',
          }}>
          ...
          <button onClick={handleCopyLink} style={{
            ...
            padding: '10px 16px', background: 'none', border: `1px solid ${C.border}`, borderRadius: 6,
            color: C.text, fontFamily: FONT.sans, fontSize: 13, fontWeight: 500, cursor: 'pointer',
          }}>
          ...
          <button onClick={onRevoke} style={{
            display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6,
            padding: '10px 16px', background: 'none', border: `1px solid rgba(239,68,68,0.3)`, borderRadius: 6,
            color: '#EF4444', fontFamily: FONT.sans, fontSize: 13, fontWeight: 500, cursor: 'pointer',
          }}>
...
function TabColaboradores({ search, setSearch, showInviteModal, setShowInviteModal }: {
  search: string;
  ...
  return (
    <div style={{ animation: 'fadeIn 300ms ease' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 24 }}>
        <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 6, padding: '20px 20px 16px', display: 'flex', flexDirection: 'column', gap: 4 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <span style={{ fontFamily: FONT.sans, fontSize: 12, color: C.secondary, fontWeight: 500 }}>Total Colaboradores</span>
            <span style={{ color: C.muted }}>{IC.users(16)}</span>
          </div>
          <span style={{ fontFamily: FONT.mono, fontSize: 24, fontWeight: 700, color: C.text, letterSpacing: '-0.02em' }}>{total}</span>
        </div>
        <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 6, padding: '20px 20px 16px', display: 'flex', flexDirection: 'column', gap: 4 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <span style={{ fontFamily: FONT.sans, fontSize: 12, color: C.secondary, fontWeight: 500 }}>Online Agora</span>
            <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#10B981' }} />
          </div>
          <span style={{ fontFamily: FONT.mono, fontSize: 24, fontWeight: 700, color: C.text, letterSpacing: '-0.02em' }}>{online}</span>
          <span style={{ fontFamily: FONT.sans, fontSize: 11, color: C.muted }}>ativos no momento</span>
        </div>
        <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 6, padding: '20px 20px 16px', display: 'flex', flexDirection: 'column', gap: 4 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <span style={{ fontFamily: FONT.sans, fontSize: 12, color: C.secondary, fontWeight: 500 }}>Convites Pendentes</span>
            <span style={{ color: C.muted }}>{IC.mail(16)}</span>
          </div>
          <span style={{ fontFamily: FONT.mono, fontSize: 24, fontWeight: 700, color: C.text, letterSpacing: '-0.02em' }}>{pendingInvites}</span>
        </div>
        <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 6, padding: '20px 20px 16px', display: 'flex', flexDirection: 'column', gap: 4 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <span style={{ fontFamily: FONT.sans, fontSize: 12, color: C.secondary, fontWeight: 500 }}>Funcoes Ativas</span>
            <span style={{ color: C.muted }}>{IC.shield(16)}</span>
          </div>
          <span style={{ fontFamily: FONT.mono, fontSize: 24, fontWeight: 700, color: C.text, letterSpacing: '-0.02em' }}>{rolesUsed}</span>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20, gap: 12 }}>
        <div style={{ position: 'relative', flex: 1, maxWidth: 360 }}>
          ...
          <input
            ...
            style={{
              width: '100%', padding: '9px 14px 9px 34px', background: C.card,
              border: `1px solid ${C.border}`, borderRadius: 6, color: C.text,
              fontFamily: FONT.sans, fontSize: 13, outline: 'none', boxSizing: 'border-box' as const,
            }}
          />
        </div>
        <button
          ...
          style={{
            ...
            background: C.ember, border: 'none', borderRadius: 6, color: '#fff',
      ...
      {/* Table header */}
      <div style={{
        display: 'grid', gridTemplateColumns: '44px 1fr 160px 100px 100px 120px',
        gap: 14, padding: '10px 18px', marginBottom: 4,
      }}>
        <span />
        <span style={{ fontFamily: FONT.sans, fontSize: 11, color: C.muted, fontWeight: 500, textTransform: 'uppercase' as const, letterSpacing: '0.05em' }}>Nome</span>
        <span style={{ fontFamily: FONT.sans, fontSize: 11, color: C.muted, fontWeight: 500, textTransform: 'uppercase' as const, letterSpacing: '0.05em' }}>Email</span>
        <span style={{ fontFamily: FONT.sans, fontSize: 11, color: C.muted, fontWeight: 500, textTransform: 'uppercase' as const, letterSpacing: '0.05em' }}>Funcao</span>
        <span style={{ fontFamily: FONT.sans, fontSize: 11, color: C.muted, fontWeight: 500, textTransform: 'uppercase' as const, letterSpacing: '0.05em' }}>Status</span>
        <span style={{ fontFamily: FONT.sans, fontSize: 11, color: C.muted, fontWeight: 500, textTransform: 'uppercase' as const, letterSpacing: '0.05em' }}>Ultimo acesso</span>
      </div>
          ...
          return (
            ...
            >
              ...
              <div style={{ position: 'relative' }}>
                ...
                <div style={{
                  width: 8, height: 8, borderRadius: '50%', background: c.status === 'online' ? '#10B981' : C.muted,
                  border: `2px solid ${C.card}`, position: 'absolute' as const, bottom: 0, right: 0,
                }} />
              </div>
              ...
              <span style={{
                display: 'inline-block', padding: '2px 8px', borderRadius: 4, fontSize: 10, fontWeight: 600,
                fontFamily: FONT.sans, color: roleConf.color, background: `${roleConf.color}15`,
                letterSpacing: '0.02em', textTransform: 'uppercase' as const, width: 'fit-content',
              }}>
                {roleConf.label}
              </span>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                <span style={{ fontFamily: FONT.sans, fontSize: 12, color: c.status === 'online' ? '#10B981' : C.muted }}>
...
function TabAfiliados({ search, setSearch, filterType, setFilterType, detailId, setDetailId }: {
  search: string;
  setSearch: (s: string) => void;
  filterType: string;
  ...
  const totalRevenue = affStats?.totalRevenue || displayAffiliates.reduce((sum: number, a: any) => sum + (a.revenue || 0), 0);
  const totalCommissions = affStats?.totalCommissions || displayAffiliates.reduce((sum: number, a: any) => sum + ((a.revenue || 0) * (a.commission || 0) / 100), 0);
  const topPartner = affStats?.topPartner || displayAffiliates.reduce((top: any, a: any) => (!top || (a.revenue || 0) > (top.revenue || 0) ? a : top), null)?.name || null;
    ...
    <div style={{ animation: 'fadeIn 300ms ease' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 14, marginBottom: 24 }}>
        <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 6, padding: '18px 16px 14px', display: 'flex', flexDirection: 'column', gap: 4 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <span style={{ fontFamily: FONT.sans, fontSize: 11, color: C.secondary, fontWeight: 500 }}>Afiliados Ativos</span>
            <span style={{ color: C.muted }}>{IC.users(14)}</span>
          </div>
          <span style={{ fontFamily: FONT.mono, fontSize: 22, fontWeight: 700, color: C.text }}>{activeAffiliates}</span>
        </div>
        <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 6, padding: '18px 16px 14px', display: 'flex', flexDirection: 'column', gap: 4 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <span style={{ fontFamily: FONT.sans, fontSize: 11, color: C.secondary, fontWeight: 500 }}>Produtores</span>
            <span style={{ color: C.muted }}>{IC.shield(14)}</span>
          </div>
          <span style={{ fontFamily: FONT.mono, fontSize: 22, fontWeight: 700, color: C.text }}>{producers}</span>
        </div>
        <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 6, padding: '18px 16px 14px', display: 'flex', flexDirection: 'column', gap: 4 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <span style={{ fontFamily: FONT.sans, fontSize: 11, color: C.secondary, fontWeight: 500 }}>Receita Total</span>
        ...
        <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 6, padding: '18px 16px 14px', display: 'flex', flexDirection: 'column', gap: 4 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <span style={{ fontFamily: FONT.sans, fontSize: 11, color: C.secondary, fontWeight: 500 }}>Comissoes Pagas</span>
        ...
        <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 6, padding: '18px 16px 14px', display: 'flex', flexDirection: 'column', gap: 4 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <span style={{ fontFamily: FONT.sans, fontSize: 11, color: C.secondary, fontWeight: 500 }}>Top Parceiro</span>
            <span style={{ color: C.ember }}>{IC.star(14)}</span>
          </div>
          <span style={{ fontFamily: FONT.sans, fontSize: 14, fontWeight: 600, color: C.ember, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' as const }}>
            {topPartner || '--'}
          </span>
        </div>
      </div>
      ...
      {/* Table header */}
      <div style={{
        display: 'grid', gridTemplateColumns: '46px 1fr 90px 70px 110px 90px 60px 100px',
        gap: 10, padding: '10px 18px', marginBottom: 4,
      }}>
        <span />
        <span style={{ fontFamily: FONT.sans, fontSize: 11, color: C.muted, fontWeight: 500, textTransform: 'uppercase' as const, letterSpacing: '0.05em' }}>Parceiro</span>
        <span style={{ fontFamily: FONT.sans, fontSize: 11, color: C.muted, fontWeight: 500, textTransform: 'uppercase' as const, letterSpacing: '0.05em' }}>Tipo</span>
        <span style={{ fontFamily: FONT.sans, fontSize: 11, color: C.muted, fontWeight: 500, textTransform: 'uppercase' as const, letterSpacing: '0.05em', textAlign: 'right' as const }}>Vendas</span>
        <span style={{ fontFamily: FONT.sans, fontSize: 11, color: C.muted, fontWeight: 500, textTransform: 'uppercase' as const, letterSpacing: '0.05em', textAlign: 'right' as const }}>Receita</span>
        <span style={{ fontFamily: FONT.sans, fontSize: 11, color: C.muted, fontWeight: 500, textTransform: 'uppercase' as const, letterSpacing: '0.05em', textAlign: 'right' as const }}>Comissao</span>
        <span style={{ fontFamily: FONT.sans, fontSize: 11, color: C.muted, fontWeight: 500, textTransform: 'uppercase' as const, letterSpacing: '0.05em', textAlign: 'right' as const }}>Taxa</span>
        <span style={{ fontFamily: FONT.sans, fontSize: 11, color: C.muted, fontWeight: 500, textTransform: 'uppercase' as const, letterSpacing: '0.05em' }}>Temperatura</span>
      </div>
        ...
        {filtered.map((a: any) => (
          ...
          >
            ...
            <div style={{
              ...
              color: a.type === 'producer' ? '#8B5CF6' : C.ember, flexShrink: 0,
            }}>
              {(a.name || '?')[0].toUpperCase()}
            </div>
            ...
            <span style={{
              display: 'inline-block', padding: '2px 8px', borderRadius: 4, fontSize: 10, fontWeight: 600,
              fontFamily: FONT.sans, width: 'fit-content',
              color: a.type === 'producer' ? '#8B5CF6' : C.ember,
              background: a.type === 'producer' ? 'rgba(139,92,246,0.15)' : C.emberStrong,
              letterSpacing: '0.02em', textTransform: 'uppercase' as const,
            }}>
            ...
            <div style={{ textAlign: 'right' as const }}>
              <span style={{ fontFamily: FONT.mono, fontSize: 14, fontWeight: 600, color: C.text }}>{a.totalSales || 0}</span>
            </div>
            ...
            <div style={{ textAlign: 'right' as const }}>
              <span style={{ fontFamily: FONT.mono, fontSize: 12, color: C.ember, fontWeight: 600 }}>
                R$ {((a.revenue || 0) * (a.commission || 0) / 100).toLocaleString('pt-BR', { minimumFractionDigits: 0 })}
              </span>
            </div>

            {/* Rate */}
            <div style={{ textAlign: 'right' as const }}>
              <span style={{ fontFamily: FONT.mono, fontSize: 13, fontWeight: 600, color: C.text }}>{a.commission || 0}%</span>
            </div>

            {/* Temperature */}
            <div style={{ width: 100 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                <span style={{ fontFamily: FONT.mono, fontSize: 10, fontWeight: 600, color: (a.temperature || 0) > 70 ? '#10B981' : (a.temperature || 0) > 40 ? '#F59E0B' : C.muted }}>
                  {a.temperature || 0}%
                </span>
              </div>
              <TempBar
                ...
                max={100}
                color={(a.temperature || 0) > 70 ? '#10B981' : (a.temperature || 0) > 40 ? '#F59E0B' : C.muted}
              />
            </div>
          </div>
        ))}
...
function TabChat({ selectedChat, setSelectedChat, chatInput, setChatInput, messages, setMessages, search, setSearch }: {
  selectedChat: any;
  ...
  const handleSend = async () => {
    ...
    const newMsg = { id: `local-${Date.now()}`, sender: 'Voce', content, time: new Date().toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' }), isMe: true };
  ...
  return (
    <div style={{
      display: 'flex', height: 'calc(100vh - 180px)', background: C.card,
      border: `1px solid ${C.border}`, borderRadius: 6, overflow: 'hidden',
      animation: 'fadeIn 300ms ease',
    }}>
      {/* Contact List - 280px */}
      <div style={{
        width: 280, borderRight: `1px solid ${C.divider}`, display: 'flex',
        ...
        <div style={{ padding: '16px 16px 12px', borderBottom: `1px solid ${C.divider}` }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
            ...
            {totalUnread > 0 && (
              <span style={{
                padding: '2px 8px', background: C.ember, borderRadius: 10,
                fontFamily: FONT.mono, fontSize: 11, fontWeight: 600, color: '#fff',
              }}>
                {totalUnread}
              </span>
            )}
          ...
          <div style={{ position: 'relative' }}>
            <div style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', color: C.muted }}>
              {IC.search(13)}
            </div>
            <input
              type="text"
              ...
              onChange={(e) => setSearch(e.target.value)}
              style={{
                width: '100%', padding: '8px 12px 8px 30px', background: C.bg,
                border: `1px solid ${C.border}`, borderRadius: 6, color: C.text,
                fontFamily: FONT.sans, fontSize: 12, outline: 'none', boxSizing: 'border-box' as const,
              }}
            />
          </div>
        </div>
            ...
            return (
              <div
                ...
                style={{
                  ...
                  cursor: 'pointer', background: isSelected ? C.emberBg : 'transparent',
              ...
              >
                ...
                <div style={{ position: 'relative', flexShrink: 0 }}>
                  ...
                  {contact.online && (
                    <div style={{
                      ...
                      borderRadius: '50%', background: '#10B981', border: `2px solid ${C.card}`,
                    }} />
                  )}
                </div>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 2 }}>
                    <span style={{
                      fontFamily: FONT.sans, fontSize: 13, fontWeight: contact.unread ? 600 : 500, color: C.text,
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <span style={{
                      fontFamily: FONT.sans, fontSize: 12, color: contact.unread ? C.secondary : C.muted,
                      fontWeight: contact.unread ? 500 : 400, overflow: 'hidden', textOverflow: 'ellipsis',
                    ...
                    {contact.unread > 0 && (
                      <span style={{
                        ...
                        borderRadius: 9, background: C.ember, fontFamily: FONT.mono, fontSize: 10,
                        fontWeight: 600, color: '#fff', padding: '0 4px', flexShrink: 0,
                      }}>
                        {contact.unread}
                      </span>
                    )}
          ...
          <>
              ...
              <div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <span style={{
                    ...
                    background: selectedChat.type === 'producer' ? 'rgba(139,92,246,0.15)' : C.emberStrong,
                    letterSpacing: '0.02em', textTransform: 'uppercase' as const,
                  }}>
            ...
            <div style={{
              ...
              {displayMessages.map((msg: any) => (
                ...
                >
                  <div style={{
                    maxWidth: '70%', padding: '10px 14px', borderRadius: 6,
                    ...
                    <div style={{
                      fontFamily: FONT.sans, fontSize: 13, color: msg.isMe ? '#fff' : C.text, lineHeight: 1.5,
                    }}>
                      {msg.content}
                    </div>
                    <div style={{
                      fontFamily: FONT.sans, fontSize: 10, color: msg.isMe ? 'rgba(255,255,255,0.6)' : C.muted,
            ...
            <div style={{
              ...
              <button
                ...
                style={{
                  ...
                  background: chatInput.trim() ? C.ember : C.elevated, border: 'none', borderRadius: 6,
                  cursor: chatInput.trim() ? 'pointer' : 'default', transition: 'background 150ms ease',
                  flexShrink: 0,
                }}
              >
