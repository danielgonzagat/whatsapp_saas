    ...
    return (
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: C.bg }}>
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 12 }}>
          <div style={{ width: 64, height: 64, borderRadius: '16%', background: C.card, border: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <p style={{ fontFamily: FONT.sans, fontSize: 16, color: C.muted, margin: 0, maxWidth: 300, textAlign: 'center' as const }}>{kloelT(`Escolha um parceiro na lista ao lado para iniciar ou continuar uma conversa`)}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: C.bg }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 20px', borderBottom: `1px solid ${C.divider}`, background: C.card }}>
        <div style={{ width: 36, height: 36, borderRadius: '16%', background: selectedChat.type === 'producer' ? 'rgba(139,92,246,0.12)' : C.elevated, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: FONT.sans, fontSize: 16, fontWeight: 600, color: selectedChat.type === 'producer' ? colors.semantic.purple : C.text }}>
          {(selectedChat.name || '?')[0].toUpperCase()}
        </div>
        <div>
          <div style={{ fontFamily: FONT.sans, fontSize: 16, fontWeight: 600, color: C.text }}>{selectedChat.name}</div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <span style={{ display: 'inline-block', padding: '2px 8px', borderRadius: 4, fontSize: 16, fontWeight: 600, fontFamily: FONT.sans, color: selectedChat.type === 'producer' ? colors.semantic.purple : C.ember, background: selectedChat.type === 'producer' ? 'rgba(139,92,246,0.15)' : C.emberStrong, letterSpacing: '0.02em', textTransform: 'uppercase' as const }}>
              {selectedChat.type === 'producer' ? 'Produtor' : 'Afiliado'}
            </span>
            {selectedChat.online && <span style={{ fontFamily: FONT.sans, fontSize: 16, color: colors.semantic.success }}>online</span>}
          </div>
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 12 }}>
        {displayMessages.length === 0 && (
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
            ...
            <p style={{ fontFamily: FONT.sans, fontSize: 16, color: C.muted, margin: 0 }}>{kloelT(`Nenhuma mensagem ainda`)}</p>
          </div>
        )}
        {displayMessages.map((msg) => (
          <div key={msg.id} style={{ display: 'flex', justifyContent: msg.isMe ? 'flex-end' : 'flex-start' }}>
            <div style={{ maxWidth: '70%', padding: '10px 14px', borderRadius: 6, background: msg.isMe ? C.ember : C.card, border: msg.isMe ? 'none' : `1px solid ${C.border}` }}>
              {!msg.isMe && <div style={{ fontFamily: FONT.sans, fontSize: 16, fontWeight: 600, color: C.ember, marginBottom: 4 }}>{msg.sender}</div>}
              <div style={{ fontFamily: FONT.sans, fontSize: 16, color: msg.isMe ? colors.text.silver : C.text, lineHeight: 1.5 }}>{msg.content}</div>
              <div style={{ fontFamily: FONT.sans, fontSize: 16, color: msg.isMe ? 'rgba(255,255,255,0.6)' : C.muted, textAlign: 'right' as const, marginTop: 4 }}>{msg.time}</div>
            </div>
          </div>
        ))}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 20px', borderTop: `1px solid ${C.divider}`, background: C.card }}>
        <input aria-label="Mensagem" type="text" placeholder={kloelT(`Digite sua mensagem...`)} value={chatInput}
          ...
          style={{ flex: 1, padding: '10px 14px', background: C.bg, border: `1px solid ${C.border}`, borderRadius: 6, color: C.text, fontFamily: FONT.sans, fontSize: 16, outline: 'none' }} />
