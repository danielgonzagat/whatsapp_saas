      ...
      <div style={{ padding: '16px 16px 12px', borderBottom: `1px solid ${C.divider}` }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <span style={{ fontFamily: FONT.sans, fontSize: 16, fontWeight: 600, color: C.text }}>{kloelT(`Conversas`)}</span>
          </div>
          {totalUnread > 0 && (
            <span style={{ padding: '2px 8px', background: C.ember, borderRadius: 8, fontFamily: FONT.mono, fontSize: 16, fontWeight: 600, color: colors.text.silver }}>{totalUnread}</span>
          )}
        </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
          ...
          return (
            <div key={contact.id} onClick={() => handleSelectContact(contact)}
              ...
              <div style={{ position: 'relative', flexShrink: 0 }}>
                ...
                {contact.online && <div style={{ position: 'absolute', bottom: 0, right: 0, width: 8, height: 8, borderRadius: '16%', background: colors.semantic.success, border: `2px solid ${C.card}` }} />}
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 2 }}>
                  <span style={{ fontFamily: FONT.sans, fontSize: 16, fontWeight: contact.unread ? 600 : 500, color: C.text, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' as const }}>{contact.name}</span>
                  <span style={{ fontFamily: FONT.sans, fontSize: 16, color: C.muted, flexShrink: 0, marginLeft: 8 }}>{contact.time}</span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <span style={{ fontFamily: FONT.sans, fontSize: 16, color: contact.unread ? C.secondary : C.muted, fontWeight: contact.unread ? 500 : 400, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' as const, flex: 1 }}>{contact.lastMessage}</span>
                  {(contact.unread || 0) > 0 && (
                    <span style={{ minWidth: 18, height: 18, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 8, background: C.ember, fontFamily: FONT.mono, fontSize: 16, fontWeight: 600, color: colors.text.silver, padding: '0 4px', flexShrink: 0 }}>{contact.unread}</span>
                  )}
                </div>
              </div>
            </div>
        ...
        {filteredContacts.length === 0 && displayContacts.length === 0 && (
          <div style={{ background: 'var(--app-bg-card)', border: '1px solid var(--app-border-primary)', borderRadius: 6, padding: '60px 20px', textAlign: 'center' as const }}>
            <span style={{ fontSize: 16, color: 'var(--app-text-tertiary)', display: 'block', marginBottom: 8 }}>{kloelT(`Nenhum contato`)}</span>
            <span style={{ fontSize: 16, color: 'var(--app-text-tertiary)' }}>{kloelT(`Conversas com parceiros aparecerao aqui`)}</span>
          </div>
        )}
        {filteredContacts.length === 0 && displayContacts.length > 0 && (
          <div style={{ textAlign: 'center' as const, padding: 32, color: C.muted }}>
            <span style={{ color: C.muted }}>{IC.chat(24)}</span>
            <p style={{ fontFamily: FONT.sans, fontSize: 16, marginTop: 8 }}>{kloelT(`Nenhuma conversa encontrada`)}</p>
          </div>
        )}
      </div>
    </div>
