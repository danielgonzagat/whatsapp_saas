...
type DeleteTarget = { id: string; agentName: string };

function stopDialogClose(event: React.MouseEvent<HTMLDivElement>) {
  event.stopPropagation();
}

function DialogFrameDescription({ description }: { description?: string }) {
  if (!description) {
    return null;
  }

  return (
    <p style={{ margin: '8px 0 0', fontSize: 12, color: V.t2, lineHeight: 1.5 }}>{description}</p>
  );
}
  ...
  return (
    ...
    >
      <div>
        ...
        <DialogFrameDescription description={description} />
      </div>
      <button
        type="button"
        onClick={onClose}
...
function DialogFrame(props: DialogFrameProps) {
  ...
  return (
    ...
    >
      ...
      >
        <DialogFrameHeader title={title} description={description} onClose={onClose} />
...
function AffiliateMetricCard({ label, value }: { label: string; value: string | number }) {
  return (
...
function AffiliateProgramStatusCard({
  affiliateProduct,
}: {
  affiliateProduct?: AffiliateProductRecord;
}) {
  return (
    <div
      style={{
...
function AffiliateRequestActions({
  request,
  requestActionId,
  onRequestAction,
}: {
  request: AffiliateRequestRecord;
  ...
  return (
    <>
      <Bt
        primary
        onClick={() => onRequestAction(String(request.id), 'approve')}
        style={{ padding: '4px 8px' }}
      >
        {requestActionId === `approve-${request.id}` ? 'Aprovando...' : 'Aprovar'}
      </Bt>
      <Bt
        onClick={() => onRequestAction(String(request.id), 'reject')}
...
function AffiliateRequestRow({
  ...
  return (
    ...
    >
      ...
      >
        <Bg color={request.status === 'APPROVED' ? V.g : request.status === 'REJECTED' ? V.r : V.y}>
...
function AffiliateRequestsPanel({
  requests,
  requestActionId,
  onRequestAction,
}: {
  requests: AffiliateRequestRecord[];
  requestActionId: string | null;
  onRequestAction: (requestId: string, action: 'approve' | 'reject') => void;
}) {
  return (
...
function AffiliateLinkRow({
  ...
  onLinkToggle: (linkId: string, active: boolean) => void;
}) {
  return (
    <div style={{ padding: '10px 0', borderBottom: `1px solid ${V.b}` }}>
      <div
        style={{
          display: 'flex',
          justifyContent: 'space-between',
      ...
      >
        ...
        >
          ...
          <Bt
            onClick={() => onLinkToggle(String(link.id), !link.active)}
            style={{ padding: '4px 8px' }}
          >
...
function AffiliateSummarySection({
  affiliateLoading,
  affiliateProduct,
  stats,
  requests,
  ...
  return (
    <div style={{ ...cs, padding: 24 }}>
      ...
      {affiliateLoading ? (
        ...
        <>
          <AffiliateProgramStatusCard affiliateProduct={affiliateProduct} />
          <div
            style={{
              display: 'grid',
          ...
          >
            {metricCards.map(([label, value]) => (
              <AffiliateMetricCard key={label} label={label} value={value} />
            ))}
          </div>
          <div
...
function DeleteCoprodutorDialog({
  deleteTarget,
  onClose,
  onDelete,
}: {
...
function AfiliadosSubTab(props: AfiliadosSubTabProps) {
  ...
  const affiliateProduct = affiliateSummary?.affiliateProduct as AffiliateProductRecord | undefined;
