          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            {detailType === 'sub' && item.status === 'ACTIVE' && (
              <>
                ...
                <button
                  disabled
                  title="Alterar plano em breve — use o chat para solicitar"
                  style={{
                    flex: 1,
                    padding: '10px 16px',
                    background: 'none',
...
export function VendasView({ defaultTab = 'vendas' }: VendasViewProps) {
  ...
  const handleChangePlan = async (_id: string) => {
    // Change plan requires a proper UI — marked as "Em breve"
    // No-op: the button in DetailModal is now disabled with tooltip
  };
      ...
      >
        {[
          ...
          },
          {
            ...
            metric: `${subStats?.lifecycle?.past_due || 0} atrasadas`,
