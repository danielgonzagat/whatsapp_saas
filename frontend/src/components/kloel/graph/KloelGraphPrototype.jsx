...
function GraphCanvas({ tab, recenterNonce, selectedId, onSelectNode, settings, dynamicGraph, channels }) {
  ...
  // Physics mutates nodesRef.current in place each frame; forceRender snapshots those
  // live positions into state so render reads STATE (compiler-safe), not the ref —
  // same per-frame cadence as the prior reducer bump, visual output identical.
  const [graphSnapshot, setGraphSnapshot] = useState([]);
  const forceRender = useCallback(() => { setGraphSnapshot(nodesRef.current.map(n => ({ ...n }))); }, []);
  const snapshotById = useMemo(() => { const m = new Map(); for (const sn of graphSnapshot) { m.set(sn.id, sn); } return m; }, [graphSnapshot]);
  ...
  useEffect(() => {
    ...
    for (let ri = nodesRef.current.length - 1; ri >= 0; ri--) { if (!newIds.has(nodesRef.current[ri].id)) { nodesRef.current.splice(ri, 1); } }
      ...
      <svg ref={svgRef} width={size.w} height={size.h} style={{ display: "block", userSelect: "none", position: "relative", zIndex: 2 }} onMouseDown={onSvgMouseDown} onMouseMove={onMouseMove} onMouseUp={onMouseUp} onMouseLeave={onMouseUp} onWheel={onWheel} onClick={onCanvasClick}>
        ...
        <g transform={`translate(${cx + pan.x}, ${cy + pan.y}) scale(${zoom})`}>
          {visibleEdges.map((e, i) => {
            const a = snapshotById.get(e.from);
            const b = snapshotById.get(e.to);
            if (!a || !b) {return null;}
...
function StepVoice({ ch, update, onDone, onBack }) {
  ...
  useEffect(() => { if (!ch.voice) {update({ voice: { tone: 1, edge: 1 } });} }, [ch.voice, update]);
  return (
    <div>
  ...
  return (
    <div style={{ width: "100%" }}>
      ...
      <button type="button" onClick={() => inputRef.current?.click()} style={{ width: "100%", padding: value ? 8 : "22px 14px", borderRadius: 6, background: "transparent", border: `1px dashed ${C.border}`, color: C.muted, fontSize: 12.5, fontFamily: FONT, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 10, transition: "all .15s ease" }}
        ...
        {value
          ? <Image src={value} alt="" width={200} height={120} unoptimized style={{ maxHeight: 120, maxWidth: "100%", borderRadius: 4, objectFit: "contain" }} />
...
function KloelInner() {
  ...
  // Sync real KYC/profile into editable local state via React's sanctioned
  // adjust-state-during-render pattern (re-syncs only when a source ref changes).
  const [prevAcctSrc, setPrevAcctSrc] = useState(null);
  if (!prevAcctSrc || prevAcctSrc.p !== realProfile || prevAcctSrc.f !== realFiscal || prevAcctSrc.b !== realBank || prevAcctSrc.d !== realDocs) {
    setPrevAcctSrc({ p: realProfile, f: realFiscal, b: realBank, d: realDocs });
  ...
  const products = useMemo(
    () => (Array.isArray(realProducts) ? realProducts.map(adaptRealProduct) : []),
    [realProducts],
  );
  ...
  const affiliate = useMemo(
    () => ({
      marketplace: [],
      myAffiliates: (Array.isArray(realAffiliates) ? realAffiliates : []).map(adaptRealAffiliate),
      partnerChats: [],
      collaborators: [],
    }),
    [realAffiliates],
  ...
  const educar = useMemo(
    () => ({ areas: (Array.isArray(realAreas) ? realAreas : []).map(adaptRealArea) }),
    [realAreas],
  );
  ...
  // Sync real CRM data into editable local state via the same render-time pattern.
  const [prevCrmSrc, setPrevCrmSrc] = useState(null);
  if (!prevCrmSrc || prevCrmSrc.c !== realContacts || prevCrmSrc.d !== realDeals || prevCrmSrc.p !== realPipelines) {
    setPrevCrmSrc({ c: realContacts, d: realDeals, p: realPipelines });
  ...
  // Recolor settings groups when the theme mode flips, via the render-time seed pattern.
  const [prevColorMode, setPrevColorMode] = useState(null);
  if (prevColorMode !== mode) {
    setPrevColorMode(mode);
