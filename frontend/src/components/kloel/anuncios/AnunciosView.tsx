...
const INITIAL_PLATFORMS: Record<'meta' | 'google' | 'tiktok', PlatformData> = {
  meta: {
    name: 'Meta Ads',
    color: '#1877F2',
    spend: 0,
...
// Campaign type — state is managed via useState in the main component
type Campaign = {
  id: string;
...
function WarRoom({
  ...
  platforms: Record<'meta' | 'google' | 'tiktok', PlatformData>;
  campaigns: Campaign[];
    ...
    >
      ...
      >
        ...
        {campaigns.length > 0 ? (
          ...
          >
            Nenhuma campanha sincronizada. Conecte uma plataforma de anuncios para importar
...
function TrackingTab({ focus }: { focus?: string }) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <div>
            ...
            >
              Use rastreamento para captar o abandono, acionar follow-ups, abrir campanhas e medir o
      ...
      {/* Setup required — tracking metrics */}
      <div
        style={{
          background: '#111113',
      ...
      >
        ...
        >
          <span style={{ color: EMBER }}>{IC.link(20)}</span>
          <div style={{ fontSize: 15, fontFamily: SORA, color: '#E0DDD8', fontWeight: 600 }}>
            Configure o pixel de rastreamento
          </div>
        </div>
        <div
          style={{
        ...
        >
          Configure o pixel de rastreamento para ver metricas de conversao. Conecte o Meta Pixel e o
          Google Tag para medir vendas rastreadas, eventos, postbacks, UTMs e atribuicao.
        </div>
        <div
          style={{
            display: 'grid',
        ...
        >
          {[
            { label: 'VENDAS RASTREADAS', value: '\u2014' },
            { label: 'PIXEL FIRES', value: '\u2014' },
            { label: 'POSTBACKS', value: '\u2014' },
            { label: 'ATRIBUICAO', value: '\u2014' },
          ].map((s) => (
            <div key={s.label} style={{ background: '#19191C', borderRadius: 6, padding: 14 }}>
              <div
                style={{
                  fontSize: 10,
                  fontFamily: MONO,
              ...
              <div style={{ fontSize: 20, fontWeight: 700, fontFamily: MONO, color: '#3A3A3F' }}>
                {s.value}
              </div>
            </div>
          ))}
          ...
          {[
            {
              step: '1',
              title: 'Meta Pixel',
              desc: 'Acesse o Meta Events Manager, copie seu Pixel ID e insira em Configuracoes > Integracoes > Meta Pixel.',
            },
            {
              step: '2',
              title: 'Google Tag',
              desc: 'Acesse o Google Tag Manager, copie o ID da tag (GTM-XXXXX) e insira em Configuracoes > Integracoes > Google Tag.',
            },
            {
              step: '3',
              title: 'Verificar eventos',
              desc: 'Apos configurar, os eventos PageView, AddToCart, InitiateCheckout e Purchase serao rastreados automaticamente.',
            },
          ].map((item) => (
            <div
              key={item.step}
...
export default function AnunciosView({ defaultTab = 'visao' }: { defaultTab?: string }) {
  ...
  // ── Platform & campaign state (React-managed for proper re-renders) ──
  const [platforms, setPlatforms] =
    useState<Record<'meta' | 'google' | 'tiktok', PlatformData>>(INITIAL_PLATFORMS);
