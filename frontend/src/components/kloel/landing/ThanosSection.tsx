...
const SALES_CHANNELS: Record<ChannelKey, { n: string; c: string }> = {
  wa: { n: 'WhatsApp', c: 'rgb(37, 211, 102)' },
  ig: { n: 'Instagram', c: 'rgb(225, 48, 108)' },
  fb: { n: 'Messenger', c: 'rgb(0, 132, 255)' },
  em: { n: 'Email', c: E },
  sms: { n: 'SMS', c: SUCCESS },
  tt: { n: 'TikTok', c: 'rgb(255, 0, 80)' },
};
    ...
    <div style={{ animation: runToken ? 'thanosIn .6s cubic-bezier(.22,1,.36,1) both' : 'none' }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'var(--c3)', gap: 16 }}>
        {(Object.keys(SALES_CHANNELS) as ChannelKey[]).map((key) => (
          <div
            key={key}
            style={{ background: SURFACE, borderRadius: 6, border: '1px solid #19191C' }}
          >
            <div
              style={{
                padding: '8px 12px',
