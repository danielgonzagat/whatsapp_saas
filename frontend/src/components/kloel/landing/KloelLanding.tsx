...
function Heartbeat() {
  const cv = useRef<HTMLCanvasElement>(null), raf = useRef(0), wp = useRef(0), h = useRef<Float32Array | null>(null), wi = useRef(0), si = useRef(0), wv = useRef<number[][]>([]);
  useEffect(() => {
...
function HeroLoop() {
  ...
  const [gx, setGx] = useState<{on:boolean;text:string;shk:number[];chr:number;slices:any[];flash:boolean}>({ on: false, text: "", shk: [0,0], chr: 0, slices: [], flash: false });
  const [resurrected, setResurrected] = useState(false);
  const noiseRef = useRef<HTMLCanvasElement>(null);
  const gxRef = useRef(false);
  const m = useRef(true);
  ...
  useEffect(() => {
    ...
    const ctx = cv.getContext("2d"); if (!ctx) return; cv.width = 600; cv.height = 120;
    let raf2: number;
    const drawN = () => {
  ...
  const scramble = (src: string, chaos: number) => src.split("").map((c: string) => c===" "?" ":Math.random()<chaos?rc():c).join("");
...
function MultiChannel() {
  const [msgs, setMsgs] = useState<Record<string,any[]>>({wa:[],ig:[],em:[]});
  ...
  const colors: Record<string,string>={wa:"#25D366",ig:"#E1306C",em:E};
  const names: Record<string,string>={wa:"WhatsApp",ig:"Instagram DM",em:"Email"};
  const Panel=({ch}: {ch: string})=>(
...
export default function KloelLanding() {
  ...
  return (
    <div style={{background:V,color:"#E0DDD8",fontFamily:F,overflowX:"hidden"}}>
      ...
      <div style={{borderTop:"1px solid #19191C",background:"#111113"}}>
        <section style={{padding:"100px 24px",maxWidth:1000,margin:"0 auto"}}>
      ...
      <div style={{borderTop:"1px solid #19191C",background:"#111113"}}>
        <section style={{padding:"100px 24px",maxWidth:860,margin:"0 auto"}}>
          ...
          <div className="grid2" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:24,alignItems:"start"}}>
            <Reveal><div style={{background:V,border:"1px solid #222226",borderRadius:6,padding:20}}><div style={{fontFamily:M,fontSize:9,color:"#3A3A3F",letterSpacing:".1em",marginBottom:12}}>FERRAMENTAS SEPARADAS</div>{[["Automacao email","R$189"],["Chatbot","R$75"],["Funis","R$500"],["Hospedagem","R$45"],["CRM","R$300"],["Chat","R$90"],["SMS","R$120"],["Afiliados","R$200"]].map(([t,p])=><div key={t} style={{display:"flex",justifyContent:"space-between",padding:"5px 0",borderBottom:"1px solid #222226"}}><span style={{fontSize:11,color:"#6E6E73"}}>{t}</span><span style={{fontSize:10,color:"#3A3A3F",fontFamily:M}}>{p}</span></div>)}<div style={{display:"flex",justifyContent:"space-between",padding:"10px 0 0",marginTop:6}}><span style={{fontSize:12,fontWeight:600,color:"#6E6E73"}}>Total</span><span style={{fontSize:16,fontWeight:800,color:"#EF4444",fontFamily:M}}>R$1.519+/mes</span></div></div></Reveal>
            <Reveal delay={200}><div style={{background:V,border:`2px solid ${E}`,borderRadius:6,padding:22,position:"relative"}}><div style={{position:"absolute",top:-1,left:"50%",transform:"translateX(-50%)",background:E,padding:"2px 12px",borderRadius:"0 0 4px 4px",fontSize:9,fontWeight:700,color:V,fontFamily:M,letterSpacing:".08em"}}>KLOEL</div><div style={{textAlign:"center",padding:"24px 0 16px"}}><div style={{fontSize:48,fontWeight:800,fontFamily:M,letterSpacing:"-.04em"}}>R$ 0</div><div style={{fontSize:14,color:"#6E6E73",marginTop:4}}>por mes</div><div style={{fontSize:12,color:E,fontWeight:600,marginTop:10}}>Taxa apenas sobre vendas.</div><div style={{fontSize:11,color:"#3A3A3F",marginTop:2}}>Sem venda, sem custo.</div></div></div></Reveal>
          </div>
        </section>
      </div>
      ...
      <div id="ativar" style={{borderTop:"1px solid #19191C"}}>
      ...
      <footer style={{borderTop:"1px solid #19191C",padding:"36px 24px"}}>
