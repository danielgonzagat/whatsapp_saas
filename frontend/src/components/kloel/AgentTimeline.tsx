            ...
            <div key={step.id} className="flex items-start gap-3 py-2 relative">
              ...
              {idx < steps.length - 1 && (
                <div
                  ...
                  style={{
                    backgroundColor:
                      step.status === 'completed' ? colors.brand.green : colors.stroke,
                  }}
                />
              )}
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span
                    ...
                    style={{
                      color: step.status === 'error' ? colors.state.error : colors.text.primary,
                    }}
                  >
                    {step.title}
                  </span>
...
export function AgentTimelineMini({
  ...
  const currentStep = steps.find(
    (s) => s.status === 'in-progress' || s.status === 'awaiting-confirmation',
  );
  const completedCount = steps.filter((s) => s.status === 'completed').length;
