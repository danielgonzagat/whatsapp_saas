...
export async function runCreateContact(form: ContactFormSnapshot): Promise<{ ok: true } | { ok: false; error: string }> {
  if (!form.phone.trim()) {
...
export async function runCreatePipeline(name: string): Promise<{ ok: true; id?: string } | { ok: false; error: string }> {
  if (!name.trim()) {
...
export async function runCreateDeal(form: DealFormSnapshot): Promise<{ ok: true } | { ok: false; error: string }> {
...
export async function runAutoSegment(): Promise<{ ok: true; processed: number } | { ok: false; error: string }> {
  try {
    ...
    return { ok: false, error: errorMessage(segmentError, 'Nao foi possivel rodar a auto-segmentacao.') };
  }
}
