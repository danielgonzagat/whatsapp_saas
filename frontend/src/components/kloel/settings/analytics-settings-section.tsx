...
import { kloelSettingsClass, SettingsCard, SettingsHeader, SettingsMetricTile, SettingsNotice } from "./contract"
      ...
      {loading && !stats ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
            <SettingsCard>
              <SettingsHeader icon={<BarChart3 className="h-4 w-4" />} title="Atividade diária" className="mb-0" />
              <div className="mt-4 space-y-2">
                {activity.length === 0 ? (
                  <p className="text-sm text-[#6E6E73]">Sem atividade diária consolidada ainda.</p>
                ) : (
                  activity.slice(-7).map((item) => (
                    <div key={item.date} className="flex items-center justify-between rounded-md border border-[#19191C] bg-[#0A0A0C] px-4 py-3">
                      <span className="text-sm font-medium text-[#E0DDD8]">{formatShortDate(item.date)}</span>
              ...
              <div className="mt-4 grid grid-cols-2 gap-3">
                <SettingsMetricTile>
                  ...
                  <p className="mt-1 text-lg font-semibold text-[#E0DDD8]">{activityTotals.inbound}</p>
                </SettingsMetricTile>
                <SettingsMetricTile>
                  <p className="text-xs font-medium text-[#6E6E73]">Outbound 7d</p>
                  <p className="mt-1 text-lg font-semibold text-[#E0DDD8]">{activityTotals.outbound}</p>
                </SettingsMetricTile>
              </div>
            </SettingsCard>
            ...
            <SettingsCard>
              ...
              <div className="mt-4 grid grid-cols-3 gap-3">
                <SettingsMetricTile><p className="text-xs font-medium text-[#6E6E73]">Positivo</p><p className="mt-1 text-lg font-semibold text-[#E0DDD8]">{stats.sentiment.positive}</p></SettingsMetricTile>
                <SettingsMetricTile><p className="text-xs font-medium text-[#6E6E73]">Neutro</p><p className="mt-1 text-lg font-semibold text-[#E0DDD8]">{stats.sentiment.neutral}</p></SettingsMetricTile>
                <SettingsMetricTile><p className="text-xs font-medium text-[#6E6E73]">Negativo</p><p className="mt-1 text-lg font-semibold text-[#E0DDD8]">{stats.sentiment.negative}</p></SettingsMetricTile>
              </div>
              <div className="mt-4 grid grid-cols-3 gap-3">
                <SettingsMetricTile><p className="text-xs font-medium text-[#6E6E73]">Lead alto</p><p className="mt-1 text-lg font-semibold text-[#E0DDD8]">{stats.leadScore.high}</p></SettingsMetricTile>
                <SettingsMetricTile><p className="text-xs font-medium text-[#6E6E73]">Lead médio</p><p className="mt-1 text-lg font-semibold text-[#E0DDD8]">{stats.leadScore.medium}</p></SettingsMetricTile>
                <SettingsMetricTile><p className="text-xs font-medium text-[#6E6E73]">Lead baixo</p><p className="mt-1 text-lg font-semibold text-[#E0DDD8]">{stats.leadScore.low}</p></SettingsMetricTile>
              </div>
            </SettingsCard>
          </div>
              ...
              <SettingsCard>
                ...
                <div className="mt-4 space-y-2">
                  {(advanced.funnels.topFlows || []).slice(0, 5).map((flow) => (
                    <div key={flow.flowId} className="flex items-center justify-between rounded-md border border-[#19191C] bg-[#0A0A0C] px-4 py-3">
                      <span className="text-sm text-[#E0DDD8]">{flow.name || flow.flowId}</span>
                      <span className="text-sm font-semibold text-[#E0DDD8]">{flow.executions}</span>
                    </div>
                  ))}
                  {(advanced.queues.stats || []).slice(0, 4).map((queue) => (
                    <div key={queue.id} className="flex items-center justify-between rounded-md border border-[#19191C] bg-[#0A0A0C] px-4 py-3">
                      <span className="text-sm text-[#E0DDD8]">{queue.name}</span>
                      <span className="text-sm font-semibold text-[#E0DDD8]">{queue.waitingCount}</span>
                    </div>
                  ))}
                </div>
              </SettingsCard>
