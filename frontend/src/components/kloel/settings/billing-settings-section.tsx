...
export function BillingSettingsSection({
  ...
  const estimatedMessages = useMemo(() => Math.max(0, Math.floor(creditsBalance * 100)), [creditsBalance])
  const creditPercent = useMemo(() => Math.max(0, Math.min(100, (creditsBalance / 5) * 100)), [creditsBalance])

  const loadPaymentMethods = useCallback(async () => {
    try {
      const response = await billingApi.getPaymentMethods()
      const paymentMethods = (response.data as Record<string, any> | undefined)?.paymentMethods || []
      setCards(
        paymentMethods.map((pm: any) => ({
          id: pm.id,
          last4: pm.last4,
          brand: pm.brand ? String(pm.brand).toUpperCase() : "CARD",
          expiry: pm.expMonth && pm.expYear ? `${String(pm.expMonth).padStart(2, "0")}/${String(pm.expYear).slice(-2)}` : "",
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className={`mt-1 ${kloelSettingsClass.sectionDescription}`}>
          O Kloel cria checkouts, processa cobrancas e opera a infraestrutura financeira internamente. Aqui voce cuida apenas da sua assinatura do Kloel.
        </p>
      </div>
      ...
      <SettingsCard>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <SettingsInset className="p-4">
            ...
            <p className="mt-2 text-xl font-semibold text-[#E0DDD8]">US$ {creditsBalance.toFixed(2)}</p>
            <p className="text-xs text-[#6E6E73]">Aproximadamente {estimatedMessages} mensagens disponiveis.</p>
            <div className="mt-3 h-2 overflow-hidden rounded-full bg-[#19191C]">
              <div className="h-full rounded-full bg-[#E85D30]" style={{ width: `${creditPercent}%` }} />
            </div>
          </SettingsInset>
          ...
          <SettingsInset className="p-4">
            ...
            <p className="mt-2 text-xl font-semibold text-[#E0DDD8]">{salesReport?.totalSales || 0}</p>
            <p className="text-xs text-[#6E6E73]">
              {formatMoney(salesReport?.totalAmount || 0)} em {salesPeriod === "week" ? "7 dias" : "30 dias"}.
            </p>
            <div className="mt-3 flex gap-2">
              <Button
                variant={salesPeriod === "week" ? "default" : "outline"}
                onClick={() => setSalesPeriod("week")}
                className={salesPeriod === "week" ? "bg-[#E0DDD8] text-[#0A0A0C] hover:bg-[#E0DDD8]" : ""}
              >
                7 dias
              </Button>
              <Button
                ...
                className={salesPeriod === "month" ? "bg-[#E0DDD8] text-[#0A0A0C] hover:bg-[#E0DDD8]" : ""}
              >
                30 dias
              </Button>
            </div>
          ...
          <SettingsInset className="p-4">
            ...
            <p className="mt-2 text-sm font-semibold text-[#E0DDD8]">Checkout, pagamento e repasse</p>
            <p className="mt-1 text-xs leading-relaxed text-[#6E6E73]">
              O Kloel controla checkout, meios de pagamento, confirmacao e rotas de cobranca sem exigir configuracao manual do usuario.
            </p>
          </SettingsInset>
        </div>
        ...
        {creditsBalance < 1 ? (
          <SettingsNotice tone="warning" className="mt-4 flex items-center gap-2">
            ...
            O saldo operacional esta baixo. Revise sua assinatura e forma de pagamento para evitar interrupcoes.
          </SettingsNotice>
        ) : null}
        ...
        {subscriptionStatus === "none" ? (
          <div className="mt-4 space-y-4">
            ...
            <Button onClick={handleActivateTrialClick} className={`w-full ${kloelSettingsClass.primaryButton}`}>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            <Button onClick={() => setShowManageModal(true)} className={kloelSettingsClass.primaryButton}>
              Gerenciar assinatura
            </Button>
            <Button variant="outline" onClick={() => void startAddCardFlow()} className={kloelSettingsClass.outlineButton}>
      ...
      <SettingsCard>
        ...
        {cards.length === 0 ? (
          <div className="rounded-md border border-dashed border-[#222226] bg-[#0A0A0C] px-4 py-8 text-center">
            ...
            <p className="mt-1 text-xs text-[#6E6E73]">Cadastre um cartao para manter sua assinatura ativa.</p>
            <Button onClick={() => void startAddCardFlow()} className={`mt-4 ${kloelSettingsClass.primaryButton}`}>
              <Plus className="mr-2 h-4 w-4" />
              Adicionar cartao
            </Button>
          </div>
          ...
          <div className="space-y-3">
            {cards.map((card) => (
              <SettingsInset key={card.id} className="flex flex-wrap items-center justify-between gap-3 p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-[#6E6E73]">{card.expiry || "Sem validade informada"}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  {card.isDefault ? (
                    ...
                    <Button variant="outline" onClick={() => void handleSetDefault(card.id)} className={kloelSettingsClass.outlineButton}>
                      Definir padrao
                    </Button>
                  )}
                  <Button variant="outline" onClick={() => void handleRemove(card.id)} className={kloelSettingsClass.dangerButton}>
      ...
      {showConfirmTrialModal && (
        <SettingsModal className="max-w-sm">
          ...
          <p className="mb-4 text-sm text-[#6E6E73]">
            Seu teste do Plano Basic sera iniciado agora. Durante o periodo de teste, o checkout e a cobranca dos seus clientes seguem operando pelo Kloel.
          </p>
          <div className="flex gap-2">
            <Button variant="outline" onClick={() => setShowConfirmTrialModal(false)} className={`flex-1 ${kloelSettingsClass.outlineButton}`}>
              Cancelar
            </Button>
        ...
        <SettingsModal className="max-w-sm">
          ...
          <div className="space-y-3">
            ...
            <div className="rounded-md border border-[#19191C] bg-[#0A0A0C] p-4 text-sm text-[#6E6E73]">
              O Kloel renova sua assinatura com base no cartao padrao cadastrado. Se precisar ajustar limite ou revisar falhas de cobranca, atualize o cartao antes do vencimento.
            </div>
          </div>
          <Button variant="outline" onClick={() => setShowManageModal(false)} className={`mt-4 w-full ${kloelSettingsClass.outlineButton}`}>
            Fechar
          </Button>
        </SettingsModal>
      )}
