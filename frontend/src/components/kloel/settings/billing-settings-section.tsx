...
import { apiFetch, billingApi, tokenStorage, type SalesReportSummary } from '@/lib/api';
...
const PLAN_DISPLAY: Record<string, { name: string; price: number }> = {
  ...
  PRO: { name: 'Pro', price: 297 },
  ENTERPRISE: { name: 'Enterprise', price: 997 },
...
export function BillingSettingsSection({
  ...
  const [planInfo, setPlanInfo] = useState<{ plan?: string; currentPeriodEnd?: string } | null>(
    null,
  );

  // Fetch real plan info from backend
  useEffect(() => {
    let cancelled = false;
    async function fetchPlan() {
      try {
        ...
        if (!cancelled && data) {
          setPlanInfo({ plan: data.plan, currentPeriodEnd: data.currentPeriodEnd });
        }
      } catch {
        // Silently fail — we fall back to "Sem plano ativo"
      }
    }
    void fetchPlan();
    return () => {
  ...
  const resolvedPlan = PLAN_DISPLAY[(planInfo?.plan || '').toUpperCase()] || null;
  const planName =
    resolvedPlan?.name || (subscriptionStatus === 'none' ? 'Sem plano ativo' : 'Plano Kloel');
  const planPrice = resolvedPlan?.price ?? null;
  ...
  const handleCancelSubscription = useCallback(async () => {
    ...
    setBillingError('');
    try {
      const wId = tokenStorage.getWorkspaceId();
      await apiFetch(`/billing/cancel?workspaceId=${encodeURIComponent(wId || '')}`, {
        method: 'POST',
      });
      ...
      try {
        const res = await billingApi.getSubscription();
        const data = res.data as Record<string, any> | undefined;
        if (data) setPlanInfo({ plan: data.plan, currentPeriodEnd: data.currentPeriodEnd });
      } catch {
        /* ignore */
      }
    } catch {
      setBillingError('Nao foi possivel cancelar a assinatura.');
      setShowCancelConfirm(false);
    } finally {
      setBillingLoading(false);
    }
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="mb-4 flex items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-2">
              <h4 className="text-lg font-semibold text-[#E0DDD8]">{planName}</h4>
          ...
          {planPrice !== null && planPrice > 0 ? (
            <div className="text-right">
              <p className="text-2xl font-bold text-[#E0DDD8]">R$ {planPrice}</p>
              <p className="text-xs text-[#6E6E73]">/mes</p>
            </div>
          ) : subscriptionStatus === 'none' ? (
            <div className="text-right">
              <p className="text-sm text-[#6E6E73]">--</p>
            </div>
          ) : (
            <div className="text-right">
              <p className="text-2xl font-bold text-[#E0DDD8]">R$ 0</p>
              <p className="text-xs text-[#6E6E73]">/mes</p>
            </div>
          )}
        </div>
      ...
      {showManageModal && (
        <SettingsModal className="max-w-sm">
          ...
          <div className="space-y-3">
            ...
            {subscriptionStatus !== 'none' && (
              <button
                className="flex w-full items-center justify-between rounded-md border border-[#E05252]/25 bg-[#0A0A0C] p-4 text-left transition-colors hover:border-[#E05252]/50 hover:bg-[#E05252]/5"
      ...
      {showCancelConfirm && (
        <SettingsModal className="max-w-sm">
          <h3 className="mb-2 text-lg font-semibold text-[#E0DDD8]">Tem certeza?</h3>
          <p className="mb-4 text-sm text-[#6E6E73]">
            Ao cancelar, sua assinatura sera encerrada ao final do periodo atual. Voce perdera
          ...
          <div className="flex gap-2">
            ...
            <Button
              ...
              className={`flex-1 ${kloelSettingsClass.dangerButton}`}
            >
              {billingLoading ? 'Cancelando...' : 'Confirmar cancelamento'}
            </Button>
          </div>
        </SettingsModal>
      )}
