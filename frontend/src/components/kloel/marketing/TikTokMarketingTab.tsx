...
export default function TikTokMarketingTab({ channelData }: TikTokMarketingTabProps) {
  ...
  const { tiktokStatus, isConnected, isLoading, refreshStatus, openTikTokConnect } = useTikTokMarketing();
  const [busyKind, setBusyKind] = useState<string | null>(null);
  const [message, setMessage] = useState<string | null>(null);
  const [tiktokMode, setTikTokMode] = useState<TikTokModeData | null>(null);

  const handleConnect = useCallback(async (kind: 'creator' | 'advertiser') => {
    setBusyKind(kind);
    setMessage(null);
    try { await openTikTokConnect(kind); } catch (error: unknown) {
      setBusyKind(null);
  ...
  useEffect(() => {
    ...
    async function fetchMode() {
      try {
        const response = await apiFetch<TikTokModeData>('/marketing/tiktok/mode');
        if (!cancelled && !response.error) {
          setTikTokMode(response.data || null);
        }
      } catch {
        // mode fetch is advisory — silence failures
      }
    }
    fetchMode();
      ...
      <div>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <ConnectedBadge />
            {tiktokMode ? <ModeBadge mode={tiktokMode.mode} /> : null}
          </div>
          <button type="button" onClick={() => refreshStatus()} style={{ fontFamily: SORA, fontSize: 12, padding: '6px 14px', borderRadius: 6, border: `1px solid ${ch.color}40`, background: `${ch.color}10`, color: ch.color, cursor: 'pointer' }}>Recarregar</button>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(200px,1fr))', gap: 10, marginBottom: 16 }}>
          {[{ label: 'Status', value: tiktokStatus.status || 'Ativo' }, { label: 'Tipo', value: tiktokStatus.kind || 'creator' }, { label: 'Open ID', value: tiktokStatus.openId || 'N/A' }].map((item) => (
        ...
        {channelData && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {[{ label: 'Mensagens', value: Fmt(channelData.messages) }, { label: 'Leads', value: Fmt(channelData.leads) }, { label: 'Vendas', value: channelData.sales.toString() }].map((s) => (
              <div key={s.label} style={rowBase}>
                <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: 3, background: ch.color }} />
                <span style={{ fontFamily: SORA, fontSize: 11, color: 'var(--app-text-secondary)', textTransform: 'uppercase', letterSpacing: '0.25em', minWidth: 120 }}>{s.label}</span>
                <span style={{ fontFamily: MONO, fontSize: 16, color: 'var(--app-text-primary)', flex: 1 }}>{s.value}</span>
              </div>
            ))}
          </div>
        )}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: 400, gap: 20 }}>
      ...
      <div style={{ fontFamily: SORA, fontSize: 22, color: 'var(--app-text-primary)' }}>{kloelT(`Conectar`)} {ch.label}</div>
      <div style={{ fontFamily: SORA, fontSize: 14, color: 'var(--app-text-secondary)', maxWidth: 420, textAlign: 'center', lineHeight: 1.6 }}>
        {isLoading ? 'Verificando status do TikTok...' : kloelT(`Conecte sua conta TikTok para operar campanhas e mensagens diretas dentro do Marketing da KLOEL.`)}
      </div>
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', justifyContent: 'center' }}>
        <button type="button" onClick={() => handleConnect('creator')} disabled={busyKind !== null || isLoading} style={{ ...btnStyle, background: ch.color }}>
          {busyKind === 'creator' ? 'Abrindo...' : 'Conectar conta TikTok'}
        </button>
        <button type="button" onClick={() => handleConnect('advertiser')} disabled={busyKind !== null || isLoading} style={secondaryBtnStyle}>
          {busyKind === 'advertiser' ? 'Abrindo...' : 'Conectar advertiser'}
        </button>
      </div>
      {message && (
        <div style={{ fontFamily: MONO, fontSize: 12, color: 'var(--app-text-secondary)', padding: '10px 16px', borderRadius: 6, background: BG_CARD, border: `1px solid ${BORDER}`, maxWidth: 420 }}>{message}</div>
      )}
      {tiktokMode ? <ModeBadge mode={tiktokMode.mode} /> : null}
      {channelData && (channelData.messages > 0 || channelData.leads > 0) && <RegisteredDataList channelData={channelData} color={ch.color} />}
    </div>
  );
}
...
function ModeBadge({ mode }: { mode: TikTokMode }) {
  const colorMap: Record<TikTokMode, { bg: string; fg: string; label: string }> = {
    sell: { bg: 'rgba(16,185,129,0.12)', fg: 'rgb(16,185,129)', label: 'Vendendo' },
    listen: { bg: 'rgba(245,158,11,0.12)', fg: 'rgb(245,158,11)', label: 'Recebendo' },
    blocked: { bg: 'rgba(239,68,68,0.12)', fg: 'rgb(239,68,68)', label: 'Bloqueado' },
  };
