...
import { buildConversationTacticPlan, assertConversationTacticPlan } from "../processors/cia/conversation-tactics";
...
describe("cia-conversation-tactics", () => {
  it("keeps an exhaustive tactic universe for executable actions", () => {
    const states = [
      ...
      { action: "ASK_CLARIFYING", state: buildState({ lastMessageText: "quero entender melhor antes" }) },
      { action: "SOCIAL_PROOF", state: buildState({ lastMessageText: "isso funciona mesmo? posso confiar?" }) },
      { action: "OFFER", state: buildState({ lastMessageText: "quero fechar hoje" }) },
      { action: "PAYMENT_RECOVERY", state: buildState({ lastMessageText: "me manda o pix novamente" }) },
      { action: "FOLLOWUP_SOFT", state: buildState({ unreadCount: 0, lastMessageText: "vou pensar" }) },
      { action: "FOLLOWUP_URGENT", state: buildState({ unreadCount: 0, lastMessageText: "preciso resolver hoje" }) },
    ] as const;
    ...
    for (const item of states) {
      ...
      expect(plan.candidates.some((candidate) => candidate.tactic === plan.selectedTactic)).toBe(true);
    }
  });
