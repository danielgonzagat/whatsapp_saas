...
import { getSharedReplyLockKey, findConversationAutomationState } from './backlog';
...
export async function checkScanPreFlight(params: {
  ...
  if (isWorkspaceSelfTarget({ phone, chatId, selfIdentity })) {
    ...
    return {
      skip: true,
      summary: 'O agente ignorou o próprio número da sessão.',
      replyLockKey: null,
    };
  }
  ...
  if (replyReserved !== 'OK') {
    return {
      skip: true,
      summary: 'Contato já está sendo respondido por outro pipeline.',
      replyLockKey: null,
    };
  }
...
export async function checkScanAutonomyBilling(params: {
  ...
  if (settings?.billingSuspended === true) {
    ...
    return {
      skip: true,
      summary: 'Billing suspenso. O contato não pode ser atendido automaticamente.',
    };
  }
