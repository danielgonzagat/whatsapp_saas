...
export async function dispatchAutopilotAction(input: DispatchInput): Promise<DispatchResult> {
  ...
  const displayName =
    contactName || (contactRecord?.name as string | undefined) || phone || 'contato';
  ...
  try {
    ...
    if (isAudioAction) {
      ...
      if (!audioSent) {
        const replyPlan = await dispatchAutonomousReplyPlan({
          ...
          mirrorReplies:
            deliveryMode === 'reactive' && isRecentLiveConversation(customerMessages || []),
        });
        executionResponse = {
    ...
    if (followupEligible && isExplicitProactiveOutreachAllowed(settings)) {
      await autopilotQueue.add(
        ...
        {
          delay: 45 * 60 * 1000,
          jobId: `followup-${contactId || phone}-bs`,
  ...
  const finalContactName =
    contactName || (contactRecord?.name as string | undefined) || displayName;
  await publishAgentEvent({
    type: 'contact',
    workspaceId,
    runId,
