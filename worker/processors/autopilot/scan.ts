...
export async function runScanContact(data: UnknownRecord) {
  ...
  const aggregated = await buildPendingMessageBatch({
    ...
    chatId: data?.chatId, fallbackMessageContent: data?.messageContent,
    selfIdentity,
  });
  ...
  try {
    ...
    const {
      ...
      messageContent, messageCount, messageIds, providerMessageIds, customerMessages,
    } = aggregated;
    const effectiveDeliveryMode: 'reactive' | 'proactive' =
      requestedDeliveryMode === 'reactive' ? 'reactive'
      : isRecentLiveConversation(customerMessages || []) ? 'reactive' : 'proactive';
    ...
    if (contactId && chatId) {
      const existingContact = await prisma.contact.findUnique({ where: { id: contactId }, select: { customFields: true } }).catch(() => null);
      const existingCustomFields = normalizeJsonObject(existingContact?.customFields);
      await prisma.contact.update({
        where: { id: contactId },
        data: { customFields: { ...existingCustomFields, lastRemoteChatId: chatId, lastResolvedChatId: chatId } },
    ...
    await reportSmokeTest(smokeTestId, { status: 'processing', workspaceId, contactId, phone, messageCount });

    await publishAgentEvent({
      type: 'thought', workspaceId, runId, phase: 'open_contact',
      message: `Abrindo conversa com ${contactName || phone}`,
      meta: { contactId, contactName, phone, backlogIndex: data?.backlogIndex, backlogTotal: data?.backlogTotal },
    });
    ...
    const cognitiveResult = await runScanCognitivePipeline({
      ...
      leadScore, conversationId: undefined, deliveryMode: effectiveDeliveryMode,
      settings, workspaceRecord: (workspace as UnknownRecord) || {},
    ...
    if (cognitiveResult.resolvedAction && cognitiveResult.resolvedText) {
      const sendResult = await sendDirectAutopilotText({
        workspaceId, contactId, phone, contactName,
        text: cognitiveResult.resolvedText, settings,
        intent: undefined, reason: 'cognitive_next_best_action',
        workspaceRecord: (workspace as UnknownRecord) || undefined,
        actionLabel: cognitiveResult.resolvedAction,
        usedHistory: true, usedKb: cognitiveResult.productMatches.length > 0,
        deliveryMode: effectiveDeliveryMode,
        smokeTestId, smokeMode, runId, customerMessages,
        idempotencyContext: {
          source: 'scan_contact_cognitive_action',
          action: cognitiveResult.resolvedAction,
          messageIds, providerMessageIds, runId: runId || null,
        },
      });
    ...
    const decisionResult = await runScanDecisions({
      workspaceId, contactId, phone, chatId, contactName,
      messageContent, messageCount, leadScore,
    ...
    if (finalStatus === 'sent') {
      await runPostSendCleanup(
        workspaceId, finalContactId, finalPhone, finalChatId,
        finalContactName, runId, replyLockKey, keepReplyLock);
    }
...
async function runPostSendCleanup(
  workspaceId: string, finalContactId?: string,
  finalPhone?: string, finalChatId?: string,
  finalContactName?: string, runId?: string,
  _replyLockKey?: string | null, _keepReplyLock?: boolean,
) {
  const finalContactRecord = finalContactId
    ? await prisma.contact.findUnique({ where: { id: finalContactId }, select: { id: true, name: true, customFields: true } }).catch(() => null)
    : null;
  const finalCustomFields = normalizeJsonObject(finalContactRecord?.customFields);

  if (finalPhone) {
    await ensureTrustedContactProfile({
      workspaceId, contactId: finalContactId, phone: finalPhone,
      chatId: String(finalChatId || '').trim() || String(finalCustomFields.lastRemoteChatId || '').trim() || String(finalCustomFields.lastCatalogChatId || '').trim() || String(finalCustomFields.lastResolvedChatId || '').trim() || undefined,
  ...
  if (finalContactId && finalPhone) {
    await Promise.resolve(
      autopilotQueue.add('score-contact', {
        workspaceId, contactId: finalContactId, phone: finalPhone,
        contactName: finalContactName, chatId: finalChatId || `${finalPhone}@c.us`,
  ...
  if (runId) {
    const runState = await finishBacklogRunTask({
      workspaceId, runId, contactId: finalContactId,
      contactName: finalContactName, phone: finalPhone,
      status: 'sent', summary: 'Resposta enviada.',
    });
