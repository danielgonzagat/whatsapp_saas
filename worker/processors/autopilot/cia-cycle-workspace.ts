...
async function publishCiaIdleResult(input: {
  ...
  const { state, exhaustionReport, cycleProofId, accountProofId, workspaceId, opportunityRefresh } =
    input;
...
export async function runCiaCycleWorkspace(workspaceId: string, presetSettings?: UnknownRecord) {
  ...
  await publishCiaProofEvent({
    ...
    candidateLength: state.candidates.length,
    exhaustionReport: exhaustionReport as never,
    cycleProofId,
    accountProofId,
    workspaceId,
    opportunityRefresh,
    actions: batch.actions as never,
    batchSize: batch.actions.length,
  });

  if (!batch.actions.length) {
    await publishCiaIdleResult({
      state,
      batch,
      guaranteeReport,
      exhaustionReport,
