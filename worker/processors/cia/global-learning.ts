...
export function anonymizeDecisionLog(input: {
  ...
  const createdAt = input.log?.createdAt
    ? new Date(input.log.createdAt)
    : new Date();
  const hour = Number.isFinite(createdAt.getUTCHours())
...
export function computeGlobalPatterns(
  ...
  return [...grouped.entries()]
    .map(([key, items]) => {
      ...
      const replied = items.filter((item) =>
        ["replied", "sold"].includes(item.outcome),
      ).length;
      ...
      const bucketFrequency = items.reduce<Record<string, number>>(
        (acc, item) => {
      ...
      const variantFrequency = items.reduce<Record<string, number>>(
        (acc, item) => {
      ...
      const preferredLength = (
        Object.entries(bucketFrequency).sort((left, right) => right[1] - left[1])[0]?.[0] ||
        "medium"
      ) as GlobalLearningPattern["preferredLength"];
      const preferredVariantFamily =
        Object.entries(variantFrequency).sort(
...
export function buildGlobalStrategy(input: {
  ...
  const pattern =
    input.patterns.find(
      (item) => item.domain === domainKey && item.intent === intentKey,
    ) ||
  ...
  return {
    ...
    confidence: Number(
      Math.min(pattern.samples / 25, 1).toFixed(3),
    ),
  };
}
