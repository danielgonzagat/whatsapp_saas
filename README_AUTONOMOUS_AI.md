...
- Checar status/QR com credenciais (cookies/JWT). QR deve aparecer após
  `initiate` .
...
- Checar filas: job em `flow-engine` ( `resume-flow` ), job em `autopilot-jobs`
  ( `scan-message` ).

### 4) Fluxos criados pela IA

- Após step 1, confirme `flow.triggerType`/`triggerCondition` e `isActive=true`.
- Simule palavra-chave (“comprar”) para acionar `flow-engine` ; ver execução e
  logs em Redis/DB.
