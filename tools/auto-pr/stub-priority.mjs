#!/usr/bin/env node
// tools/auto-pr/stub-priority.mjs — Wave 11 prep: prioritised stub backlog.
//
// Ranks the 121 stub routes detected by stub-route-detector.mjs using:
...
//
// Output: graphify-out/stub-priority.md — markdown table sorted by score, ready
// for human prioritisation. NOT auto-fix — each stub needs a template
// (Wave 11) before conversion can run through the L13 loop.
...
const STUBS_FILE = join(ROOT, 'graphify-out/stub-routes.json');
...
const OUT = join(ROOT, 'graphify-out/stub-priority.md');
...
async function main() {
  const { stubs } = JSON.parse(await readFile(STUBS_FILE, 'utf8'));
  ...
  const lines = [
    '# Stub Routes — Prioritised Backlog',
    '',
    `Generated by \`tools/auto-pr/stub-priority.mjs\` from \`graphify-out/stub-routes.json\` (${stubs.length} stubs) cross-referenced with the enriched graph (${graph.nodes?.length || 0} nodes) and AppShell sidebar.`,
    '',
    '## Scoring',
  ...
  lines.push('');
  lines.push('1. **P1 critical**: write template for the top-3 modules these stubs belong to (likely Auth/Workspaces/Settings). One template = N conversions.');
  ...
  console.log(`[stub-priority] wrote ${OUT}`);
  for (const [tier, items] of Object.entries(tierBuckets)) {
