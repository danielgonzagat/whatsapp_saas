...
export function writeOutputs({ OUT_DIR, domains, services, controllers, modules, processors, events, queues, routes, prismaUsage, symbols, fileCount, domainOf, capabilityMap, eventDuplicates, exactDuplicates }) {
// ─────────── write outputs ───────────
...
// 1) CANONICAL_DOMAINS.md
{
  ...
  const lines = [
    ...
    '',
    `Total domains: ${domains.size}. Total files scanned: ${fileCount}.`,
    '',
...
// 4) EVENT_TAXONOMY.md
{
  ...
  for (const [name, refs] of [...events.entries()].sort()) {
    ...
    const catalog = refs.filter((r) => r.kind === 'catalog').length;
    const catalogTag = catalog > 0 ? ` / ${catalog} catalog` : '';
    lines.push(`- \`${name}\` — ${emits} emit / ${listens} listen${catalogTag}`);
  }
