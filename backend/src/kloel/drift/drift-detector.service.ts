...
function computeDimensions(
  ...
  const decisionsSimilarity = jaccardSimilarity(previous.decisionsRanked, current.decisionsRanked);
  dims.push({
  ...
  const patScore = 1 - patternOverlapScore(previous.decisionPatterns, current.decisionPatterns);
  dims.push({
...
function stylesummary(tone: Record<ToneClass, number>): string {
  ...
  const dominant = ALL_TONES.reduce((best, t) => (tone[t] > tone[best] ? t : best));
...
function buildNarrative(
  ...
  for (const d of drifted) {
    switch (d.dimension) {
      case 'messagesSent':
        parts.push(`- Volume de mensagens: ${d.before} → ${d.after} (${deltaPct(d)}).`);
        break;
      case 'decisionsRanked':
        parts.push(`- Decisoes priorizadas: "${d.before}" → "${d.after}".`);
        break;
      case 'conversionsAttributed':
        parts.push(`- Conversoes: ${d.before} → ${d.after} (${deltaPct(d)}).`);
        break;
      case 'narrativeStyleHash':
        parts.push('- Estilo narrativo mudou significativamente.');
        break;
      case 'toneClassification':
        parts.push(`- Tom de comunicacao: ${d.before} → ${d.after}.`);
        break;
      case 'decisionPatterns':
        parts.push(`- Padroes de decisao: ${d.before} padroes → ${d.after} padroes.`);
        break;
    }
  }

  if (stable.length > 0) {
    parts.push(`Dimensoes estaveis: ${stable.map((d) => d.dimension).join(', ')}.`);
  }

  const mag = Math.round((drifted.length / DIMENSION_COUNT) * 100);
  parts.push(`Magnitude de drift: ${mag}%. Semana ${previous.weekStart} → ${current.weekStart}.`);
...
export class DriftDetectorService {
  compare(current: WeeklyBehaviorSnapshot, previous: WeeklyBehaviorSnapshot): DriftResult {
    ...
    const driftedDimensions = parts.filter((d) => d.drifted).map((d) => d.dimension);
