...
export function attributeHierarchy(decision: HierarchyDecision): HierarchyJustification {
  const deferred = (
    ...rules: Array<HierarchyJustification | null>
  ): HierarchyJustification | null => rules.find((r) => r !== null) ?? null;

  const formatLevel = (raw: string): HierarchyLevel => {
    if ((HIERARCHY_LEVELS as readonly string[]).includes(raw)) {
      return raw as HierarchyLevel;
    }
    return 'conversion';
  };
...
function ruleHumanTransferUx(decision: HierarchyDecision): HierarchyJustification | null {
  if (decision.type !== 'human_transfer') {
    return null;
  }
...
function ruleDiscountComplianceMargin(decision: HierarchyDecision): HierarchyJustification | null {
  if (decision.type !== 'coupon_offer' && decision.type !== 'apply_discount') {
    return null;
  }
  ...
  const marginRemaining = Number(
    decision.context.marginRemaining ?? decision.context.productMargin ?? -1,
  );
...
function ruleProactiveOutboundCompliance(
  decision: HierarchyDecision,
): HierarchyJustification | null {
  if (decision.type !== 'proactive_outbound' && decision.type !== 'channel_choice') {
    return null;
  }
...
function ruleRefundLegitimacy(decision: HierarchyDecision): HierarchyJustification | null {
  if (decision.type !== 'refund_request') {
    return null;
  }
  const concepts = extractConcepts(decision.context);
...
function ruleAntiRemorseRetention(decision: HierarchyDecision): HierarchyJustification | null {
  if (decision.type !== 'buyer_remorse') {
    return null;
  }
...
function rulePostSaleSatisfactionRetention(
  decision: HierarchyDecision,
): HierarchyJustification | null {
  if (decision.type !== 'post_sale_offer') {
    return null;
  }
...
function ruleAggressivenessCeilingUx(decision: HierarchyDecision): HierarchyJustification | null {
  ...
  const rank = (label: string): number => {
    if (label.includes('alta') || label.includes('agress') || label.includes('high')) {
      return 3;
    }
    if (label.includes('normal') || label.includes('moder') || label.includes('medium')) {
      return 2;
    }
    if (label.includes('baixa') || label.includes('low')) {
      return 1;
    }
    return 2;
  };
...
function ruleAudioPreferenceUx(decision: HierarchyDecision): HierarchyJustification | null {
  if (decision.type !== 'audio_vs_text') {
    return null;
  }
...
function ruleHighestMarginOffer(decision: HierarchyDecision): HierarchyJustification | null {
  if (decision.type !== 'product_offer') {
    return null;
  }
  const chosen = String(decision.chosen).toLowerCase();
...
function ruleAggressivenessEscalationConversion(
  ...
  const concepts = extractConcepts(decision.context);
  if (!concepts.has('imminent_purchase') && !concepts.has('hot_lead')) {
    return null;
  }
