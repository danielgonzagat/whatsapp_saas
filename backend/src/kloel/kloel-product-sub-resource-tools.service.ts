...
export class KloelProductSubResourceToolsService {
  ...
  async toolCreateCheckout(workspaceId: string, args: UnknownRecord) {
    try {
      ...
      const paymentMethods = Array.isArray(args.paymentMethods)
        ? args.paymentMethods.filter((method): method is string => typeof method === 'string')
        : ['card', 'pix', 'boleto'];
      const linkedPlanNames = Array.isArray(args.linkedPlanNames)
        ? args.linkedPlanNames.filter((name): name is string => typeof name === 'string')
        : [];
      const checkoutConfig: Prisma.InputJsonObject = {
        paymentMethods,
