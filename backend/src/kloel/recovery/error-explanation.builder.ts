...
> = {
  handoff: {
    ...
    why:
      'O limite de confiança configurado para resposta automática não ' +
  ...
  decline: {
    whatHappened: 'Um ou mais pagamentos foram recusados durante o processamento.',
    why:
      'Pagamentos podem ser recusados por restrições do emissor do ' +
      'cartão, saldo insuficiente, ou configuração de pagamento ' +
      'incompleta. O sistema detectou e registrou cada ocorrência.',
  },
  misclassification: {
    whatHappened: 'Leads foram classificados de forma que posteriormente exigiu ' + 'correção.',
    why:
      'Modelos de classificação operam com probabilidades. Quando ' +
      'eventos subsequentes contradizem a classificação original, ' +
      'o sistema registra a divergência para recalibração do modelo.',
  },
  missed_opportunity: {
    whatHappened: 'Leads que haviam sido contatados ficaram sem resposta ou foram ' + 'perdidos.',
    why:
      'A janela de follow-up pode ter sido insuficiente, o timing da ' +
      'mensagem inadequado, ou o lead perdeu interesse. O sistema ' +
      'detectou o padrão para ajuste de cadência.',
  },
  wrong_action: {
    whatHappened: 'Uma ação automática produziu resultado divergente do esperado.',
    why:
      'A decisão automática foi baseada em estado que mudou entre a ' +
      'decisão e a execução, ou o modelo interpretou incorretamente ' +
      'o contexto.',
  },
  double_send: {
    whatHappened: 'Uma mesma mensagem foi enviada mais de uma vez ao mesmo ' + 'destinatário.',
    why:
      'Condição de corrida entre processos paralelos ou retry sem ' +
      'verificação de idempotência. O sistema detectou a duplicação.',
  },
  delayed: {
    whatHappened: 'Uma resposta automática levou mais tempo que o esperado.',
    why:
      'Alta carga no provedor de IA, timeout de rede, ou fila de ' +
      'processamento acima da capacidade.',
  },
  inappropriate_timing: {
    whatHappened: 'Uma ação automática foi executada em horário inadequado.',
    why:
      'O detector de timing não bloqueou a ação por ausência de ' +
...
export function buildExplanation(error: DetectedError): ErrorExplanation {
  ...
  const certaintyLevel: 'high' | 'moderate' | 'low' =
    severity === 'high' ? 'high' : severity === 'medium' ? 'moderate' : 'low';
  return {
