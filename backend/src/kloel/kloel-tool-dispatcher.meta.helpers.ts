...
export function withCanonicalReceipt(
  deps: ProductDeps,
  capabilityId: string,
  workspaceId: string,
  args: UnknownRecord,
  ...
  const evidenceUrl = result.success
    ? buildReceiptEvidenceUrl(capabilityId, cap.evidenceUrlBuilder, outputs)
    : undefined;
...
export async function handleProductTool(
  ...
  switch (toolName) {
    case 'save_product':
    case 'create_product':
      return deps.chatToolsService.toolSaveProduct(
        workspaceId,
  ...
  switch (toolName) {
    ...
    case 'self.health': {
      ...
      return {
        ...
        outputs: await deps.selfHealth.snapshot(workspaceId),
      };
    }
...
export async function handleCodeAndReportTool(
  ...
  switch (toolName) {
    ...
    case 'list_source_dir':
      return deps.codeToolsService.toolListSourceDir(
    ...
    case 'code_detect_issues':
      return deps.codeAnalysisService.toolCodeDetectIssues(
        typeof args.path === 'string' ? args.path : '',
      );
    case 'codegraph_status':
      return deps.codeToolsService.toolCodeGraphStatus();
    case 'codegraph_search':
      return deps.codeToolsService.toolCodeGraphSearch(
        typeof args.query === 'string' ? args.query : '',
      );
    case 'codegraph_context':
      return deps.codeToolsService.toolCodeGraphContext(
        typeof args.task === 'string' ? args.task : 'overview',
      );
    case 'codegraph_callers':
      return deps.codeToolsService.toolCodeGraphCallers(
        typeof args.symbol === 'string' ? args.symbol : '',
      );
    case 'codegraph_callees':
      return deps.codeToolsService.toolCodeGraphCallees(
        typeof args.symbol === 'string' ? args.symbol : '',
      );
    case 'codegraph_impact':
      return deps.codeToolsService.toolCodeGraphImpact(
        typeof args.symbol === 'string' ? args.symbol : '',
      );
    case 'codegraph_node':
      return deps.codeToolsService.toolCodeGraphNode(
        typeof args.symbol === 'string' ? args.symbol : '',
      );
