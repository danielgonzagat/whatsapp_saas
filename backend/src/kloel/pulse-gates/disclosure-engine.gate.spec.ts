  ...
  it('4. both affiliate and commercial relationship fully disclosed passes', () => {
    const v = check(
      validInput({
        ...
        hasCommercialRelationship: true,
        disclosureStatement:
          'Kloel is an affiliate of Acme Tools and receives compensation for referrals.',
        commercialRelationship: {
          type: 'affiliate',
  ...
  it('6. commercial relationship with long disclosure and isPublic=true passes', () => {
    const v = check(
      validInput({
        hasCommercialRelationship: true,
        disclosureStatement:
          'Kloel maintains a strategic partnership with Acme Tools since March 2026.',
        commercialRelationship: {
          type: 'sponsorship',
  ...
  it('22. commercialRelationship with empty type and valid disclosure passes', () => {
    const v = check(
      validInput({
        hasCommercialRelationship: true,
        disclosureStatement:
          'Kloel maintains a commercial relationship with Acme Tools since 2025.',
        commercialRelationship: {
          type: '',
...
describe('disclosure-engine gate — mode contract', () => {
  ...
  it('25. FAIL verdict carries hard_fail mode when configured', () => {
    const v = makeDisclosureEngineGate('hard_fail').check(
      validInput({
        isKloelAffiliate: true,
      }),
    );
