...
export class KloelCodeToolsService {
  ...
  async toolSearchCodebase(pattern: string, glob?: string): Promise<ToolResult> {
    try {
      ...
      const stderrOut = typeof commandError.stdout === 'string' ? commandError.stdout : undefined;
      const stderrErr = typeof commandError.stderr === 'string' ? commandError.stderr : undefined;
