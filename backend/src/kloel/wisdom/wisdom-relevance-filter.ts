 ...
 *   - channel match:  0.20
 */
export function relevanceScore(taxonomy: PatternTaxonomy, target: TargetWorkspaceContext): number {
  let score = 0;
  ...
  if (target.ticketRange && taxonomy.tickethint) {
    if (target.ticketRange === taxonomy.tickethint || taxonomy.tickethint === 'all') {
      score += 0.25;
    }
  }
  ...
  if (target.activeChannels && taxonomy.channelHint) {
    if (target.activeChannels.includes(taxonomy.channelHint)) {
      score += 0.2;
    }
  }
