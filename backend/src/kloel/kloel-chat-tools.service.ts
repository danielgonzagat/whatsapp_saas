...
export class KloelChatToolsService {
  ...
  async toolUpdateProduct(
    workspaceId: string,
    args: { productId: string; name?: string; price?: number; description?: string; active?: boolean; imageUrl?: string; category?: string; format?: string; tags?: string[]; warrantyDays?: number; salesPageUrl?: string; thankyouUrl?: string; thankyouPixUrl?: string; thankyouBoletoUrl?: string; reclameAquiUrl?: string; supportEmail?: string; affiliateEnabled?: boolean; commissionPercent?: number },
  ): Promise<ToolResult> {
    const { productId, ...fields } = args;
    if (!productId) return { success: false, error: 'productId_required' };
    ...
    if (fields.warrantyDays !== undefined) updateData.warrantyDays = fields.warrantyDays;
    if (fields.salesPageUrl !== undefined) updateData.salesPageUrl = fields.salesPageUrl;
    if (fields.thankyouUrl !== undefined) updateData.thankyouUrl = fields.thankyouUrl;
    if (fields.thankyouPixUrl !== undefined) updateData.thankyouPixUrl = fields.thankyouPixUrl;
    if (fields.thankyouBoletoUrl !== undefined) updateData.thankyouBoletoUrl = fields.thankyouBoletoUrl;
    if (fields.reclameAquiUrl !== undefined) updateData.reclameAquiUrl = fields.reclameAquiUrl;
    if (fields.supportEmail !== undefined) updateData.supportEmail = fields.supportEmail;
    if (fields.affiliateEnabled !== undefined) updateData.affiliateEnabled = fields.affiliateEnabled;
    if (fields.commissionPercent !== undefined) updateData.commissionPercent = fields.commissionPercent;
    if (Object.keys(updateData).length === 0) return { success: false, error: 'no_fields_to_update' };
    const product = await this.prisma.product.update({
      where: { id: productId, workspaceId },
      data: updateData,
    }).catch(() => null);
    if (!product) return { success: false, error: 'product_not_found' };
    return { success: true, product: { id: product.id, name: product.name, price: product.price }, message: `Produto "${product.name}" atualizado.` };
  }

  async toolGetProductPlans(workspaceId: string, args: { productId: string }): Promise<ToolResult> {
    if (!args.productId) return { success: false, error: 'productId_required' };
    const product = await this.prisma.product.findFirst({ where: { id: args.productId, workspaceId }, select: { id: true, name: true, price: true } });
    if (!product) return { success: false, error: 'product_not_found' };
    const plans = await this.prisma.productPlan.findMany({
      where: { productId: args.productId },
      select: { id: true, name: true, price: true, itemsPerPlan: true, maxInstallments: true, active: true },
      orderBy: { price: 'asc' },
    });
    return { success: true, product: { id: product.id, name: product.name, price: product.price }, plans, count: plans.length };
  }

  async toolGetProductUrls(workspaceId: string, args: { productId: string }): Promise<ToolResult> {
    if (!args.productId) return { success: false, error: 'productId_required' };
    const product = await this.prisma.product.findFirst({ where: { id: args.productId, workspaceId }, select: { id: true, name: true, salesPageUrl: true, thankyouUrl: true, thankyouPixUrl: true, thankyouBoletoUrl: true, reclameAquiUrl: true, supportEmail: true, slug: true } });
    if (!product) return { success: false, error: 'product_not_found' };
    return { success: true, product: { id: product.id, name: product.name }, urls: { salesPageUrl: product.salesPageUrl, thankyouUrl: product.thankyouUrl, thankyouPixUrl: product.thankyouPixUrl, thankyouBoletoUrl: product.thankyouBoletoUrl, reclameAquiUrl: product.reclameAquiUrl, supportEmail: product.supportEmail, slug: product.slug } };
  }

  async toolGetProductReviews(workspaceId: string, args: { productId: string }): Promise<ToolResult> {
    if (!args.productId) return { success: false, error: 'productId_required' };
    const product = await this.prisma.product.findFirst({ where: { id: args.productId, workspaceId }, select: { id: true, name: true } });
    if (!product) return { success: false, error: 'product_not_found' };
    const reviews = await this.prisma.productReview.findMany({
      where: { productId: args.productId },
      select: { id: true, rating: true, comment: true, authorName: true, createdAt: true, verified: true },
      orderBy: { createdAt: 'desc' },
      take: 50,
    });
    return { success: true, product: { id: product.id, name: product.name }, reviews, count: reviews.length };
  }

  async toolGetProductAiConfig(workspaceId: string, args: { productId: string }): Promise<ToolResult> {
    if (!args.productId) return { success: false, error: 'productId_required' };
    const product = await this.prisma.product.findFirst({ where: { id: args.productId, workspaceId }, select: { id: true, name: true, aiConfig: true } });
    if (!product) return { success: false, error: 'product_not_found' };
    return { success: true, product: { id: product.id, name: product.name }, aiConfig: product.aiConfig || null };
  }

  async toolValidateCoupon(_workspaceId: string, args: { productId: string; code: string }): Promise<ToolResult> {
    if (!args.productId || !args.code) return { success: false, error: 'productId_and_code_required' };
    const coupon = await this.prisma.productCoupon.findFirst({
      where: { productId: args.productId, code: args.code, active: true },
    });
    if (!coupon) return { success: false, error: 'coupon_not_found_or_inactive', valid: false };
    const now = new Date();
    if (coupon.expiresAt && new Date(coupon.expiresAt) < now) return { success: true, valid: false, reason: 'expired', coupon: { code: coupon.code, discount: coupon.discountValue } };
    if (coupon.maxUses && coupon.usedCount >= coupon.maxUses) return { success: true, valid: false, reason: 'max_uses_reached', coupon: { code: coupon.code } };
    return { success: true, valid: true, coupon: { id: coupon.id, code: coupon.code, discountValue: coupon.discountValue, type: coupon.discountType } };
  }

  async toolGetAnalytics(workspaceId: string, args: { metric: string; period?: string }): Promise<ToolResult> {
    ...
    const [contactCount, _orderCount, orders] = await Promise.all([
      ...
      this.prisma.checkoutOrder.findMany({ where: { workspaceId, createdAt: { gte: since } }, select: { totalInCents: true, status: true } }),
    ]);
    ...
    return { success: true, period, metrics: { totalContacts: contactCount, ordersInPeriod: orders.length, paidOrders, totalRevenueCents: totalRevenue, conversionRate: contactCount > 0 ? ((paidOrders / contactCount) * 100).toFixed(1) + '%' : '0%' } };
  }

  async toolCreateBroadcast(workspaceId: string, args: { name: string; message: string; targetTags?: string[]; scheduleAt?: string }): Promise<ToolResult> {
    if (!args.name || !args.message) return { success: false, error: 'name_and_message_required' };
    ...
    return { success: true, campaign: { id: campaign.id, name: campaign.name, status: campaign.status }, message: `Campanha "${args.name}" criada.` };
  }

  async toolConfigureAiPersona(workspaceId: string, args: { name?: string; personality?: string; tone?: string; language?: string; useEmojis?: boolean }): Promise<ToolResult> {
    const persona = {
    ...
    await this.prisma.kloelMemory.upsert({
      ...
      update: { value: persona, category: 'preferences', type: 'persona', content: `Persona: ${persona.name}, Tom: ${persona.tone}`, metadata: persona },
      create: { workspaceId, key: 'aiPersona', value: persona, category: 'preferences', type: 'persona', content: `Persona: ${persona.name}, Tom: ${persona.tone}`, metadata: persona },
    });
