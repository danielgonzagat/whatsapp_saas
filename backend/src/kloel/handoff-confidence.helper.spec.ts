...
describe('Phase-2 flag-gated integration', () => {
  ...
  describe('Phase-3 blocking gate integration', () => {
    it('behaves as original when both flags are off', () => {
      ...
      const gateEnabled = process.env['HANDOFF_CONFIDENCE_GATE_ENABLED'] === 'true';
      const blockingEnabled = process.env['HANDOFF_CONFIDENCE_GATE_BLOCKING_ENABLED'] === 'true';
      ...
      const shouldEscalate =
        (gateEnabled || blockingEnabled) && blockingEnabled && snapshot.wouldEscalateAtThreshold04;
