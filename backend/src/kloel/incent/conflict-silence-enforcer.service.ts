...
export class ConflictSilenceEnforcerService {
  ...
  public enforceBatch(inputs: readonly SilenceInput[]): readonly SilenceUnderConflict[] {
    return inputs.map((inp) => this.enforce(inp));
  }

  public isSilenced(silence: SilenceUnderConflict, nowMs?: number): boolean {
    if (!silence.silenced) {
      return false;
    }
    if (!silence.expirationAt) {
      return true;
    }
  ...
  public silenceRate(inputs: readonly SilenceInput[]): {
  ...
  private evaluate(input: SilenceInput): boolean {
    if (input.conflict) {
      if (input.conflict.severity === 'structural') {
        return true;
      }
    ...
    if (input.biasDetected && !input.biasMitigated) {
      return true;
    }

    if (input.userOverridden) {
      return true;
    }
  ...
  private determineEnforcement(input: SilenceInput): 'automatic' | 'human_review_required' {
    if (input.conflict?.severity === 'structural') {
      return 'automatic';
    }
    if (input.biasDetected && !input.biasMitigated) {
