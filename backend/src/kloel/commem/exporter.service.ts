...
export class ExporterService {
  ...
  public export(projections: readonly MemoryProjection[], workspaceId: string): ExportedCapsule {
    this.capsuleVersion++;
    const exportedAt = new Date().toISOString();

    const sorted = [...projections].sort((a, b) => a.dimension.localeCompare(b.dimension));
    ...
    const checksum = simpleHash(
      `${workspaceId}:${exportedAt}:${this.capsuleVersion}:${contentForHash}`,
    );
  ...
  public batchExport(
    ...
    return Object.entries(projectionsByWorkspace).map(([wsId, projs]) => this.export(projs, wsId));
  }

  public verifyCapsule(capsule: ExportedCapsule): boolean {
    const sorted = [...capsule.projections].sort((a, b) => a.dimension.localeCompare(b.dimension));
