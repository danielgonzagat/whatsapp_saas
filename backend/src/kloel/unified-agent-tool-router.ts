...
export async function executeUnifiedAgentToolAction(params: {
  ...
  switch (tool) {
    ...
    case 'create_payment_link': {
      ...
      if (riskGate && paymentAmount > 0) {
        ...
        if (gateDecision.verdict === 'block') {
          logger.error(`R4 BLOCKED: create_payment_link — ${gateDecision.reason}`);
          return {
            success: false,
            blocked: true,
      ...
      const result = await actions.actionCreatePaymentLink(workspaceId, phone, args, context);
      try {
    ...
    case 'validate_coupon':
      return actions.validateCoupon(actions.str(args.productId), actions.str(args.code));
    case 'create_flow':
    ...
    case 'apply_discount':
      if (riskGate) {
        ...
        if (gateDecision.verdict === 'block') {
          logger.error(`R4 BLOCKED: apply_discount — ${gateDecision.reason}`);
          return {
            success: false,
            blocked: true,
            riskClass: gateDecision.classification.class,
