...
import {
  ...
  provenanceFor,
  validateSkill,
} from './agent-runtime.skill-registry.helpers';
import type {
...
export class AgentRuntimeSkillRegistry {
  ...
  async listSkills(workspaceId: string): Promise<AgentSkillDefinition[]> {
    ...
    const stored = rows
      .map((row) => parseSkill(row.value))
      .filter((skill): skill is AgentSkillDefinition => skill !== null);
    return mergeSkillsWithDefaults(DEFAULT_SKILLS, stored);
  }
  ...
  async upsertSkill(
    ...
    try {
      ...
      const existingSkill = existingRow ? parseSkill(existingRow.value) : null;
      ...
      const persistedContent = buildSkillContent(persistedSkill);
      const metadata = {
        kind: 'agent_skill',
        version,
      ...
      this.logger.warn(`Failed to upsert agent skill: ${messageFor(error)}`);
  ...
  async recordSkillUsage(
    ...
    if (!isUsageOutcome(params.outcome)) {
    ...
    try {
      ...
      const previous = existingRow ? parseUsageStats(existingRow.value, skillId) : null;
      const now = new Date().toISOString();
      const next: AgentSkillUsageStats = {
        skillId,
        provenance: provenanceFor(params.provenance ?? previous?.provenance),
        lifecycleState: lifecycleFor(params.lifecycleState ?? previous?.lifecycleState),
      ...
      this.logger.warn(`Failed to record agent skill usage: ${messageFor(error)}`);
  ...
  private async loadUsageStats(workspaceId: string): Promise<Map<string, AgentSkillUsageStats>> {
    ...
    for (const row of rows) {
      const parsed = parseUsageStats(row.value);
      if (parsed) {
