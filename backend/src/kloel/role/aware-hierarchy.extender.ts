...
export function roleAwareReTier(
  ...
  return weights.map((w) => {
    ...
    if (w.adjustedScore >= 0.75) {
      tier = 'AGORA';
    } else if (w.adjustedScore >= 0.5) {
      tier = 'ESTA_SEMANA';
    } else if (w.adjustedScore >= 0.25) {
      tier = 'PARA_SABER';
    } else {
      tier = 'ARQUIVO';
    }
...
export function countTierChanges(
  ...
  if (original.length !== reTiered.length) {
    return 0;
  }
  let changes = 0;
  for (let i = 0; i < original.length; i++) {
    const originalItem = original[i];
    const reTieredItem = reTiered[i];
    if (originalItem === undefined || reTieredItem === undefined) {
      continue;
    }
    if (originalItem.tier !== reTieredItem.adjustedTier) {
      changes++;
    }
  }
  return changes;
