/**
 * MindCommercialGraph — canonical name for the commercial knowledge graph
 * service (ADR-0013 Wave M1).
 *
 * Builds a weighted commercial graph from autopilot events / Mind beliefs /
 * Mind policies and persists it through `persistWorkspaceCommercialGraph`.
 * Backwards-compat alias `BrainCommercialGraphService` is preserved for the
 * existing DI tokens during the Wave M1 alias window (4 weeks).
 *
 * Legacy shim: `backend/src/kloel/brain-commercial-graph.service.ts` re-exports
 * `MindCommercialGraph` under the deprecated alias `BrainCommercialGraphService`.
 *
...
@Injectable()
export class MindCommercialGraph {
  private readonly logger = StructuredLogger.from(MindCommercialGraph.name);
...
/**
 * @deprecated Use {@link MindCommercialGraph}. Backwards-compat alias kept for
 * the ADR-0013 Wave M1 alias window (4 weeks). Remove after sweep.
 */
export { MindCommercialGraph as BrainCommercialGraphService };
