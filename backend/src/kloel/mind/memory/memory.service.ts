...
import {
  recallMemoryGraph,
  updateMemoryGraphNode,
  type MemoryGraphNodePatch,
} from './memory.service.graph';
...
export class MemoryService {
   ...
   * Implementation lives in `memory.service.graph.ts` (sibling-module pattern,
   * same as retrieval) so this service stays within the size guardrail.
   */
  async recallGraph(workspaceId: string, userId: string): Promise<MemoryGraphPayload> {
    return recallMemoryGraph({
      workspaceId,
      userId,
      prisma: this.prisma,
      logger: this.logger,
   ...
   * fresh graph. Implementation lives in `memory.service.graph.ts`.
   */
