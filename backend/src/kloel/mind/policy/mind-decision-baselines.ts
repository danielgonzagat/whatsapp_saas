...
export function resolveMessageFormatBaseline(channel: string, supports: string[]): string {
  if (channel === 'email' && supports.includes('html_rich')) {
    return 'html_rich';
  }
...
export function resolveObjectionResponseBaseline(concept: string, priceBand: string): string {
  ...
  if (normalized.includes('confian') || normalized.includes('trust')) {
    return 'social_proof';
  }
  if (normalized.includes('risco') || normalized.includes('garantia')) {
    return 'guarantee';
  }
  if (normalized.includes('premium') || normalized.includes('over_')) {
...
export function resolveCouponBaseline(priceBand: string, soldRate: number): string {
  ...
  if (priceBand === 'over_1000' && soldRate < 0.08) {
    return 'coupon_15';
  }
  if (highBands.has(priceBand) && soldRate < 0.1) {
    return 'coupon_10';
  }
  if (priceBand === 'over_100' && soldRate < 0.05) {
    return 'coupon_5';
  }
  return 'no_coupon';
}
...
export function resolveHumanTransferBaseline(
  ...
  if (ticketRisk >= 0.4) {
    return 'transfer_after_next_reply';
  }
  return 'continue_ai';
}
...
export function resolveProductOfferBaseline(
  ...
  if (segment === 'new_lead' || segment === 'cold') {
    return 'entry_product';
  }
  const highBands = new Set(['over_300', 'over_500', 'over_1000']);
  if (highBands.has(priceBand)) {
