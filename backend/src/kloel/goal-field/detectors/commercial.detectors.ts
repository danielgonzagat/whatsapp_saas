...
export const hotLeadWithoutResponseDetector: Detector = {
  ...
  detect: (events, nowMs) => {
    ...
    for (const [, list] of byEntity) {
      const lastReply = lastEventBefore(
        list,
        nowMs + 1,
        (e) => e.eventName === 'commerce.lead.replied',
      );
      if (!lastReply) {
        continue;
      }
      const replyMs = Date.parse(lastReply.occurredAt);
      if (nowMs - replyMs < HOT_LEAD_REPLY_RESPONSE_BUDGET_MS) {
        continue;
      }
      const ourReplyAfter = list.find(
        (e) =>
          e.eventName === 'commerce.whatsapp.message_replied' && Date.parse(e.occurredAt) > replyMs,
      );
      if (ourReplyAfter) {
        continue;
      }
...
export const abandonedCartDetector: Detector = {
  ...
  detect: (events, nowMs) => {
    ...
    for (const [, list] of byEntity) {
      ...
      if (nowMs - cartMs < ABANDONED_CART_WINDOW_MS) {
        continue;
      }
...
const clickedWithoutConversionDetector: Detector = {
  ...
  detect: (events, nowMs) => {
    ...
    for (const [, list] of byEntity) {
      ...
      if (nowMs - clickMs < CHANNEL_CLICK_TO_CONVERSION_BUDGET_MS) {
        continue;
      }
...
const dormantCustomerDetector: Detector = {
  ...
  detect: (events, nowMs) => {
    ...
    for (const [, list] of byEntity) {
      ...
      if (dormancyDays < DORMANT_CUSTOMER_DAYS) {
        continue;
      }
      out.push(
        makeTension(
...
const checkoutStartedWithoutPaymentDetector: Detector = {
  ...
  detect: (events, nowMs) => {
    ...
    for (const [, list] of byEntity) {
      ...
      if (nowMs - checkoutMs < 60 * 60 * 1000) {
        continue;
      } // wait 1h
      const pay = list.find(
        (e) =>
...
const leadWithoutFollowupDetector: Detector = {
  ...
  detect: (events, nowMs) => {
    ...
    for (const [, list] of byEntity) {
      ...
      if (nowMs - silenceMs < SILENCE_FOLLOWUP_BUDGET_MS) {
        continue;
      }
...
const performingAffiliateDetector: Detector = {
  ...
  detect: (events, nowMs) => {
    ...
    for (const e of perf) {
      ...
      if (!Number.isFinite(mag) || mag < 0.1) {
        continue;
      }
      out.push(
        makeTension(
