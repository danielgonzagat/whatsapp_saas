...
const queueBuildupDetector: Detector = {
  ...
  detect: (events, nowMs) => {
    const incoming = events.filter(
      (e) => e.eventName === 'commerce.whatsapp.message_received',
    ).length;
    const outgoing = events.filter(
      (e) => e.eventName === 'commerce.whatsapp.message_replied',
    ).length;
...
const repetitiveQuestionDetector: Detector = {
  ...
  detect: (events, nowMs) => {
    ...
    for (const e of events) {
      if (e.eventName !== 'commerce.whatsapp.message_received') {
        continue;
      }
      if (!e.entityRef) {
        continue;
...
const frictionAtConversionDetector: Detector = {
  ...
  detect: (events, nowMs) => {
    ...
    const declines = events.filter(
      (e) =>
        e.eventName === 'commerce.payment.declined' || e.eventName === 'commerce.cart.abandoned',
    );
...
const toneMismatchDetector: Detector = {
  ...
  detect: (events, nowMs) => {
    ...
    for (const e of events) {
      ...
      if (objKind !== 'tone' && objKind !== 'frustration') {
        continue;
      }
      out.push(
        makeT(
