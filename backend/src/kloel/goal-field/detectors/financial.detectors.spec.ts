...
import { FINANCIAL_DETECTORS } from './financial.detectors';
...
describe('Financial detectors', () => {
  it('exports FINANCIAL_DETECTORS array with detectors', () => {
    expect(FINANCIAL_DETECTORS).toBeDefined();
    expect(Array.isArray(FINANCIAL_DETECTORS)).toBe(true);
    expect(FINANCIAL_DETECTORS.length).toBeGreaterThan(0);
  });

  it('each detector has required name and dimension fields', () => {
    for (const d of FINANCIAL_DETECTORS) {
      expect(d.name).toBeDefined();
      expect(typeof d.name).toBe('string');
      expect(d.dimension).toBe('financial');
