...
const makeCustomState = (): AbiReadinessTruth => ({
...
describe('ReadinessTruthSnapshotService', () => {
  describe('default state (no injection)', () => {
    ...
    it('uses DEFAULT_STATE when undefined is explicitly passed', () => {
      const service = new ReadinessTruthSnapshotService(undefined);
      const result = service.snapshot();
    ...
    it('NestJS DI creates service with default state when token is absent', async () => {
      const module: TestingModule = await Test.createTestingModule({
        providers: [ReadinessTruthSnapshotService],
      }).compile();

      const service = module.get(ReadinessTruthSnapshotService);
      const result = service.snapshot();
  ...
  describe('injected state', () => {
    it('uses custom state injected via constructor argument', () => {
      ...
      const service = new ReadinessTruthSnapshotService(custom);
      const result = service.snapshot();
    ...
    it('NestJS DI injects custom state via ABI_READINESS_TRUTH_STATE token', async () => {
      const custom = makeCustomState();
      const module: TestingModule = await Test.createTestingModule({
        providers: [
          ReadinessTruthSnapshotService,
          { provide: 'ABI_READINESS_TRUTH_STATE', useValue: custom },
        ],
      }).compile();
