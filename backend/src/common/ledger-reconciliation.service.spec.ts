...
function makePrisma(overrides: LedgerPrismaOverrides = {}): LedgerPrismaMock {
  return {
    checkoutOrder: {
...
describe('LedgerReconciliationService — invariant I12 (wallet ledger consistency)', () => {
  ...
  describe('append-only invariant & double-entry balance', () => {
  ...
  describe('replay safety & transaction consistency', () => {
  ...
  describe('divergence reporting & audit trail', () => {
          ...
          data: {
            ...
            details: expect.objectContaining({
              ...
              sampleDrifts: expect.arrayContaining([
                expect.objectContaining({ details: { bucket: 'available' } }),
                expect.objectContaining({ details: { bucket: 'blocked' } }),
              ]),
            }),
          },
        }),
