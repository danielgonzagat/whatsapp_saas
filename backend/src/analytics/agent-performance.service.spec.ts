...
describe('AgentPerformanceService', () => {
  ...
  beforeEach(async () => {
    prisma = createPartialPrismaMock({
      message: ['groupBy', 'findMany'],
    });
    prisma.message.groupBy.mockResolvedValue([]);
    prisma.message.findMany.mockResolvedValue([]);
    const module: TestingModule = await Test.createTestingModule({
      providers: [AgentPerformanceService, { provide: PrismaService, useValue: prisma }],
    }).compile();
  ...
  it('computes real avg response time from inbound→outbound pairs', async () => {
    ...
    prisma.message.groupBy.mockResolvedValue([{ agentId: 'agent-1', _count: { id: 5 } }]);
    prisma.message.findMany.mockResolvedValue([
    ...
    expect(result).toEqual([{ agentId: 'agent-1', messageCount: 5, avgResponseTime: 30 }]);
  });

  it('drops responses with delta >= 24h (outlier filter)', async () => {
    const t0 = new Date('2026-01-01T00:00:00Z');
    const tNextDay = new Date('2026-01-02T01:00:00Z'); // 25h later
    prisma.message.groupBy.mockResolvedValue([{ agentId: 'agent-1', _count: { id: 1 } }]);
    prisma.message.findMany.mockResolvedValue([
      { id: 'm1', agentId: null, direction: 'INBOUND', createdAt: t0, conversationId: 'c1' },
      {
        id: 'm2',
  ...
  it('returns avgResponseTime=0 when there are no inbound→outbound pairs', async () => {
    prisma.message.groupBy.mockResolvedValue([{ agentId: 'agent-2', _count: { id: 10 } }]);
    prisma.message.findMany.mockResolvedValue([]);
    const result = await service.getAgentPerformance(
      'ws-1',
      new Date('2026-01-01'),
    ...
    expect(result).toEqual([{ agentId: 'agent-2', messageCount: 10, avgResponseTime: 0 }]);
  });
});
