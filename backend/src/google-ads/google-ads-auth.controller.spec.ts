...
describe('GoogleAdsAuthController', () => {
  ...
  describe('getConnectUrl', () => {
    it('returns auth URL for google platform', async () => {
      getConnectUrl.mockResolvedValue({ authUrl: 'https://accounts.google.com/o/oauth2/v2/auth?...' });
  ...
  describe('oauthCallback', () => {
    it('returns error when oauth error param present', async () => {
      const result = await controller.oauthCallback(mockRequest('ws-1'), undefined, 'access_denied');
      expect(result.data.connected).toBe(false);
      expect(result.data.status).toBe('oauth_error');
    });

    it('throws BadRequestException when no code provided', async () => {
      await expect(controller.oauthCallback(mockRequest('ws-1'), undefined, undefined)).rejects.toThrow(BadRequestException);
    });

    it('completes OAuth and returns result', async () => {
      completeOAuth.mockResolvedValue({ connected: true, status: 'connected' });
      const result = await controller.oauthCallback(mockRequest('ws-1'), 'auth-code-123', undefined);
  ...
  describe('getStatus', () => {
    it('returns google platform status', async () => {
      getPlatformStatuses.mockResolvedValue([
        { platform: 'meta', connected: true, status: 'connected', accountId: '', clientConfigured: true },
        { platform: 'google', connected: false, status: 'disconnected', accountId: '', clientConfigured: true },
      ]);
