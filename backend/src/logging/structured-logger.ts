...
export class StructuredLogger extends Logger {
  ...
  private normalize(
    ...
    if (typeof a === 'string') {
      ...
      if (typeof b === 'string') {
        return { message: a, extra: { context: b } };
      }
      if (typeof b === 'object') {
        return { message: a, extra: b };
      }
  ...
  info(a: string | LogExtra, b?: string | LogExtra | unknown): void {
    ...
    const { message, extra } = this.normalize(a, b as string | LogExtra | undefined);
