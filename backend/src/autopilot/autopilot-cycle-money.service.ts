...
export class AutopilotCycleMoneyService {
  ...
  async moneyMachine(workspaceId: string, topN = 200, autoSend = false, useSmartTime = false) {
    ...
    const operation = 'autopilot/money-machine';
    try {
      if (!this.isLegacyExecutionEnabled()) {
        ...
        this.logger.log(
          ...
          'Autopilot money-machine succeeded (disabled)',
        );
        return result;
      }
      ...
      this.logger.log(
        {
          workspaceId,
          campaignCount: createdIds.length,
          operation,
          durationMs: Date.now() - startedAt,
          status: 'ok',
        },
        'Autopilot money-machine succeeded',
      );
      return result;
    } catch (error: unknown) {
      this.logger.error(
        ...
        'Autopilot money-machine failed',
      );
      throw error;
    }
  }
  ...
  async nextBestAction(workspaceId: string, contactId: string) {
    const startedAt = Date.now();
    const operation = 'autopilot/next-best-action';
    try {
      ...
      this.logger.error(
        ...
        error instanceof Error ? error.stack : undefined,
        'Autopilot next-best-action failed',
      );
      throw error;
    }
  }
