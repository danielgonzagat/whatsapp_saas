...
function resolveTextLlmBaseUrl(config?: ConfigLike): string | undefined {
  const provider = resolveTextLlmProvider(config);
  if (provider === 'deepseek') {
    return readFirstConfig(['DEEPSEEK_BASE_URL', 'LLM_BASE_URL'], config) || DEFAULT_DEEPSEEK_BASE_URL;
  }
  if (provider === 'generic') {
    return readFirstConfig(['LLM_BASE_URL', 'DEEPSEEK_BASE_URL'], config) || DEFAULT_DEEPSEEK_BASE_URL;
  }
  if (provider === 'openai') {
