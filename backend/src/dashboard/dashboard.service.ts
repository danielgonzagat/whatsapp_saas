...
export class DashboardService {
  ...
  async getStats(workspaceId: string) {
    return this.cache.wrap(
      `cache:dashboard:stats:${workspaceId}`,
      async () => {
  ...
  async getHomeSnapshot(
    ...
    const cacheKey = `cache:dashboard:home:${workspaceId}:${input?.period ?? 'default'}:${input?.startDate ?? ''}:${input?.endDate ?? ''}`;
    return this.cache.wrap(
      cacheKey,
      async () => {
        ...
        const allOrdersSeries = countByBuckets(
          currentOrders,
          range.buckets,
          (row) => row.createdAt,
        );
            ...
            checkpoints: [
              {
                ...
                description:
                  'O workspace precisa registrar ao menos uma venda paga dentro do recorte.',
