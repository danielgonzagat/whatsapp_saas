...
export class WhatsAppDispatchAdapter implements ChannelDispatchPort {
  ...
  private buildOpts(input: WhatsAppSendInput): WhatsappSendOpts | undefined {
    ...
    if (input.mediaUrl !== undefined) {
      o.mediaUrl = input.mediaUrl;
    }
    if (input.mediaType !== undefined) {
      o.mediaType = input.mediaType;
    }
    if (input.caption !== undefined) {
      o.caption = input.caption;
    }
    if (input.quotedMessageId !== undefined) {
      o.quotedMessageId = input.quotedMessageId;
    }
    if (input.externalId !== undefined) {
      o.externalId = input.externalId;
    }
    if (input.complianceMode !== undefined) {
      o.complianceMode = input.complianceMode;
    }
    if (input.forceDirect !== undefined) {
      o.forceDirect = input.forceDirect;
    }
  ...
  private mapResult(
    ...
    if (!success) {
      ...
      if (typeof obj.blocked === 'boolean') {
        failure.blocked = obj.blocked;
      }
      if (typeof obj.blockedReason === 'string') {
        failure.blockedReason = obj.blockedReason;
      }
      return failure;
    }
    const messageId =
    ...
    if (input.externalId) {
      ok.externalId = input.externalId;
    } else if (messageId) {
      ok.externalId = messageId;
    }
    return ok;
  }
}
