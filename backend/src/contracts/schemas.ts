 ...
 * the backend must produce and the frontend must consume. It is
 * mirrored byte-for-byte at:
 *
 *   frontend/src/__tests__/contracts/schemas.ts  (PR P1-2)
 *
 * A CI script (scripts/ops/check-contract-sync.mjs) enforces that the
 * two files are identical. Any divergence between them is a bug, by
 * definition: the contract has only one valid form.
 *
