...
export function normalizeRecurringAmountToMonthlyCents(amount: number, interval: string): number {
  ...
  if (normalized === 'YEARLY' || normalized === 'ANNUAL') return Math.round(cents / YEARLY_MONTHS);
  if (normalized === 'WEEKLY') return Math.round(cents * WEEKS_PER_MONTH);
  if (normalized === 'DAILY') return Math.round(cents * DAYS_PER_MONTH);
  if (normalized === 'QUARTERLY') return Math.round(cents / QUARTERLY_MONTHS);
  if (normalized === 'SEMIANNUAL') return Math.round(cents / SEMIANNUAL_MONTHS);
  return cents;
}
