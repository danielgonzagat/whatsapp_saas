...
type WahaMessagePayload = { _data?: { key?: { remoteJidAlt?: string; remoteJid?: string }; [key: string]: unknown }; key?: { remoteJidAlt?: string; remoteJid?: string }; [key: string]: unknown };
...
export class WhatsappCatchupHistoryService {
  ...
  private normalizePhone(phone: string): string { return normalizePhoneExt(phone); }
  private normalizeTimestamp(value?: Date | string | number | null): Date | null { return normalizeTimestampExt(value); }
  private normalizeJsonObject(value: unknown): NormalizedJsonObj { return normalizeJsonObjExt(value); }
  ...
  private async getLidPnMap(ws: string): Promise<Map<string, string>> {
    return getLidPnMapExt({ providerRegistry: this.providerRegistry }, ws, this.lidMapCacheTtlMs, this.lidMapCache);
  }

  private async resolveCanonicalPhone(ws: string, chatId: string): Promise<string> {
    return resolveCanonicalPhoneExt({ providerRegistry: this.providerRegistry }, ws, chatId, this.lidMapCacheTtlMs, this.lidMapCache);
  }
  ...
  resolveRemoteContactName(chat: WahaChatSummary): string {
    ...
    for (const c of [chat?.name, chat?.contact?.pushName, chat?.contact?.name, chat?.pushName, chat?.notifyName, chat?.lastMessage?._data?.notifyName, chat?.lastMessage?._data?.verifiedBizName]) {
  ...
  resolveBackfillCursor(sm: { backfillCursor?: unknown; [key: string]: unknown }): CatchupBackfillCursor {
    ...
    return { chatId: cid, activityTimestamp: at, updatedAt: normalizeTimestampExt(c.updatedAt as string | number | Date | null | undefined)?.toISOString() || new Date(at).toISOString() };
  }

  rotateFallbackChatsByCursor(chats: WahaChatSummary[], cursor?: CatchupBackfillCursor): WahaChatSummary[] {
    if (!cursor || !chats.length) return chats;
    const i = chats.findIndex((c) => c.id === cursor.chatId);
    if (i >= 0) { const s = (i + 1) % chats.length; return s === 0 ? chats : [...chats.slice(s), ...chats.slice(0, s)]; }
    const ai = chats.findIndex((c) => this.resolveChatActivityTimestamp(c) < cursor.activityTimestamp);
    if (ai > 0) return [...chats.slice(ai), ...chats.slice(0, ai)];
    return chats;
  }

  isRemoteChatAwaitingReply(chat: WahaChatSummary): boolean { return chat.lastMessageFromMe === false; }
  ...
  extractSenderName(payload: WahaMessagePayload | null | undefined): string | undefined {
    ...
    const cs: unknown[] = [data?.pushName, payload?.pushName, data?.notifyName, payload?.notifyName, payload?.author, payload?.senderName];
  ...
  resolvePreferredChatId(payload: WahaMessagePayload | null | undefined): string | null {
    ...
    const candidates = [dk?.remoteJidAlt, pk?.remoteJidAlt, payload?.remoteJidAlt, payload?.chatId, payload?.from, dk?.remoteJid, pk?.remoteJid, payload?.to]
      .filter((c) => typeof c === 'string').map((c) => String(c).trim()).filter(Boolean);
    return !candidates.length ? null : candidates.find((c) => !c.includes('@lid')) || candidates[0] || null;
  }

  toInboundMessage(ws: string, m: WahaChatMessage, provider: InboundMessage['provider'] = 'meta-cloud'): InboundMessage | null {
    const pid = String(m.id || '').trim();
    const from = String(m.from || m.chatId || '').trim();
    if (!pid || !from) return null;
    return {
      workspaceId: ws, provider, ingestMode: 'catchup',
      createdAt: this.normalizeTimestamp(m.timestamp),
      providerMessageId: pid, from, to: m.to,
      senderName: this.extractSenderName(m.raw),
      type: this.mapInboundType(m.type),
      text: m.body, mediaUrl: m.mediaUrl, mediaMime: m.mimetype, raw: m.raw,
    } as InboundMessage;
  }
  ...
  async persistHistoricalOutboundMessage(ws: string, message: WahaChatMessage): Promise<boolean> {
    ...
    try {
      await this.inbox.saveMessageByPhone({ workspaceId: ws, phone, content: message.body || '', direction: 'OUTBOUND', externalId: pid, type: this.mapInboundType(message.type).toUpperCase(), mediaUrl: message.mediaUrl, status: 'READ', createdAt: this.normalizeTimestamp(message.timestamp), countAsUnread: false, resetUnreadOnOutbound: false, silent: true } as Parameters<typeof this.inbox.saveMessageByPhone>[0]);
      return true;
    } catch (e: unknown) {
  ...
  async reconcileRemoteChatState(ws: string, chat: WahaChatSummary): Promise<void> {
    ...
    if (!phone) return;
    const ec = await this.prisma.contact.findUnique({ where: { workspaceId_phone: { workspaceId: ws, phone } }, select: { id: true, name: true, customFields: true } });
    const ecf = this.normalizeJsonObject(ec?.customFields);
    const erp = safeStr(ecf.remotePushName).trim();
    const esn = String(ec?.name || '').trim();
    const rpn = this.resolveRemoteContactName(chat) || (!this.isPlaceholderContactName(erp, phone) ? erp : '') || null;
    ...
    const contact = await this.prisma.contact.upsert({
      ...
      update: { name: cn, customFields: JSON.parse(JSON.stringify({ ...ecf, remotePushName: rpn || undefined, remotePushNameUpdatedAt: rpn ? new Date().toISOString() : ecf.remotePushNameUpdatedAt || undefined, lastRemoteChatId: cid, lastResolvedChatId: rcid || cid })) as Prisma.InputJsonObject },
    ...
    const exC = await this.prisma.conversation.findFirst({ where: { workspaceId: ws, contactId: contact.id }, orderBy: { updatedAt: 'desc' }, select: { id: true, unreadCount: true, lastMessageAt: true } });
    if (!exC) {
      await this.prisma.conversation.create({ data: { workspaceId: ws, contactId: contact.id, status: 'OPEN', priority: 'MEDIUM', channel: 'WHATSAPP', mode: 'AI', unreadCount: Math.max(0, Number(chat.unreadCount || 0) || 0), lastMessageAt: rAt || new Date() } });
      return;
    }
    const clm = exC.lastMessageAt instanceof Date ? exC.lastMessageAt : this.normalizeTimestamp(exC.lastMessageAt);
    await this.prisma.conversation.updateMany({
      where: { id: exC.id, workspaceId: ws },
      data: { unreadCount: Math.max(0, Number(exC.unreadCount || 0) || 0, Number(chat.unreadCount || 0) || 0), lastMessageAt: rAt && (!clm || rAt > clm) ? rAt : clm || new Date() },
    });
  }
  ...
  async sanitizePlaceholderContacts(ws: string): Promise<void> {
    ...
    const contacts = await this.prisma.contact.findMany({
      take: 5000, where: { workspaceId: ws },
      select: { id: true, phone: true, name: true, customFields: true, _count: { select: { messages: true, conversations: true, deals: true, executions: true, autopilotEvents: true, insights: true } } },
    ...
    await forEachSequential(contacts, async (contact) => {
      ...
      const tn = (!this.isPlaceholderContactName(rp, contact.phone) ? rp : '') || (!this.isPlaceholderContactName(sn, contact.phone) ? sn : '');
      const hp = this.isPlaceholderContactName(sn, contact.phone) || this.isPlaceholderContactName(rp, contact.phone);
      if (!hp) return;
      const ncf = { ...cf };
      const rc = Number(contact._count?.messages || 0) + Number(contact._count?.conversations || 0) + Number(contact._count?.deals || 0) + Number(contact._count?.executions || 0) + Number(contact._count?.autopilotEvents || 0) + Number(contact._count?.insights || 0);
      if (this.isPlaceholderContactName(rp, contact.phone)) { delete ncf.remotePushName; delete ncf.remotePushNameUpdatedAt; }
      else if (tn) { ncf.remotePushName = tn; ncf.remotePushNameUpdatedAt = ncf.remotePushNameUpdatedAt || new Date().toISOString(); }
      ...
      await this.prisma.contact.updateMany({ where: { id: contact.id, workspaceId: ws }, data: { name: tn || null, customFields: ncf as Prisma.InputJsonValue } });
    });
  }
}
