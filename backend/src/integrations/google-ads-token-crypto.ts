...
import { handleMissingTokenCryptoKey } from './token-crypto-shared.helper';
...
function resolveKeyForVersion(version: number): Buffer | null {
  const envMap: Record<number, string> = {
    1: 'GOOGLE_ADS_TOKEN_ENCRYPTION_KEY',
  };
...
export function encryptGoogleAdsToken(token: string | null | undefined): string | null | undefined {
  if (!token) {
    return token;
  }
...
export function decryptGoogleAdsToken(token: string | null | undefined): string | null {
  if (!token) {
    return null;
  }
...
export function isEncryptedGoogleAdsToken(token: string): boolean {
