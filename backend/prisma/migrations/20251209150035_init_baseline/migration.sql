-- PULSE_SAFE: Static DDL migration generated by Prisma, no dynamic SQL injection possible.
--
-- RAC_ PREFIX NOTE: This is the INITIAL baseline migration (2025-12-09).
-- All tables, indexes, and FK constraints reference unprefixed table names
-- ("Workspace", "Agent", ...). The RAC_ prefix was applied in a later
-- forward-only rename migration: 20260425013841_rac_table_rename.
--
-- The Semgrep "rac-table-access" rule correctly flags unprefixed references
-- as HIGH severity, but these are CORRECT for this historical migration
-- because the tables did not carry the RAC_ prefix at this point in time.
-- Changing the table names here would break the DDL for any database that
-- runs this migration before the rename migration.
--
-- DO NOT edit table/constraint names in this file. If a rule-flagged line
-- here corresponds to a real problem (e.g. missing index on a FK), create
...
-- ==========================================================================
-- AddForeignKey — Historical baseline FK constraints.
--
-- All table references below use the ORIGINAL unprefixed table names.
-- These were correct at baseline time (2025-12-09). The RAC_ prefix
-- was applied by migration 20260425013841_rac_table_rename, which
-- renamed every legacy table to its RAC_<Name> equivalent.
--
-- Semgrep rac-table-access flags every line in this section because
-- none references RAC_* tables. This is a HISTORICAL FALSE POSITIVE.
-- Fixing it would require rewriting FK targets to RAC_* names, which
-- breaks the DDL contract: at the point this migration runs, the
-- referenced tables do NOT yet carry the RAC_ prefix.
--
-- If a real issue exists (e.g. missing performance index), create a
-- NEW migration — never alter this baseline's constraint targets.
-- ==========================================================================
