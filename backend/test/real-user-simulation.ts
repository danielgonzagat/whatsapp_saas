...
async function runTest(name: string, fn: () => Promise<string>) {
  ...
  try {
    ...
    results.push({ name, passed: false, details: err.message || String(err), duration: Date.now() - start });
...
async function main() {
  ...
  await runTest('1. Register new user', async () => {
    ...
    assert(status === 200 || status === 201, `Register failed: ${status} ${JSON.stringify(data)}`);
  ...
  await runTest('2. Login with credentials', async () => {
    const { status, data } = await api('POST', '/auth/login', { email: EMAIL, password: PASSWORD });
    assert(status === 200 || status === 201, `Login failed: ${status} ${JSON.stringify(data)}`);
  ...
  await runTest('3. KYC status is pending', async () => {
    ...
    assert(status === 200, `KYC status failed: ${status} ${JSON.stringify(data)}`);
  ...
  await runTest('5. Complete KYC and auto-approve', async () => {
    ...
    await api('PUT', '/kyc/profile', { name: 'Kloel Tester Bot', phone: '+5511999999999', birthDate: '1990-01-15' }, token);
    await api('PUT', '/kyc/fiscal', { documentType: 'CPF', documentNumber: '12345678900', legalName: 'Kloel Tester Bot', cep: '01001000', street: 'Rua Teste', number: '100', city: 'Sao Paulo', state: 'SP', neighborhood: 'Centro' }, token);
    await api('POST', `/kloel/wallet/${workspaceId}/bank-accounts`, { bankCode: '001', bankName: 'Banco do Brasil', agency: '1234', accountNumber: '56789-0', accountType: 'checking', holderName: 'Kloel Tester Bot', holderDocument: '12345678900' }, token);
  ...
  await runTest('6. Create product', async () => {
    const { status, data } = await api('POST', '/products', {
      name: PRODUCT_NAME,
      description: 'Creme hidratante facial com acido hialuronico e vitamina C. Resultados visiveis em 15 dias.',
      price: 89.90,
      category: 'Cosmeticos',
      format: 'PHYSICAL',
    ...
    assert(status === 200 || status === 201, `Create product failed: ${status} ${JSON.stringify(data)}`);
  ...
  await runTest('8. KloelMemory has product', async () => {
    ...
    const { status, data } = await api('POST', '/kloel/think/sync', {
      message: 'Liste meus produtos cadastrados',
    }, token);

    if (status === 200 && data.response) {
      const knows = data.response.toLowerCase().includes(PRODUCT_NAME.toLowerCase().split(' ')[0]);
  ...
  await runTest('9. Update product', async () => {
    ...
    assert(status === 200 || status === 201, `Update failed: ${status} ${JSON.stringify(data)}`);
  ...
  await runTest('10. Create checkout product', async () => {
    const { status, data } = await api('POST', '/checkout/products', {
      productId: productId,
      name: PRODUCT_NAME,
      slug: `checkout-${TS}`,
  ...
  await runTest('11. Create checkout plan', async () => {
    ...
    const { status, data } = await api('POST', `/checkout/products/${checkoutProductId}/plans`, {
      name: '1 Unidade',
      slug: planSlug,
      priceInCents: 9990,
      maxInstallments: 12,
  ...
  await runTest('12. Create order (public checkout)', async () => {
    ...
    const { status, data } = await api('POST', '/checkout/public/order', {
      ...
      totalInCents: 9990,
      shippingAddress: {
        street: 'Rua das Flores', number: '456',
        neighborhood: 'Jardim', city: 'Sao Paulo',
  ...
  await runTest('15. Sales stats', async () => {
    ...
    assert(data.totalRevenue !== undefined, 'Stats should have totalRevenue field');
  ...
  await runTest('16. Wallet balance', async () => {
    const { status, data } = await api('GET', `/kloel/wallet/${workspaceId}/balance`, undefined, token);
    assert(status === 200, `Wallet failed: ${status} ${JSON.stringify(data)}`);
    assert(typeof data.pending === 'number', 'Wallet should have pending field');
  ...
  await runTest('17. Wallet transactions', async () => {
    const { status, data } = await api('GET', `/kloel/wallet/${workspaceId}/transactions`, undefined, token);
  ...
  await runTest('18. Reports: vendas summary', async () => {
    const { status, data } = await api('GET', '/reports/vendas/summary', undefined, token);
  ...
  await runTest('19. Reports: metricas', async () => {
    const { status, data } = await api('GET', '/reports/metricas', undefined, token);
  ...
  await runTest('20. Reports: origem', async () => {
    const { status, data } = await api('GET', '/reports/origem', undefined, token);
    assert(status === 200, `Origem failed: ${status}`);
    return `Origem: ${JSON.stringify(data).slice(0, 200)}`;
  });

  await runTest('21. Reports: churn', async () => {
    const { status, data } = await api('GET', '/reports/churn', undefined, token);
  ...
  await runTest('22. Generate order alerts', async () => {
    const { status, data } = await api('POST', '/sales/orders/alerts/generate', undefined, token);
    return `Generate: ${status} ${JSON.stringify(data).slice(0, 200)}`;
  });

  await runTest('23. List order alerts', async () => {
    const { status, data } = await api('GET', '/sales/orders/alerts', undefined, token);
  ...
  await runTest('24. Create ad rule', async () => {
    const { status, data } = await api('POST', '/ad-rules', {
      name: 'Test Rule',
      condition: 'ROAS caiu abaixo de 2.0',
      action: 'Reduzir budget em 30%',
      active: true,
    }, token);
    assert(status === 200 || status === 201, `Ad rule failed: ${status} ${JSON.stringify(data)}`);
  ...
  await runTest('26. Create member area', async () => {
    const { status, data } = await api('POST', '/member-areas', {
  ...
  await runTest('27. Marketing stats', async () => {
    const { status, data } = await api('GET', '/marketing/stats', undefined, token);
    assert(status === 200, `Marketing stats failed: ${status}`);
    return `Stats: ${JSON.stringify(data).slice(0, 200)}`;
  });

  await runTest('28. Marketing channels', async () => {
    const { status, data } = await api('GET', '/marketing/channels', undefined, token);
  ...
  await runTest('29. Marketing AI brain', async () => {
    const { status, data } = await api('GET', '/marketing/ai-brain', undefined, token);
  ...
  await runTest('30. Create chat thread', async () => {
    const { status, data } = await api('POST', '/kloel/threads', { title: 'Test Thread' }, token);
    if (status === 200 || status === 201) {
  ...
  await runTest('32. Search threads by content', async () => {
    const { status, data } = await api('GET', '/kloel/threads/search?q=test', undefined, token);
  ...
  await runTest('33. Partnership affiliates', async () => {
    const { status, data } = await api('GET', '/partnerships/affiliates/stats', undefined, token);
  ...
  await runTest('34. Register ad spend', async () => {
    const { status, data } = await api('POST', '/reports/ad-spend', {
  ...
  const warnings = results.filter(r => r.passed && r.details.includes('WARNING')).length;
  ...
  if (warnings > 0) {
    ...
    results.filter(r => r.details.includes('WARNING')).forEach(r => {
  ...
  results.forEach(r => {
    console.log(`  ${r.passed ? '✅' : '❌'} ${r.name} (${r.duration}ms) — ${r.details.slice(0, 120)}`);
  });
