# PULSE REPORT — 2026-04-29T21:23:45.927Z
...
- execution_matrix: PASS - Execution matrix classified 4034 path(s) with zero unknown and zero non-terminal paths.
- critical_path_terminal: PASS - All critical matrix paths are observed pass/fail or carry a precise terminal reason in PULSE_EXECUTION_MATRIX.json. 2682 terminal critical path(s) still need observed proof; next ai_safe action is to run the path validation command from PULSE_EXECUTION_MATRIX.json and refresh PULSE_PATH_COVERAGE.json.
...
- self_trust: PASS - All parsers loaded and no phantom capability/flow remains. 10 aspirational structure(s) remain explicitly marked as aspirational.
...
- Capabilities: real=112, partial=190, latent=12, phantom=0
- Capability maturity: foundational=4, connected=71, operational=186, productionReady=53
- Flows: real=23, partial=95, latent=0, phantom=0
- Execution matrix: paths=4034, observedPass=17, observedFail=232, criticalUnobserved=0, unknown=0
- Structural parity gaps: total=47, critical=44, high=3
...
- Structural Graph Coverage: 39% (814/2093 connected)
  Reason: 814/2093 structural files connected.
- Test Coverage: 11%
  Reason: 173/1612 source modules have spec files.
...
- Production Proof Coverage: 36%
  Reason: 112/314 capabilities real.
- Unknown Files: 68
- Orphan Files: 200
- Excluded Directories: 18
...
- codacy/static_hotspot: impact=80%, mode=ai_safe, mappedCapabilities=76, mappedFlows=112, summary=2 HIGH Codacy issue(s) remain in backend/src/auth/email.service.ts.
...
- Current checkpoint: The current product-facing system materializes 101 real capability(ies), 190 partial capability(ies), 0 latent capability(ies), and 0 product-facing phantom capability(ies). System-wide phantom capability count is 0.
...
- Distance: Distance to projected readiness is driven by 0 product-facing phantom capability(ies), 0 system-wide phantom capability(ies), 0 phantom flow(s), 47 structural parity gap(s), and 1116 HIGH Codacy issue(s).
...
- Followups: status=real, completion=74%, capabilities=66, flows=112, blocker=Admin Accounts remains partial.
...
- Marketing: status=real, completion=70%, capabilities=67, flows=112, blocker=Admin Accounts remains partial.
